...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_ramal_horario"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["id_ramal_horario"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"sip_buddies_horario_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("sip_buddies_horario_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=sip_buddies_horario.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=sip_buddies_horario.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=sip_buddies_horario.xml");
	...
	{
		$values = array();
			$values["id_ramal_horario"] = GetData($row,"id_ramal_horario","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=sip_buddies_horario.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["id_ramal_horario"] = htmlspecialchars(GetData($row,"id_ramal_horario",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_ramal_horario"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["id_ramal_horario"] = GetData($row,"id_ramal_horario",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["id_ramal_horario"]);
			echo '</td>';
							echo '<td>';
