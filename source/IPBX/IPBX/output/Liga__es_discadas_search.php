...
include("include/Liga__es_discadas_variables.php");
...
$mode=SEARCH_SIMPLE;
$templatefile = "Liga__es_discadas_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "Liga__es_discadas_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'Liga__es_discadas';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "Graf. Lig. Discadas";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("Período");
	...
	{
		$firstFieldParams['fName'] = "Período";
	...
	if(isEnableSection508())
		$xt->assign_section("Per_odo_label","<label for=\"".GetInputElementId("Período", $id)."\">","</label>");
	else 
		$xt->assign("Per_odo_label", true);
	
	$xt->assign("Per_odo_fieldblock", true);		
	$xt->assignbyref("Per_odo_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Per_odo_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Per_odo_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Per_odo", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("Período", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("Período")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	...
	{
	$contents_block["begin"].="action=\"Liga__es_discadas_chart.php\" ";
	}
