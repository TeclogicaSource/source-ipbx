...
$pageObject->body["begin"] .="<form action=\"Rel__Produt_filas_grupo_por_mes_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel__Produt_filas_grupo_por_mes_export.htm");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Produt_filas_grupo_por_mes.xml");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Produt_filas_grupo_por_mes.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values["nm_grupo"] = DisplayLookupWizard("nm_grupo",$row["nm_grupo"],$row,"",MODE_EXPORT);
			...
			$format="HTML";
			$values["Aband.Real."] = htmlspecialchars(GetData($row,"Aband.Real.",$format));
			$format="";
			$values["NivelServicoEsperaContratado"] = htmlspecialchars(GetData($row,"NivelServicoEsperaContratado",$format));
			$format="HTML";
			$values["NivelServicoEsperaRealizado"] = htmlspecialchars(GetData($row,"NivelServicoEsperaRealizado",$format));
			$format="";
			...
			$format="HTML";
			$values["NivelServicoAtendimentoRealizado"] = htmlspecialchars(GetData($row,"NivelServicoAtendimentoRealizado",$format));
			$format="";
			...
			$format="HTML";
			$values["NivelServicoOperacaoRealizado"] = htmlspecialchars(GetData($row,"NivelServicoOperacaoRealizado",$format));
			$format="";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Abd.Contr.").'</td>';	
