...
$templatefile = ( $inlineedit ) ? "ramal_autorizado_inline_edit.htm" : "ramal_autorizado_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_ramal"]=postvalue("editid1");
$savedKeys["id_ramal"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_ramal"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ramal_autorizado_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ramal_autorizado->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ramal_autorizado_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_ramal"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_ramal", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ramal_autorizado_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_ramal"))
		$key[]=GetFieldIndex("id_ramal");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ramal_autorizado_edit.php','".$skeys."','',function(){window.location.href='ramal_autorizado_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ramal_autorizado_edit.php','".$skeys."','',function(){window.location.href='ramal_autorizado_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_ramal"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_ramal"]));
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ramal_autorizado_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
