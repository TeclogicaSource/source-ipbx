...
include("include/sip_users_horario_variables.php");
...
if($inlineadd==ADD_INLINE)
	$templatefile = "sip_users_horario_inline_add.htm";
else
	$templatefile = "sip_users_horario_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'sip_users_horario', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
if(@$_POST["a"]=="added")
{
	...
	$files_move=array();
	$files_save=array();
//	processing dsc_plano - start
    
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
	$value = postvalue("value_dsc_plano_".$id);
	$type=postvalue("type_dsc_plano_".$id);
	if (FieldSubmitted("dsc_plano_".$id))
	{
		$value=prepare_for_db("dsc_plano",$value,$type);
	}
	else
		$value=false;
	if(!($value===false))
	{
		...
		if(0 && "dsc_plano"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["dsc_plano"]=$value;
	}
	}
//	processibng dsc_plano - end
//	processing id - start
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="horario")
	{
		if(!@$_SESSION[$sessionPrefix."_masterkey1"] && postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		if($avalues["id_horario"]=="")
			$avalues["id_horario"]=prepare_for_db("id_horario",$_SESSION[$sessionPrefix."_masterkey1"]);
	}
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'sip_users_horario_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'sip_users_horario_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: sip_users_horario_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	if(array_key_exists("copyid1",$_REQUEST))
	{
		...
		$copykeys["id_horario"]=postvalue("copyid2");
	}
	else
	{
		$copykeys["id"]=postvalue("editid1");
		$copykeys["id_horario"]=postvalue("editid2");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="horario")
{
	if(!@$_SESSION[$sessionPrefix."_masterkey1"] && postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
	$defvalues["id_horario"]=@$_SESSION[$sessionPrefix."_masterkey1"];	
}

if($readavalues)
{
	$defvalues["id"]=@$avalues["id"];
	$defvalues["dsc_plano"]=@$avalues["dsc_plano"];
}
//for basic files
$includes="";
...
if($inlineadd!=ADD_INLINE)
{
	...
	$xt->assign("dsc_plano_fieldblock",true);
	$xt->assign("dsc_plano_label",true);
	if(isEnableSection508())
		$xt->assign_section("dsc_plano_label","<label for=\"".GetInputElementId("dsc_plano", $id)."\">","</label>");
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='sip_users_horario_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="sip_users_horario_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="sip_users_horario_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$keylink.="&key2=".htmlspecialchars(rawurlencode(@$data["id_horario"]));
...
//	id - 
	...
	{	
		$value="";
				$value=DisplayLookupWizard("id",$data["id"],$data,$keylink,MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "id";
...
//	dsc_plano - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"dsc_plano", ""),"field=dsc%5Fplano".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dsc_plano";
				$showRawValues[] = substr($data["dsc_plano"],0,100);
	}	
//	hr_inicio - Time
	$display = false;
	if($inlineadd==ADD_INLINE)
...
{
				$value=DisplayLookupWizard("id",$defvalues["id"],$defvalues,"",MODE_VIEW);
		$xt->assign("id_editcontrol",$value);
}
// add prevent submit on enter js if only one text record
//	control - dsc_plano
$control_dsc_plano=array();
$control_dsc_plano["func"]="xt_buildeditcontrol";
$control_dsc_plano["params"] = array();
$control_dsc_plano["params"]["field"]="dsc_plano";
$control_dsc_plano["params"]["value"]=@$defvalues["dsc_plano"];

//	Begin Add validation
$arrValidate = array();	
$control_dsc_plano["params"]["validate"]=$arrValidate;
//	End Add validation

$control_dsc_plano["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dsc_plano["params"]["mode"]="inline_add";
else
	$control_dsc_plano["params"]["mode"]="add";

if(!$detailKeys || !in_array("dsc_plano", $detailKeys))
	$xt->assignbyref("dsc_plano_editcontrol",$control_dsc_plano);
else if(array_key_exists("dsc_plano", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dsc_plano", ""),"field=dsc%5Fplano","",MODE_VIEW);
		$xt->assign("dsc_plano_editcontrol",$value);
}
// add prevent submit on enter js if only one text record
