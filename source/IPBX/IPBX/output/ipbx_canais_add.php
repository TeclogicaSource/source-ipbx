...
include("include/ipbx_canais_variables.php");
...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_canais_inline_add.htm";
else
	$templatefile = "ipbx_canais_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$files_move=array();
	$files_save=array();
//	processing canal - start
    
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
	$value = postvalue("value_canal_".$id);
	$type=postvalue("type_canal_".$id);
	if (FieldSubmitted("canal_".$id))
	{
		$value=prepare_for_db("canal",$value,$type);
	}
	else
		$value=false;
	if(!($value===false))
	{
		...
		if(0 && "canal"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["canal"]=$value;
	}
	}
//	processibng canal - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="ipbx_interf_fxs")
	{
		if(!@$_SESSION[$sessionPrefix."_masterkey1"] && postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		if($avalues["id_interf"]=="")
			$avalues["id_interf"]=prepare_for_db("id_interf",$_SESSION[$sessionPrefix."_masterkey1"]);
	}
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="ipbx_interf_gsm")
	{
		if(!@$_SESSION[$sessionPrefix."_masterkey1"] && postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		if($avalues["id_interf"]=="")
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_canais_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_canais_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_canais_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_canal"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_canal"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="ipbx_interf_fxs")
{
	if(!@$_SESSION[$sessionPrefix."_masterkey1"] && postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
	$defvalues["id_interf"]=@$_SESSION[$sessionPrefix."_masterkey1"];	
}
if(@$_SESSION[$sessionPrefix."_mastertable"]=="ipbx_interf_gsm")
{
	if(!@$_SESSION[$sessionPrefix."_masterkey1"] && postvalue("masterkey1"))
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_canais_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_canais_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_canais_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
//	show readonly fields
	$readonlyfields["canal"] = htmlspecialchars(GetData($defvalues,"canal", ""));
$linkdata="";
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_canal"]);

	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_canal"]));
...
//	canal - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"canal", ""),"field=canal".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "canal";
				$showRawValues[] = substr($data["canal"],0,100);
	}	
//	dsc_interf - 
	$display = false;
	if($inlineadd==ADD_INLINE)
...
//	control - canal
$control_canal=array();
$control_canal["func"]="xt_buildeditcontrol";
$control_canal["params"] = array();
$control_canal["params"]["field"]="canal";
$control_canal["params"]["value"]=@$defvalues["canal"];

//	Begin Add validation
$arrValidate = array();	
$control_canal["params"]["validate"]=$arrValidate;
//	End Add validation

$control_canal["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_canal["params"]["mode"]="inline_add";
else
	$control_canal["params"]["mode"]="add";

if(!$detailKeys || !in_array("canal", $detailKeys))
	$xt->assignbyref("canal_editcontrol",$control_canal);
else if(array_key_exists("canal", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"canal", ""),"field=canal","",MODE_VIEW);
		$xt->assign("canal_editcontrol",$value);
}
