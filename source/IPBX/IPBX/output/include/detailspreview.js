...
return false;if($(tr).attr("rowid"))
return $(tr).attr("rowid");else
return $(tr).attr("id").substring(7);}
this.getCurrentTr=function(rPrm)
{var trParents=$(rPrm.linkId,this.prm.table).parents("tr");if(!trParents.length)
...
{var rowId=this.getRowId(rPrm);if(!rowId)
return false;var colsCount=new Array();var start=0,myplace=0,myPlaceFound=false;var tr=this.getCurrentTr(rPrm);if(!tr)
return false;var trChildren=$(tr).children("td");var tdParents=$(rPrm.linkId,this.prm.table).parents("td");if(!tdParents.length)
{if(this.isShowErrorHappend)
this.showErrorHappend(rPrm,"Couldn't get parent elements TD for clicked link preview");return false;}
for(i=0;i<trChildren.length;i++){if(!myPlaceFound){for(j=0;j<tdParents.length;j++){if(tdParents[j]==trChildren[i]){myPlace=colsCount.length;myPlaceFound=true;break;}}}
...
$(trChildren[i]).before("<td id=\"dpreview_"+this.prm.mSTable+"_"+(recId+i-myPlace)+"\" valign=\"top\" colspan="+colsCount[i]+"></td>");if(i)
$(trChildren[i-1]).after("<td id=\"dpreview_"+this.prm.mSTable+"_"+(recId+i-myPlace)+"\" valign=\"top\" colspan="+colsCount[i]+"></td>");else
$(previewTr).html("<td id=\"dpreview_"+this.prm.mSTable+"_"+(recId+i-myPlace)+"\" valign=\"top\" colspan="+colsCount[i]+"></td>");return true;}
this.createPreviewTr=function(rPrm)
{var rowId=this.getRowId(rPrm);if(!rowId)
return false;if(!$("#dpreviewrow_"+this.prm.mSTable+"_"+rowId,this.prm.table)[0])
{if(!this.cloneTRforPreview(rPrm))
return false;}
else{var dpInline=this;$("a[id$=_preview"+rPrm.recId+"]",dpInline.prm.table).each(function()
{if(this!=$(rPrm.linkId,dpInline.prm.table)[0])
...
{var table=getTableObj(this.prm.pageId);if(!table)
return false;this.prm.table=table;var linkId="#"+link.id;var dCaption=$(link).attr('caption');var id=++flyid;var rPrm={'id':id,'recId':recId,'link':link,'linkId':linkId,'dTable':dTable,'dCaption':dCaption};if(!this.createPreviewTr(rPrm))
return;var tdPreview=$("#dpreview_"+this.prm.mSTable+"_"+recId,this.prm.table)[0];var mData=this.getMasterData(link);if(!mData)
{if(this.isShowErrorHappend)
...
{var dPreview=$("#dpreview_"+this.prm.mSTable+"_"+rPrm.recId,this.prm.table)[0];if(dPreview)
{hide=this.getHtmlForHide(rPrm);var tdPreview=dPreview;var io=this.createPreviewIframe(rPrm);var form=this.createPreviewForm(rPrm);}
else{if(this.isShowErrorHappend)
this.showErrorHappend(rPrm,"Couldn't find element TD for preview content");return;}}
else{if($("#dpreview_"+this.prm.mSTable+"_"+rPrm.recId,this.prm.table).length)
...
dPreview=loadedContent;$(dPreview).empty();$(dPreview).html(html);if(this.prm.mode=='list_details')
{var dpRecords=$("#dpRecords"+id)[0];if(dpRecords&&dpRecords.offsetWidth>tdPreview.offsetWidth)
$(tdPreview).attr('width',dpRecords.offsetWidth+(dpRecords.offsetWidth-tdPreview.offsetWidth));}
var table=getTableObj(id);if($.browser.msie&&table)
...
{return'<div align="left" style="float:left;"><img id="dpClose'+rPrm.id+'" src="images/search/closeRed.gif" valign="middle" style="cursor:pointer;margin-right:10px;">'+'<a href="'+$(rPrm.linkId,this.prm.table).attr('href')+'">'+window.TEXT_PROCEED_TO+' '+rPrm.dCaption+'</a></div>';}
this.setLinkForHide=function(rPrm,linkId)
{var dpInline=this;$(linkId,this.prm.table)[0].onclick=function()
...
{var tdPreview=$("#dpreview_"+this.prm.mSTable+"_"+rPrm.recId,this.prm.table);if(!tdPreview)
{if(this.isShowErrorHappend)
this.showErrorHappend(rPrm,"Couldn't get preview content");return;}
$(tdPreview).empty();$(tdPreview).css('border','none');var trParents=$(tdPreview).parents("tr");if(!trParents.length)
{if(this.isShowErrorHappend)
this.showErrorHappend(rPrm,"Couldn't get parent elements TR for clicked link hide");return;}
var previewTr=trParents[0];var trPreviewRemove=true;var trChildren=$(previewTr).children("td[id^=dpreview_"+this.prm.mSTable+"_]");if(!trChildren.length)
{if(this.isShowErrorHappend)
...
{if(trChildren[i].innerHTML.length)
{trPreviewRemove=false;break;}}
var dpInline=this;$(rPrm.linkId,this.prm.table)[0].onclick=function()
{dpInline.showDPInline(rPrm.dTable,rPrm.recId,this);return false;};if(trPreviewRemove)
{$(previewTr).remove();var frameId=this.getIdForIframe(rPrm);removeFlyFrame=$("#"+frameId)[0];removeForm=$("#frmAdmin"+rPrm.id)[0];setTimeout('$(removeFlyFrame).remove(); $(removeForm).remove();',100);}}
...
mKeys=rPrm.mKeys;else{var mData=this.getMasterData($(rPrm.linkId,this.prm.table)[0]);mKeys=mData.keys;}
...
{txt='<b>Error</b>: '+txt;var dPreview=$("#dpreview_"+this.prm.mSTable+"_"+rPrm.recId,this.prm.table)[0];var detPreview=$("#detailPreview"+this.prm.pageId)[0];if(!dPreview&&!detPreview)
{if(rPrm.link)
