...
{var num_cell=0,id_cell=0;var fields={fields:[]};var myColumnDefs=new Array();var initCookies=new Array();var notInclTd=new Array();var trClasses=new Array();var listIcons=false;var colRowSpan=false;var aligns={th:[],td:[]};var styles={tr:[],td:[]};$("#"+table+" tr:eq(0)").find('th').each(function()
...
return false;}
function setTHTDAlign(id,aligns,styles,listIcons)
{var table=getTableObj(id);if(!table)
return false;$('th',table).each(function(i)
{this.align=aligns.th[i]});$('tr[@id^=yui-rec]',table).each(function(i)
{$(this).attr('style',styles.tr[i]);$('td[@class^=yui-dt]',this).each(function(j)
{this.align=aligns.td[j];$(this).attr('style',styles.td[j]);if(!j&&listIcons)
...
createTable($(old_table).parent().attr('id'),id_table,param.tName,param.id,param.firstTime);if(param.useInlineAdd&&param.permisAdd)
