...
$templatefile = "ipbx_painel_op_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_painel_op"]=postvalue("editid1");
$savedKeys["id_painel_op"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_painel_op"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_painel_op_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_painel_op->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_painel_op_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_painel"]=$evalues["dsc_painel"];
	$data["privilegios"]=$evalues["privilegios"];
	$data["usuarios"]=$evalues["usuarios"];
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_painel_op"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_painel_op", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_painel_op_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_painel_op"))
		$key[]=GetFieldIndex("id_painel_op");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_painel_op_edit.php','".$skeys."','',function(){window.location.href='ipbx_painel_op_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_painel_op_edit.php','".$skeys."','',function(){window.location.href='ipbx_painel_op_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_painel_op"]);
if($message)
{
...
$control_dsc_painel["params"]["value"]=@$data["dsc_painel"];
...
$control_dsc_painel["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_painel["params"]["mode"]="inline_edit";
else
	$control_dsc_painel["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_painel", $detailKeys))	
	$xt->assign("dsc_painel_editcontrol",$control_dsc_painel);
else if(array_key_exists("dsc_painel",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_painel", ""),"field=dsc%5Fpainel","",MODE_VIEW);
...
$control_privilegios["params"]["value"]=@$data["privilegios"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_privilegios["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_privilegios["params"]["mode"]="inline_edit";
else
	$control_privilegios["params"]["mode"]="edit";
if(!$detailKeys || !in_array("privilegios", $detailKeys))	
	$xt->assign("privilegios_editcontrol",$control_privilegios);
else if(array_key_exists("privilegios",$data))
{
				$value=DisplayLookupWizard("privilegios",$data["privilegios"],$data,"",MODE_VIEW);
...
$control_usuarios["params"]["value"]=@$data["usuarios"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_usuarios["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_usuarios["params"]["mode"]="inline_edit";
else
	$control_usuarios["params"]["mode"]="edit";
if(!$detailKeys || !in_array("usuarios", $detailKeys))	
	$xt->assign("usuarios_editcontrol",$control_usuarios);
else if(array_key_exists("usuarios",$data))
{
				$value=DisplayLookupWizard("usuarios",$data["usuarios"],$data,"",MODE_VIEW);
...
{
				$value=DisplayLookupWizard("fila",$data["fila"],$data,"",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_painel_op_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
