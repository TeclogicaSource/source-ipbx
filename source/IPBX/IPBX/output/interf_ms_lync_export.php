...
$pageObject->body["begin"] .="<form action=\"interf_ms_lync_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("interf_ms_lync_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=interf_ms_lync.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=interf_ms_lync.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=interf_ms_lync.xml");
	...
	{
		$values = array();
			$values["id_ms_lync"] = GetData($row,"id_ms_lync","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=interf_ms_lync.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["id_ms_lync"] = htmlspecialchars(GetData($row,"id_ms_lync",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_ms_lync"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Ms Lync").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["id_ms_lync"] = GetData($row,"id_ms_lync",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["id_ms_lync"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
