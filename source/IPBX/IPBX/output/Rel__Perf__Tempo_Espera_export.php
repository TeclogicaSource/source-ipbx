...
include("include/Rel__Perf__Tempo_Espera_variables.php");
...
$pageObject->body["begin"] .="<form action=\"Rel__Perf__Tempo_Espera_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel__Perf__Tempo_Espera_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Perf__Tempo_Espera.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Perf__Tempo_Espera.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Perf__Tempo_Espera.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Tempo em faixas"] = GetData($row,"Tempo em faixas","");
			$values["Quantidade"] = GetData($row,"Quantidade","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Perf__Tempo_Espera.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values["Fila"] = DisplayLookupWizard("Fila",$row["Fila"],$row,"",MODE_EXPORT);
			...
			$format="";
			$values["Tempo em faixas"] = htmlspecialchars(GetData($row,"Tempo em faixas",$format));
			$format="";
			$values["Quantidade"] = htmlspecialchars(GetData($row,"Quantidade",$format));
			$format="Number";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Tempo em faixas"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Quantidade"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Tempo em faixas").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Quantidade").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["Tempo em faixas"] = GetData($row,"Tempo em faixas",$format);
					
							$format="";
			
			$values["Quantidade"] = GetData($row,"Quantidade",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["Tempo em faixas"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="";
									echo htmlspecialchars($values["Quantidade"]);
			echo '</td>';
							echo '<td>';
