...
$pageObject->body["begin"] .="<form action=\"Rel__Detalh__Chamadas_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel__Detalh__Chamadas_export.htm");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Detalh__Chamadas.xml");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Detalh__Chamadas.csv");
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Terminado").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Agente").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Data ").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Entrada").'</td>';	
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Obs."]);
					else
