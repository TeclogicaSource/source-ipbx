...
include("include/cadeado_variables.php");
...
$templatefile = ( $inlineedit ) ? "cadeado_inline_edit.htm" : "cadeado_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["idt_cadeado"]=postvalue("editid1");
$savedKeys["idt_cadeado"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if(0 && "Ramal"=="" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["Ramal"]=$value;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idt_cadeado"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: cadeado_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: cadeado_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["idt_cadeado"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"idt_cadeado", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="cadeado_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("idt_cadeado"))
		$key[]=GetFieldIndex("idt_cadeado");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('cadeado_edit.php','".$skeys."','',function(){window.location.href='cadeado_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('cadeado_edit.php','".$skeys."','',function(){window.location.href='cadeado_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["idt_cadeado"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["idt_cadeado"]));


//	idt_cadeado - 

		$value="";
				$value = ProcessLargeText(GetData($data,"idt_cadeado", ""),"field=idt%5Fcadeado".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "idt_cadeado";
				$showRawValues[] = substr($data["idt_cadeado"],0,100);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('cadeado_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
