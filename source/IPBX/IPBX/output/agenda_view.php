...
$fieldsArr = array();
$arr = array();
$arr['fName'] = "idt_agenda";
$arr['viewFormat'] = ViewFormat("idt_agenda", $strTableName);
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Nome";
...
$arr = array();
$arr['fName'] = "Numero";
$arr['viewFormat'] = ViewFormat("Numero", $strTableName);
$fieldsArr[] = $arr;
...
while($data)
{
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"Numero", "")));
...
//idt_agenda - 
	$value="";
	$value = ProcessLargeText(GetData($data,"idt_agenda", ""),"","",MODE_VIEW);
	$xt->assign("idt_agenda_value",$value);
...
//Numero - 
	$value="";
	$value = ProcessLargeText(GetData($data,"Numero", ""),"","",MODE_VIEW);
...
$jsKeysObj = 'window.recKeysObj = {';
	$jsKeysObj .= "'".jsreplace("Numero")."': '".(jsreplace(@$data["Numero"]))."', ";
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	$where=$_SESSION[$strTableName."_where"];
		if(GetFieldIndex("Numero"))
			$key[]=GetFieldIndex("Numero");
