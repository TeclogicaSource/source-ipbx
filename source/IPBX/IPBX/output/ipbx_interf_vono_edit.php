...
$templatefile = ( $inlineedit ) ? "ipbx_interf_vono_inline_edit.htm" : "ipbx_interf_vono_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ipbx_interf_vono_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_interf_vono->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_interf_vono_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	...
	$data["ddd_localidade"]=$evalues["ddd_localidade"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_vono_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_interf_vono_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_vono_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_interf_vono_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_vono_list.php?a=return'});return false;\"");
...
$control_ddd_localidade["params"]["value"]=@$data["ddd_localidade"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_ddd_localidade["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_ddd_localidade["params"]["mode"]="inline_edit";
else
	$control_ddd_localidade["params"]["mode"]="edit";
if(!$detailKeys || !in_array("ddd_localidade", $detailKeys))	
	$xt->assign("ddd_localidade_editcontrol",$control_ddd_localidade);
else if(array_key_exists("ddd_localidade",$data))
{
				$value = ProcessLargeText(GetData($data,"ddd_localidade", ""),"field=ddd%5Flocalidade","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_interf_vono_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
