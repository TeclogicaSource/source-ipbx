...
$pageObject->body["begin"] .="<form action=\"Rel__Periodo_Fila_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel__Periodo_Fila_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Periodo_Fila.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Periodo_Fila.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Periodo_Fila.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["logados"] = GetData($row,"logados","");
			$values["pausados"] = GetData($row,"pausados","");
			$values["atendendo"] = GetData($row,"atendendo","");
			$values["discando"] = GetData($row,"discando","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Periodo_Fila.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="";
			$values["logados"] = htmlspecialchars(GetData($row,"logados",$format));
			$format="";
			$values["pausados"] = htmlspecialchars(GetData($row,"pausados",$format));
			$format="";
			$values["atendendo"] = htmlspecialchars(GetData($row,"atendendo",$format));
			$format="";
			$values["discando"] = htmlspecialchars(GetData($row,"discando",$format));
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["logados"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["pausados"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["atendendo"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["discando"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Dt Entrada").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Hr Entrada").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Logados").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Pausados").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Atendendo").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Discando").'</td>';	
	}
	else
	{
		echo "<td>"."Fila"."</td>";
		echo "<td>"."Dt Entrada"."</td>";
		echo "<td>"."Hr Entrada"."</td>";
		echo "<td>"."Logados"."</td>";
		echo "<td>"."Pausados"."</td>";
		echo "<td>"."Atendendo"."</td>";
		echo "<td>"."Discando"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["logados"] = GetData($row,"logados",$format);
					
							$format="";
			
			$values["pausados"] = GetData($row,"pausados",$format);
					
							$format="";
			
			$values["atendendo"] = GetData($row,"atendendo",$format);
					
							$format="";
			
			$values["discando"] = GetData($row,"discando",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["logados"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="";
									echo htmlspecialchars($values["pausados"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="";
									echo htmlspecialchars($values["atendendo"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="";
									echo htmlspecialchars($values["discando"]);
			echo '</td>';
			echo "</tr>";
		}		
