...
$templatefile = ( $inlineedit ) ? "ldap_inline_edit.htm" : "ldap_edit.htm";
...
//	processibng name - end
	...
	if($condition)
	{
	$value = postvalue("value_name2_".$id);
	$type=postvalue("type_name2_".$id);
	if(FieldSubmitted("name2_".$id))
	{
		
		$value=prepare_for_db("name2",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "name2"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["name2"]=$value;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ldap_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ldap_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ldap_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ldap_edit.php','".$skeys."','',function(){window.location.href='ldap_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ldap_edit.php','".$skeys."','',function(){window.location.href='ldap_list.php?a=return'});return false;\"");
...
//	nm_usuario - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"nm_usuario", ""),"field=nm%5Fusuario".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "nm_usuario";
				$showRawValues[] = substr($data["nm_usuario"],0,100);
...
//	name2 - 
		...
		$value="";
				$value=DisplayLookupWizard("name2",$data["name2"],$data,$keylink,MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "name2";
				$showRawValues[] = substr($data["name2"],0,100);

//	dt_atualizacao - Short Date

		$value="";
				$value = ProcessLargeText(GetData($data,"dt_atualizacao", "Short Date"),"field=dt%5Fatualizacao".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dt_atualizacao";
				$showRawValues[] = substr($data["dt_atualizacao"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
//	control - name2
$control_name2=array();
$control_name2["func"]="xt_buildeditcontrol";
$control_name2["params"] = array();
$control_name2["params"]["field"]="name2";
$control_name2["params"]["value"]=@$data["name2"];
//	Begin Add validation
$arrValidate = array();	

$control_name2["params"]["validate"]=$arrValidate;
//	End Add validation
$control_name2["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_name2["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_name2["params"]["mode"]="inline_edit";
else
	$control_name2["params"]["mode"]="edit";
if(!$detailKeys || !in_array("name2", $detailKeys))	
	$xt->assign("name2_editcontrol",$control_name2);
else if(array_key_exists("name2",$data))
{
				$value=DisplayLookupWizard("name2",$data["name2"],$data,"",MODE_VIEW);
		$xt->assign("name2_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ldap_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
