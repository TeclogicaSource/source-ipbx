...
$templatefile = ( $inlineedit ) ? "cc_receptivo_agentes_inline_edit.htm" : "cc_receptivo_agentes_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_cc_rec_agentes"]=postvalue("editid1");
$savedKeys["id_cc_rec_agentes"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_cc_rec_agentes"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: cc_receptivo_agentes_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_cc_receptivo_agentes->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: cc_receptivo_agentes_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["id_agente"]=$evalues["id_agente"];
	$data["nm_agente"]=$evalues["nm_agente"];
	$data["penalidade"]=$evalues["penalidade"];
	$data["senha"]=$evalues["senha"];
	$data["id_fila"]=$evalues["id_fila"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_cc_rec_agentes"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_cc_rec_agentes", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="cc_receptivo_agentes_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_cc_rec_agentes"))
		$key[]=GetFieldIndex("id_cc_rec_agentes");
...
//create a line where and order for the two queries
	if($lenArr>0 and count($key) and !$_SESSION[$strTableName."_noNextPrev"])
	{
		...
		if($where_next!="" and $order_next!="" and $where_prev!="" and $order_prev!="")
		{
			...
			{
					$next[1]=$row_next["id_cc_rec_agentes"];
			}
			
			$res_prev=db_query($sql_prev,$conn);	
			if($row_prev=db_fetch_array($res_prev))
			{
					$prev[1]=$row_prev["id_cc_rec_agentes"];
			}
		}
	}
}
	...
	$resetEditors="";
	$unblRec="UnlockRecord('cc_receptivo_agentes_edit.php','".$skeys."','',function(){window.location.href='cc_receptivo_agentes_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('cc_receptivo_agentes_edit.php','".$skeys."','',function(){window.location.href='cc_receptivo_agentes_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_cc_rec_agentes"]);
if($message)
{
...
$control_id_agente["params"]["value"]=@$data["id_agente"];
...
$control_id_agente["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_id_agente["params"]["mode"]="inline_edit";
else
	$control_id_agente["params"]["mode"]="edit";
if(!$detailKeys || !in_array("id_agente", $detailKeys))	
	$xt->assign("id_agente_editcontrol",$control_id_agente);
else if(array_key_exists("id_agente",$data))
{
				$value = ProcessLargeText(GetData($data,"id_agente", ""),"field=id%5Fagente","",MODE_VIEW);
...
$control_nm_agente["params"]["value"]=@$data["nm_agente"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_nm_agente["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_nm_agente["params"]["mode"]="inline_edit";
else
	$control_nm_agente["params"]["mode"]="edit";
if(!$detailKeys || !in_array("nm_agente", $detailKeys))	
	$xt->assign("nm_agente_editcontrol",$control_nm_agente);
else if(array_key_exists("nm_agente",$data))
{
				$value = ProcessLargeText(GetData($data,"nm_agente", ""),"field=nm%5Fagente","",MODE_VIEW);
...
$control_penalidade["params"]["value"]=@$data["penalidade"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_penalidade["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_penalidade["params"]["mode"]="inline_edit";
else
	$control_penalidade["params"]["mode"]="edit";
if(!$detailKeys || !in_array("penalidade", $detailKeys))	
	$xt->assign("penalidade_editcontrol",$control_penalidade);
else if(array_key_exists("penalidade",$data))
{
				$value = ProcessLargeText(GetData($data,"penalidade", ""),"field=penalidade","",MODE_VIEW);
...
$control_id_fila["params"]["value"]=@$data["id_fila"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_id_fila["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_id_fila["params"]["mode"]="inline_edit";
else
	$control_id_fila["params"]["mode"]="edit";
if(!$detailKeys || !in_array("id_fila", $detailKeys))	
	$xt->assign("id_fila_editcontrol",$control_id_fila);
else if(array_key_exists("id_fila",$data))
{
				$value=DisplayLookupWizard("id_fila",$data["id_fila"],$data,"",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('cc_receptivo_agentes_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
