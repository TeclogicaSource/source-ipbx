...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_dt_especifica_feriado_inline_add.htm";
else
	$templatefile = "ipbx_dt_especifica_feriado_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_dt_especifica_feriado', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	processing dsc_data - start
	...
	if(!($value===false))
	{
		...
		if(0 && "dsc_data"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["dsc_data"]=$value;
	}
	}
//	processibng dsc_data - end
//	processing dt_especifica - start
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_dt_especifica_feriado_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_dt_especifica_feriado_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_dt_especifica_feriado_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_dt_espec"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_dt_espec"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_dt_especifica_feriado_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_dt_especifica_feriado_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_dt_especifica_feriado_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_dt_espec"]);
...
$control_dsc_data["params"]["value"]=@$defvalues["dsc_data"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dsc_data["params"]["mode"]="inline_add";
else
	$control_dsc_data["params"]["mode"]="add";

if(!$detailKeys || !in_array("dsc_data", $detailKeys))
	$xt->assignbyref("dsc_data_editcontrol",$control_dsc_data);
else if(array_key_exists("dsc_data", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dsc_data", ""),"field=dsc%5Fdata","",MODE_VIEW);
...
$control_dt_especifica["params"]["value"]=@$defvalues["dt_especifica"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dt_especifica["params"]["mode"]="inline_add";
else
	$control_dt_especifica["params"]["mode"]="add";

if(!$detailKeys || !in_array("dt_especifica", $detailKeys))
	$xt->assignbyref("dt_especifica_editcontrol",$control_dt_especifica);
else if(array_key_exists("dt_especifica", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dt_especifica", "Short Date"),"field=dt%5Fespecifica","",MODE_VIEW);
