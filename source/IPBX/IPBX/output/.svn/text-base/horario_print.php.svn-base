...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_horario"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["id_horario"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
//	dsc_horario - 
			...
			$record["dsc_horario_value"]=$value;
...
//	seg - 
			$value="";
				$value = ProcessLargeText(GetData($data,"seg", ""),"field=seg".$keylink,"",MODE_PRINT);
			$record["seg_value"]=$value;

//	ter - 
			$value="";
				$value = ProcessLargeText(GetData($data,"ter", ""),"field=ter".$keylink,"",MODE_PRINT);
			$record["ter_value"]=$value;

//	qua - 
			$value="";
				$value = ProcessLargeText(GetData($data,"qua", ""),"field=qua".$keylink,"",MODE_PRINT);
			$record["qua_value"]=$value;

//	qui - 
			$value="";
				$value = ProcessLargeText(GetData($data,"qui", ""),"field=qui".$keylink,"",MODE_PRINT);
...
//	sab - 
			$value="";
				$value = ProcessLargeText(GetData($data,"sab", ""),"field=sab".$keylink,"",MODE_PRINT);
			$record["sab_value"]=$value;

//	dom - 
			$value="";
				$value = ProcessLargeText(GetData($data,"dom", ""),"field=dom".$keylink,"",MODE_PRINT);
...
//	include proper masterprint.php code
	include("include/sip_buddies_horario_masterprint.php");
	$masterkeys[]=@$_SESSION[$strTableName."_masterkey1"];
	$params=array("detailtable"=>"horario","keys"=>$masterkeys);
	$master=array();
	$master["func"]="DisplayMasterTableInfo_sip_buddies_horario";
...
$xt->assign("dsc_horario_fieldheadercolumn",true);
$xt->assign("dsc_horario_fieldheader",true);
$xt->assign("dsc_horario_fieldcolumn",true);
$xt->assign("dsc_horario_fieldfootercolumn",true);
...
$templatefile = "horario_print.htm";
