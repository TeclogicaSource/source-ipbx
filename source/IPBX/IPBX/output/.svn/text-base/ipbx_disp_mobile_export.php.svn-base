...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_idm"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"ipbx_disp_mobile_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("ipbx_disp_mobile_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_disp_mobile.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_disp_mobile.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_disp_mobile.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["name"] = DisplayLookupWizard("name",$row["name"],$row,"",MODE_EXPORT);		
			$values["dsc_dispositivo"] = GetData($row,"dsc_dispositivo","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_disp_mobile.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values["name"] = DisplayLookupWizard("name",$row["name"],$row,"",MODE_EXPORT);
			$format="";
			$values["dsc_dispositivo"] = htmlspecialchars(GetData($row,"dsc_dispositivo",$format));
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["dsc_dispositivo"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Telefone autorizado").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Ramal").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Descrição do dispositivo").'</td>';	
	}
	else
	{
		echo "<td>"."Telefone autorizado"."</td>";
		echo "<td>"."Ramal"."</td>";
		echo "<td>"."Descrição do dispositivo"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["dsc_dispositivo"] = GetData($row,"dsc_dispositivo",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["dsc_dispositivo"]);
					else
						echo htmlspecialchars($values["dsc_dispositivo"]);
			echo '</td>';
			echo "</tr>";
		}		
