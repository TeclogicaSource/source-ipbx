...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_celular"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["id_celular"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"sms_celulares_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("sms_celulares_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=sms_celulares.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=sms_celulares.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=sms_celulares.xml");
	...
	{
		$values = array();
			$values["id_grupo"] = DisplayLookupWizard("id_grupo",$row["id_grupo"],$row,"",MODE_EXPORT);		
			$values["dsc_nome"] = GetData($row,"dsc_nome","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=sms_celulares.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values["id_grupo"] = DisplayLookupWizard("id_grupo",$row["id_grupo"],$row,"",MODE_EXPORT);
			$format="";
			$values["dsc_nome"] = htmlspecialchars(GetData($row,"dsc_nome",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_grupo"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["dsc_nome"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Nome do Proprietário").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Telefone").'</td>';	
	}
	else
	{
		echo "<td>"."Grupo"."</td>";
		echo "<td>"."Nome do Proprietário"."</td>";
		echo "<td>"."Telefone"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["dsc_nome"] = GetData($row,"dsc_nome",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["id_grupo"]);
				else
					echo htmlspecialchars($values["id_grupo"]);
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["dsc_nome"]);
					else
						echo htmlspecialchars($values["dsc_nome"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
