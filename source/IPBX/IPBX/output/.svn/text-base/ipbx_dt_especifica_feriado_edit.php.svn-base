...
include("include/ipbx_dt_especifica_feriado_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_dt_especifica_feriado_inline_edit.htm" : "ipbx_dt_especifica_feriado_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_dt_espec"]=postvalue("editid1");
$savedKeys["id_dt_espec"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_dsc_data_".$id);
	$type=postvalue("type_dsc_data_".$id);
	if(FieldSubmitted("dsc_data_".$id))
	{
		
		$value=prepare_for_db("dsc_data",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "dsc_data"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["dsc_data"]=$value;
		
		
	}


//	processibng dsc_data - end
	}
	$condition = $inlineedit;

	if($condition)
	{
	$value = postvalue("value_dt_especifica_".$id);
	$type=postvalue("type_dt_especifica_".$id);
	if(FieldSubmitted("dt_especifica_".$id))
	{
		
		$value=prepare_for_db("dt_especifica",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "dt_especifica"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["dt_especifica"]=$value;
		
		
	}


//	processibng dt_especifica - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_dt_espec"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_dt_especifica_feriado_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_dt_especifica_feriado->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_dt_especifica_feriado_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_data"]=$evalues["dsc_data"];
	$data["dt_especifica"]=$evalues["dt_especifica"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_dt_espec"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_dt_espec", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_dt_especifica_feriado_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_dt_espec"))
		$key[]=GetFieldIndex("id_dt_espec");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_dt_especifica_feriado_edit.php','".$skeys."','',function(){window.location.href='ipbx_dt_especifica_feriado_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_dt_especifica_feriado_edit.php','".$skeys."','',function(){window.location.href='ipbx_dt_especifica_feriado_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_dt_espec"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_dt_espec"]));


//	dsc_data - 

		$value="";
				$value = ProcessLargeText(GetData($data,"dsc_data", ""),"field=dsc%5Fdata".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dsc_data";
				$showRawValues[] = substr($data["dsc_data"],0,100);

//	dt_especifica - Short Date

		$value="";
				$value = ProcessLargeText(GetData($data,"dt_especifica", "Short Date"),"field=dt%5Fespecifica".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dt_especifica";
				$showRawValues[] = substr($data["dt_especifica"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$regextype = '';
//	control - dsc_data
$control_dsc_data=array();
$control_dsc_data["func"]="xt_buildeditcontrol";
$control_dsc_data["params"] = array();
$control_dsc_data["params"]["field"]="dsc_data";
$control_dsc_data["params"]["value"]=@$data["dsc_data"];
//	Begin Add validation
$arrValidate = array();	

$arrValidate['basicValidate'][] = "IsRequired";

$control_dsc_data["params"]["validate"]=$arrValidate;
//	End Add validation
$control_dsc_data["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_dsc_data["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_data["params"]["mode"]="inline_edit";
else
	$control_dsc_data["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_data", $detailKeys))	
	$xt->assign("dsc_data_editcontrol",$control_dsc_data);
else if(array_key_exists("dsc_data",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_data", ""),"field=dsc%5Fdata","",MODE_VIEW);
		$xt->assign("dsc_data_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - dt_especifica
$control_dt_especifica=array();
$control_dt_especifica["func"]="xt_buildeditcontrol";
$control_dt_especifica["params"] = array();
$control_dt_especifica["params"]["field"]="dt_especifica";
$control_dt_especifica["params"]["value"]=@$data["dt_especifica"];
//	Begin Add validation
$arrValidate = array();	

$arrValidate['basicValidate'][] = "IsRequired";

$control_dt_especifica["params"]["validate"]=$arrValidate;
//	End Add validation
$control_dt_especifica["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_dt_especifica["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dt_especifica["params"]["mode"]="inline_edit";
else
	$control_dt_especifica["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dt_especifica", $detailKeys))	
	$xt->assign("dt_especifica_editcontrol",$control_dt_especifica);
else if(array_key_exists("dt_especifica",$data))
{
				$value = ProcessLargeText(GetData($data,"dt_especifica", "Short Date"),"field=dt%5Fespecifica","",MODE_VIEW);
		$xt->assign("dt_especifica_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_dt_especifica_feriado_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
