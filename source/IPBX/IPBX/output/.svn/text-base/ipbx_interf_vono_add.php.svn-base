...
include("include/ipbx_interf_vono_variables.php");
...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_interf_vono_inline_add.htm";
else
	$templatefile = "ipbx_interf_vono_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_interf_vono', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	processing ddd_localidade - start
    
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
	$value = postvalue("value_ddd_localidade_".$id);
	$type=postvalue("type_ddd_localidade_".$id);
	if (FieldSubmitted("ddd_localidade_".$id))
	{
		$value=prepare_for_db("ddd_localidade",$value,$type);
	}
	else
		$value=false;
	if(!($value===false))
	{
		...
		if(0 && "ddd_localidade"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["ddd_localidade"]=$value;
	}
	}
//	processibng ddd_localidade - end
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_interf_vono_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_interf_vono_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_interf_vono_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_interf_vono_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_vono_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_vono_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
//	ddd_localidade - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"ddd_localidade", ""),"field=ddd%5Flocalidade".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "ddd_localidade";
				$showRawValues[] = substr($data["ddd_localidade"],0,100);
	}	
...
// add prevent submit on enter js if only one text record
//	control - ddd_localidade
$control_ddd_localidade=array();
$control_ddd_localidade["func"]="xt_buildeditcontrol";
$control_ddd_localidade["params"] = array();
$control_ddd_localidade["params"]["field"]="ddd_localidade";
$control_ddd_localidade["params"]["value"]=@$defvalues["ddd_localidade"];
...
$arrValidate['basicValidate'][] = "IsRequired";
$control_ddd_localidade["params"]["validate"]=$arrValidate;
//	End Add validation

$control_ddd_localidade["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_ddd_localidade["params"]["mode"]="inline_add";
else
	$control_ddd_localidade["params"]["mode"]="add";

if(!$detailKeys || !in_array("ddd_localidade", $detailKeys))
	$xt->assignbyref("ddd_localidade_editcontrol",$control_ddd_localidade);
else if(array_key_exists("ddd_localidade", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"ddd_localidade", ""),"field=ddd%5Flocalidade","",MODE_VIEW);
		$xt->assign("ddd_localidade_editcontrol",$value);
}
// add prevent submit on enter js if only one text record
