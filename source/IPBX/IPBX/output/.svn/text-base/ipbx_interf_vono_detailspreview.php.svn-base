...
if ( $rowcount ) {
	...
	//	ddd_localidade - 
		    $value="";
				$value = ProcessLargeText(GetData($data,"ddd_localidade", ""),"field=ddd%5Flocalidade".$keylink,"",MODE_PRINT);
			$row["ddd_localidade_value"]=$value;
	$rowinfo[]=$row;
	}
	$xt->assign_loopsection("details_row",$rowinfo);
} else {
}
$xt->display("ipbx_interf_vono_detailspreview.htm");
if($mode!="inline")
