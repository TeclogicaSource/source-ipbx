...
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "dsc_conf_central";
$arr['viewFormat'] = ViewFormat("dsc_conf_central", $strTableName);
$fieldsArr[] = $arr;
...
//dsc_conf_central - 
	$value="";
	$value = ProcessLargeText(GetData($data,"dsc_conf_central", ""),"","",MODE_VIEW);
	$xt->assign("dsc_conf_central_value",$value);
	$xt->assign("dsc_conf_central_fieldblock",true);
 


$jsKeysObj = 'window.recKeysObj = {';
	$jsKeysObj .= "'".jsreplace("id_central")."': '".(jsreplace(@$data["id_central"]))."', ";
...
//Begin show Next Prev button
	...
	if(count($next))
    {
		...
		$xt->assign("nextbutton_attrs","onclick=\"window.location.href='central_view.php?".$nextlink."'\"");
	}
	else 
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","onclick=\"window.location.href='central_view.php?".$prevlink."'\"");
	}
    else 
