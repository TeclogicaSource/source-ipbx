...
if($inlineadd==ADD_INLINE)
	$templatefile = "ramal_pabx_autorizado_inline_add.htm";
else
	$templatefile = "ramal_pabx_autorizado_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ramal_pabx_autorizado', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ramal_pabx_autorizado_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ramal_pabx_autorizado_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ramal_pabx_autorizado_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_ramal_pabx"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_ramal_pabx"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ramal_pabx_autorizado_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ramal_pabx_autorizado_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ramal_pabx_autorizado_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_ramal_pabx"]);
...
{
				$value = ProcessLargeText(GetData($defvalues,"ramal", ""),"field=ramal","",MODE_VIEW);
...
$control_flg_disp["params"]["value"]=@$defvalues["flg_disp"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_flg_disp["params"]["mode"]="inline_add";
else
	$control_flg_disp["params"]["mode"]="add";

if(!$detailKeys || !in_array("flg_disp", $detailKeys))
	$xt->assignbyref("flg_disp_editcontrol",$control_flg_disp);
else if(array_key_exists("flg_disp", $defvalues))
{
				$value = GetData($defvalues,"flg_disp", "Checkbox");
