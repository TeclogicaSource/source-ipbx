...
$pageObject->body["begin"] .="<form action=\"Telefone_Recebido_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Telefone_Recebido_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Telefone_Recebido.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Telefone_Recebido.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Telefone_Recebido.xml");
	...
	{
		$values = array();
			$values["idt_agenda"] = GetData($row,"idt_agenda","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Telefone_Recebido.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["idt_agenda"] = htmlspecialchars(GetData($row,"idt_agenda",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["idt_agenda"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Idt Agenda").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Nome").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Numero").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Idt Custo").'</td>';	
	}
	else
	{
		echo "<td>"."Idt Agenda"."</td>";
		echo "<td>"."Nome"."</td>";
		echo "<td>"."Numero"."</td>";
		echo "<td>"."Idt Custo"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["idt_agenda"] = GetData($row,"idt_agenda",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["idt_agenda"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
