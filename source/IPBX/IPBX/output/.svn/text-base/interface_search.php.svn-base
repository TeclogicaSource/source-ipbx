...
$mode=SEARCH_SIMPLE;
$templatefile = "interface_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "interface_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'interface';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "interface";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("placa");
	...
	if(isEnableSection508())
		$xt->assign_section("placa_label","<label for=\"".GetInputElementId("placa", $id)."\">","</label>");
	else 
		$xt->assign("placa_label", true);
	
	$xt->assign("placa_fieldblock", true);		
	$xt->assignbyref("placa_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("placa_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("placa_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_placa", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("placa", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("placa")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	...
	{
	$contents_block["begin"].="action=\"interface_list.php\" ";
	}
