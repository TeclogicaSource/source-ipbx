...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["idt_cadeado"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["idt_cadeado"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"cadeado_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("cadeado_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=cadeado.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=cadeado.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=cadeado.xml");
	...
	{
		$values = array();
			$values["idt_cadeado"] = GetData($row,"idt_cadeado","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=cadeado.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["idt_cadeado"] = htmlspecialchars(GetData($row,"idt_cadeado",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["idt_cadeado"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Idt Cadeado").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Ramal").'</td>';	
	}
	else
	{
		echo "<td>"."Idt Cadeado"."</td>";
		echo "<td>"."Ramal"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["idt_cadeado"] = GetData($row,"idt_cadeado",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["idt_cadeado"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
