...
$templatefile = "troncos_vono_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: troncos_vono_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_troncos_vono->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: troncos_vono_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	...
	$data["prioridade"]=$evalues["prioridade"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="troncos_vono_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('troncos_vono_edit.php','".$skeys."','',function(){window.location.href='troncos_vono_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('troncos_vono_edit.php','".$skeys."','',function(){window.location.href='troncos_vono_list.php?a=return'});return false;\"");
...
{
				$value = ProcessLargeText(GetData($data,"flg_grava_ent", ""),"field=flg%5Fgrava%5Fent","",MODE_VIEW);
...
{
				$value = ProcessLargeText(GetData($data,"flg_grava_sai", ""),"field=flg%5Fgrava%5Fsai","",MODE_VIEW);
...
{
				$value = ProcessLargeText(GetData($data,"flg_pabx_remoto", ""),"field=flg%5Fpabx%5Fremoto","",MODE_VIEW);
...
{
				$value = ProcessLargeText(GetData($data,"flg_cel_local", ""),"field=flg%5Fcel%5Flocal","",MODE_VIEW);
...
{
				$value = ProcessLargeText(GetData($data,"flg_local", ""),"field=flg%5Flocal","",MODE_VIEW);
...
{
				$value = ProcessLargeText(GetData($data,"flg_fixo_ddd", ""),"field=flg%5Ffixo%5Fddd","",MODE_VIEW);
...
{
				$value = ProcessLargeText(GetData($data,"flg_cel_ddd", ""),"field=flg%5Fcel%5Fddd","",MODE_VIEW);
...
{
				$value = ProcessLargeText(GetData($data,"flg_ddi", ""),"field=flg%5Fddi","",MODE_VIEW);
...
$control_prioridade["params"]["value"]=@$data["prioridade"];
//	Begin Add validation
$arrValidate = array();	
$validatetype = getJsValidatorName("Number");
$arrValidate['basicValidate'][] = $validatetype;
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_prioridade["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_prioridade["params"]["mode"]="inline_edit";
else
	$control_prioridade["params"]["mode"]="edit";
if(!$detailKeys || !in_array("prioridade", $detailKeys))	
	$xt->assign("prioridade_editcontrol",$control_prioridade);
else if(array_key_exists("prioridade",$data))
{
				$value = ProcessLargeText(GetData($data,"prioridade", ""),"field=prioridade","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('troncos_vono_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
