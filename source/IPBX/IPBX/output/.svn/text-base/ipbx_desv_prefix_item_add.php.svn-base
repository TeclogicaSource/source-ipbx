...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_desv_prefix_item_inline_add.htm";
else
	$templatefile = "ipbx_desv_prefix_item_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_desv_prefix_item', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
if(@$_POST["a"]=="added")
{
	...
	$files_move=array();
	$files_save=array();
//	processing id_desv_prefix - start
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="ipbx_desv_prefix")
	{
		if(!@$_SESSION[$sessionPrefix."_masterkey1"] && postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		if($avalues["id_desv_prefix"]=="")
			$avalues["id_desv_prefix"]=prepare_for_db("id_desv_prefix",$_SESSION[$sessionPrefix."_masterkey1"]);
	}
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_desv_prefix_item_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_desv_prefix_item_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_desv_prefix_item_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_desv_pref_item"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_desv_pref_item"]=postvalue("editid1");
	}
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="ipbx_desv_prefix")
{
	if(!@$_SESSION[$sessionPrefix."_masterkey1"] && postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
	$defvalues["id_desv_prefix"]=@$_SESSION[$sessionPrefix."_masterkey1"];	
}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_desv_prefix_item_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_desv_prefix_item_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_desv_pref_item"]);
...
$control_id_desv_prefix["params"]["value"]=@$defvalues["id_desv_prefix"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_id_desv_prefix["params"]["mode"]="inline_add";
else
	$control_id_desv_prefix["params"]["mode"]="add";

if(!$detailKeys || !in_array("id_desv_prefix", $detailKeys))
	$xt->assignbyref("id_desv_prefix_editcontrol",$control_id_desv_prefix);
else if(array_key_exists("id_desv_prefix", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"id_desv_prefix", ""),"field=id%5Fdesv%5Fprefix","",MODE_VIEW);
