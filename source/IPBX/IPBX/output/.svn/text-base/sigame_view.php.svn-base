...
$arr = array();
$arr['fName'] = "Destino";
$arr['viewFormat'] = ViewFormat("Destino", $strTableName);
$fieldsArr[] = $arr;
...
while($data)
{
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"Ramal", "")));

	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["Ramal"]));
...
//Destino - 
	$value="";
	$value = ProcessLargeText(GetData($data,"Destino", ""),"","",MODE_VIEW);
...
$jsKeysObj = 'window.recKeysObj = {';
	$jsKeysObj .= "'".jsreplace("Ramal")."': '".(jsreplace(@$data["Ramal"]))."', ";
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	$where=$_SESSION[$strTableName."_where"];
		if(GetFieldIndex("Ramal"))
			$key[]=GetFieldIndex("Ramal");
...
//Begin show Next Prev button
	...
	if(count($next))
    {
		...
		$xt->assign("nextbutton_attrs","onclick=\"window.location.href='sigame_view.php?".$nextlink."'\"");
	}
	else 
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","onclick=\"window.location.href='sigame_view.php?".$prevlink."'\"");
	}
    else 
