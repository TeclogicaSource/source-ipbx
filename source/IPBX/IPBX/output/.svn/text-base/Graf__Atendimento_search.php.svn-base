...
$mode=SEARCH_SIMPLE;
$templatefile = "Graf__Atendimento_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "Graf__Atendimento_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'Graf__Atendimento';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "Graf. Atendimento";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("Atendimentos");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Atendimentos";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Atendimentos_label","<label for=\"".GetInputElementId("Atendimentos", $id)."\">","</label>");
	else 
		$xt->assign("Atendimentos_label", true);
	
	$xt->assign("Atendimentos_fieldblock", true);		
	$xt->assignbyref("Atendimentos_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Atendimentos_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Atendimentos_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Atendimentos", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("Atendimentos", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("Atendimentos")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("Abandonos");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Abandonos";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Abandonos_label","<label for=\"".GetInputElementId("Abandonos", $id)."\">","</label>");
	else 
		$xt->assign("Abandonos_label", true);
	
	$xt->assign("Abandonos_fieldblock", true);		
	$xt->assignbyref("Abandonos_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Abandonos_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Abandonos_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Abandonos", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("Abandonos", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("Abandonos")."', ".$pageObject->id.", ".$ctrlsMap.");";
	...
	{
	$contents_block["begin"].="action=\"Graf__Atendimento_chart.php\" ";
	}
