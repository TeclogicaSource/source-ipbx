...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["idt_siga"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"sigame_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("sigame_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=sigame.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=sigame.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=sigame.xml");
	...
	{
		$values = array();
			$values["idt_siga"] = GetData($row,"idt_siga","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=sigame.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["idt_siga"] = htmlspecialchars(GetData($row,"idt_siga",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["idt_siga"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Idt Siga").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["idt_siga"] = GetData($row,"idt_siga",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["idt_siga"]);
			echo '</td>';
							echo '<td>';
