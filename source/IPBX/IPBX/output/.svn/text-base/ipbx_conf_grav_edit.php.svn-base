...
include("include/ipbx_conf_grav_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_conf_grav_inline_edit.htm" : "ipbx_conf_grav_edit.htm";
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_tp_reter_grav_".$id);
	$type=postvalue("type_tp_reter_grav_".$id);
	if(FieldSubmitted("tp_reter_grav_".$id))
	{
		
		$value=prepare_for_db("tp_reter_grav",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "tp_reter_grav"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["tp_reter_grav"]=$value;
		
		
	}


//	processibng tp_reter_grav - end
	}
	$condition = $inlineedit;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ipbx_conf_grav_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_conf_grav->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_conf_grav_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["tp_reter_grav"]=$evalues["tp_reter_grav"];
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_conf_grav_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_conf_grav_edit.php','".$skeys."','',function(){window.location.href='ipbx_conf_grav_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_conf_grav_edit.php','".$skeys."','',function(){window.location.href='ipbx_conf_grav_list.php?a=return'});return false;\"");
...
//	tp_reter_grav - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"tp_reter_grav", ""),"field=tp%5Freter%5Fgrav".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "tp_reter_grav";
				$showRawValues[] = substr($data["tp_reter_grav"],0,100);
...
$regexmessage='';
$regextype = '';
//	control - tp_reter_grav
$control_tp_reter_grav=array();
$control_tp_reter_grav["func"]="xt_buildeditcontrol";
$control_tp_reter_grav["params"] = array();
$control_tp_reter_grav["params"]["field"]="tp_reter_grav";
$control_tp_reter_grav["params"]["value"]=@$data["tp_reter_grav"];
...
$control_tp_reter_grav["params"]["validate"]=$arrValidate;
//	End Add validation
$control_tp_reter_grav["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_tp_reter_grav["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_tp_reter_grav["params"]["mode"]="inline_edit";
else
	$control_tp_reter_grav["params"]["mode"]="edit";
if(!$detailKeys || !in_array("tp_reter_grav", $detailKeys))	
	$xt->assign("tp_reter_grav_editcontrol",$control_tp_reter_grav);
else if(array_key_exists("tp_reter_grav",$data))
{
				$value = ProcessLargeText(GetData($data,"tp_reter_grav", ""),"field=tp%5Freter%5Fgrav","",MODE_VIEW);
		$xt->assign("tp_reter_grav_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_conf_grav_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
