...
if($inlineadd==ADD_INLINE)
	$templatefile = "planos_discagem_inline_add.htm";
else
	$templatefile = "planos_discagem_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'planos_discagem', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="plano_discagem_regiao")
	{
		if(!@$_SESSION[$sessionPrefix."_masterkey1"] && postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		if($avalues["id_regiao"]=="")
			$avalues["id_regiao"]=prepare_for_db("id_regiao",$_SESSION[$sessionPrefix."_masterkey1"]);
	}
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'planos_discagem_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'planos_discagem_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: planos_discagem_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="plano_discagem_regiao")
{
	if(!@$_SESSION[$sessionPrefix."_masterkey1"] && postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
	$defvalues["id_regiao"]=@$_SESSION[$sessionPrefix."_masterkey1"];	
}

if($readavalues)
{
	$defvalues["DDD"]=@$avalues["DDD"];
	$defvalues["id_tronco"]=@$avalues["id_tronco"];
}
//for basic files
$includes="";
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='planos_discagem_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="planos_discagem_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="planos_discagem_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
$control_DDD["params"]["value"]=@$defvalues["DDD"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_DDD["params"]["mode"]="inline_add";
else
	$control_DDD["params"]["mode"]="add";

if(!$detailKeys || !in_array("DDD", $detailKeys))
	$xt->assignbyref("DDD_editcontrol",$control_DDD);
else if(array_key_exists("DDD", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"DDD", ""),"field=DDD","",MODE_VIEW);
...
$control_id_tronco["params"]["value"]=@$defvalues["id_tronco"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_id_tronco["params"]["mode"]="inline_add";
else
	$control_id_tronco["params"]["mode"]="add";

if(!$detailKeys || !in_array("id_tronco", $detailKeys))
	$xt->assignbyref("id_tronco_editcontrol",$control_id_tronco);
else if(array_key_exists("id_tronco", $defvalues))
{
				$value=DisplayLookupWizard("id_tronco",$defvalues["id_tronco"],$defvalues,"",MODE_VIEW);
