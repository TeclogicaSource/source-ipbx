...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_pesquisa_ura_rev_inline_add.htm";
else
	$templatefile = "ipbx_pesquisa_ura_rev_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_pesquisa_ura_rev', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_pesquisa_ura_rev_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_pesquisa_ura_rev_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_pesquisa_ura_rev_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_pesquisa"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_pesquisa"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["dsc_pesquisa"]=@$avalues["dsc_pesquisa"];
}
//for basic files
$includes="";
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_pesquisa_ura_rev_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_pesquisa_ura_rev_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_pesquisa_ura_rev_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$masterquery="mastertable=ipbx%5Fpesquisa%5Fura%5Frev";
	$masterquery.="&masterkey1=".rawurlencode($data["id_pesquisa"]);
	$showDetailKeys["ipbx_ura_rev_msg"]=$masterquery;

	$showKeys[] = htmlspecialchars($keys["id_pesquisa"]);
...
//	dsc_pesquisa - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"dsc_pesquisa", ""),"field=dsc%5Fpesquisa".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dsc_pesquisa";
				$showRawValues[] = substr($data["dsc_pesquisa"],0,100);
	}	
...
$control_dsc_pesquisa["params"]["value"]=@$defvalues["dsc_pesquisa"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dsc_pesquisa["params"]["mode"]="inline_add";
else
	$control_dsc_pesquisa["params"]["mode"]="add";

if(!$detailKeys || !in_array("dsc_pesquisa", $detailKeys))
	$xt->assignbyref("dsc_pesquisa_editcontrol",$control_dsc_pesquisa);
else if(array_key_exists("dsc_pesquisa", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dsc_pesquisa", ""),"field=dsc%5Fpesquisa","",MODE_VIEW);
