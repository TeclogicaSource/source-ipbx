...
include("include/ipbx_grava_ramal_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_grava_ramal_inline_edit.htm" : "ipbx_grava_ramal_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ipbx_grava_ramal_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_grava_ramal->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_grava_ramal_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_grava_ramal_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_grava_ramal_edit.php','".$skeys."','',function(){window.location.href='ipbx_grava_ramal_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_grava_ramal_edit.php','".$skeys."','',function(){window.location.href='ipbx_grava_ramal_list.php?a=return'});return false;\"");
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_grava_ramal_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
