...
include("include/Graf__Custo_variables.php");
...
$mode=SEARCH_SIMPLE;
$templatefile = "Graf__Custo_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "Graf__Custo_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'Graf__Custo';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "Graf. Custo";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("Custo");
	...
	if(isEnableSection508())
		$xt->assign_section("Custo_label","<label for=\"".GetInputElementId("Custo", $id)."\">","</label>");
	else 
		$xt->assign("Custo_label", true);
	
	$xt->assign("Custo_fieldblock", true);		
	$xt->assignbyref("Custo_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Custo_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Custo_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Custo", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("Custo", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("Custo")."', ".$pageObject->id.", ".$ctrlsMap.");";
	...
	{
	$contents_block["begin"].="action=\"Graf__Custo_chart.php\" ";
	}
