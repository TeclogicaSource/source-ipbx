...
$templatefile = "sip_users_horario_edit.htm";
...
$keys["id_horario"]=postvalue("editid2");
$savedKeys["id_horario"]=postvalue("editid2");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	$keyGetQ = "";
		...
		$keyGetQ.="editid2=".rawurldecode($keys["id_horario"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: sip_users_horario_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_sip_users_horario->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: sip_users_horario_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
{
	$data["id"]=$evalues["id"];
	$data["dsc_plano"]=$evalues["dsc_plano"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid2\" value=\"".htmlspecialchars($keys["id_horario"])."\">";
	$xt->assign("show_key2", htmlspecialchars(GetData($data,"id_horario", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="sip_users_horario_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('sip_users_horario_edit.php','".$skeys."','',function(){window.location.href='sip_users_horario_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('sip_users_horario_edit.php','".$skeys."','',function(){window.location.href='sip_users_horario_list.php?a=return'});return false;\"");
...
{
				$value=DisplayLookupWizard("id",$data["id"],$data,"",MODE_VIEW);
...
$control_dsc_plano["params"]["value"]=@$data["dsc_plano"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_dsc_plano["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_plano["params"]["mode"]="inline_edit";
else
	$control_dsc_plano["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_plano", $detailKeys))	
	$xt->assign("dsc_plano_editcontrol",$control_dsc_plano);
else if(array_key_exists("dsc_plano",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_plano", ""),"field=dsc%5Fplano","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('sip_users_horario_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
