...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["idt_custo"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["idt_custo"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
//	dsc_centro_cust - 
			$value="";
				$value = ProcessLargeText(GetData($data,"dsc_centro_cust", ""),"field=dsc%5Fcentro%5Fcust".$keylink,"",MODE_PRINT);
			$record["dsc_centro_cust_value"]=$value;

//	idt_custo - 
			$value="";
				$value = ProcessLargeText(GetData($data,"idt_custo", ""),"field=idt%5Fcusto".$keylink,"",MODE_PRINT);
			$record["idt_custo_value"]=$value;

//	flg_ativado - Checkbox
			$value="";
				$value = GetData($data,"flg_ativado", "Checkbox");
			$record["flg_ativado_value"]=$value;
			if($col<$colsonpage)
...
$xt->assign("dsc_centro_cust_fieldheadercolumn",true);
$xt->assign("dsc_centro_cust_fieldheader",true);
$xt->assign("dsc_centro_cust_fieldcolumn",true);
$xt->assign("dsc_centro_cust_fieldfootercolumn",true);
...
$xt->assign("flg_ativado_fieldheadercolumn",true);
$xt->assign("flg_ativado_fieldheader",true);
$xt->assign("flg_ativado_fieldcolumn",true);
$xt->assign("flg_ativado_fieldfootercolumn",true);
...
$templatefile = "centrocusto_print.htm";
