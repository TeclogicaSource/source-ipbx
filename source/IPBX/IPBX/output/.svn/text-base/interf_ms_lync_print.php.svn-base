...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_ms_lync"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["id_ms_lync"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
//	id_ms_lync - 
			$value="";
				$value = ProcessLargeText(GetData($data,"id_ms_lync", ""),"field=id%5Fms%5Flync".$keylink,"",MODE_PRINT);
			$record["id_ms_lync_value"]=$value;

//	ip_host - 
			$value="";
				$value = ProcessLargeText(GetData($data,"ip_host", ""),"field=ip%5Fhost".$keylink,"",MODE_PRINT);
			$record["ip_host_value"]=$value;
...
//	include proper masterprint.php code
	...
	$params=array("detailtable"=>"interf_ms_lync","keys"=>$masterkeys);
	$master=array();
...
$xt->assign("id_ms_lync_fieldheadercolumn",true);
$xt->assign("id_ms_lync_fieldheader",true);
$xt->assign("id_ms_lync_fieldcolumn",true);
$xt->assign("id_ms_lync_fieldfootercolumn",true);
$xt->assign("ip_host_fieldheadercolumn",true);
$xt->assign("ip_host_fieldheader",true);
$xt->assign("ip_host_fieldcolumn",true);
$xt->assign("ip_host_fieldfootercolumn",true);
...
$xt->assign("codec_fieldheadercolumn",true);
$xt->assign("codec_fieldheader",true);
$xt->assign("codec_fieldcolumn",true);
$xt->assign("codec_fieldfootercolumn",true);
...
$templatefile = "interf_ms_lync_print.htm";
