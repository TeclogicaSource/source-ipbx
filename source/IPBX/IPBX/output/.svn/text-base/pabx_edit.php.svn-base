...
$templatefile = ( $inlineedit ) ? "pabx_inline_edit.htm" : "pabx_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["idt_tronco"]=postvalue("editid1");
$savedKeys["idt_tronco"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idt_tronco"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: pabx_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: pabx_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["idt_tronco"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"idt_tronco", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="pabx_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("idt_tronco"))
		$key[]=GetFieldIndex("idt_tronco");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('pabx_edit.php','".$skeys."','',function(){window.location.href='pabx_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('pabx_edit.php','".$skeys."','',function(){window.location.href='pabx_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["idt_tronco"]);
if($message)
{
...
$control_tronco["params"]["value"]=@$data["tronco"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_tronco["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_tronco["params"]["mode"]="inline_edit";
else
	$control_tronco["params"]["mode"]="edit";
if(!$detailKeys || !in_array("tronco", $detailKeys))	
	$xt->assign("tronco_editcontrol",$control_tronco);
else if(array_key_exists("tronco",$data))
{
				$value=DisplayLookupWizard("tronco",$data["tronco"],$data,"",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('pabx_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
