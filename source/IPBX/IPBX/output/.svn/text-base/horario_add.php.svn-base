...
if($inlineadd==ADD_INLINE)
	$templatefile = "horario_inline_add.htm";
else
	$templatefile = "horario_add.htm";
...
//	processing hr_inicio_01 - start
	...
	if(!($value===false))
	{
		...
		if(0 && "hr_inicio_01"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["hr_inicio_01"]=$value;
	}
	}
//	processibng hr_inicio_01 - end
//	processing hr_fim_01 - start
	...
	if(!($value===false))
	{
		...
		if(0 && "hr_fim_01"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["hr_fim_01"]=$value;
	}
	}
//	processibng hr_fim_01 - end
//	processing hr_inicio_02 - start
	...
	if(!($value===false))
	{
		...
		if(0 && "hr_inicio_02"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["hr_inicio_02"]=$value;
	}
	}
//	processibng hr_inicio_02 - end
//	processing hr_fim_02 - start
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'horario_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'horario_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: horario_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["hr_inicio_01"]=@$avalues["hr_inicio_01"];
	$defvalues["hr_fim_01"]=@$avalues["hr_fim_01"];
	...
	$defvalues["hr_inicio_02"]=@$avalues["hr_inicio_02"];
	$defvalues["hr_fim_02"]=@$avalues["hr_fim_02"];
	$defvalues["dsc_horario"]=@$avalues["dsc_horario"];
}
//for basic files
$includes="";
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="horario_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="horario_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
$control_hr_inicio_01["params"]["value"]=@$defvalues["hr_inicio_01"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_hr_inicio_01["params"]["mode"]="inline_add";
else
	$control_hr_inicio_01["params"]["mode"]="add";

if(!$detailKeys || !in_array("hr_inicio_01", $detailKeys))
	$xt->assignbyref("hr_inicio_01_editcontrol",$control_hr_inicio_01);
else if(array_key_exists("hr_inicio_01", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"hr_inicio_01", "Time"),"field=hr%5Finicio%5F01","",MODE_VIEW);
...
$control_hr_fim_01["params"]["value"]=@$defvalues["hr_fim_01"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_hr_fim_01["params"]["mode"]="inline_add";
else
	$control_hr_fim_01["params"]["mode"]="add";

if(!$detailKeys || !in_array("hr_fim_01", $detailKeys))
	$xt->assignbyref("hr_fim_01_editcontrol",$control_hr_fim_01);
else if(array_key_exists("hr_fim_01", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"hr_fim_01", "Time"),"field=hr%5Ffim%5F01","",MODE_VIEW);
...
$control_hr_inicio_02["params"]["value"]=@$defvalues["hr_inicio_02"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_hr_inicio_02["params"]["mode"]="inline_add";
else
	$control_hr_inicio_02["params"]["mode"]="add";

if(!$detailKeys || !in_array("hr_inicio_02", $detailKeys))
	$xt->assignbyref("hr_inicio_02_editcontrol",$control_hr_inicio_02);
else if(array_key_exists("hr_inicio_02", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"hr_inicio_02", "Time"),"field=hr%5Finicio%5F02","",MODE_VIEW);
...
$control_hr_fim_02["params"]["value"]=@$defvalues["hr_fim_02"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_hr_fim_02["params"]["mode"]="inline_add";
else
	$control_hr_fim_02["params"]["mode"]="add";

if(!$detailKeys || !in_array("hr_fim_02", $detailKeys))
	$xt->assignbyref("hr_fim_02_editcontrol",$control_hr_fim_02);
else if(array_key_exists("hr_fim_02", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"hr_fim_02", "Time"),"field=hr%5Ffim%5F02","",MODE_VIEW);
