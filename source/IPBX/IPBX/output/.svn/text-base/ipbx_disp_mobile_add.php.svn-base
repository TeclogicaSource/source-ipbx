...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_disp_mobile_inline_add.htm";
else
	$templatefile = "ipbx_disp_mobile_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_disp_mobile', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	processing id_dispositivo - start
	...
	if(!($value===false))
	{
		...
		if(0 && "id_dispositivo"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["id_dispositivo"]=$value;
	}
	}
//	processibng id_dispositivo - end
//	processing dsc_dispositivo - start
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_disp_mobile_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_disp_mobile_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_disp_mobile_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_idm"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_idm"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_disp_mobile_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_disp_mobile_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_disp_mobile_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_idm"]);
...
$control_id_dispositivo["params"]["value"]=@$defvalues["id_dispositivo"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_id_dispositivo["params"]["mode"]="inline_add";
else
	$control_id_dispositivo["params"]["mode"]="add";

if(!$detailKeys || !in_array("id_dispositivo", $detailKeys))
	$xt->assignbyref("id_dispositivo_editcontrol",$control_id_dispositivo);
else if(array_key_exists("id_dispositivo", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"id_dispositivo", ""),"field=id%5Fdispositivo","",MODE_VIEW);
...
$control_dsc_dispositivo["params"]["value"]=@$defvalues["dsc_dispositivo"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dsc_dispositivo["params"]["mode"]="inline_add";
else
	$control_dsc_dispositivo["params"]["mode"]="add";

if(!$detailKeys || !in_array("dsc_dispositivo", $detailKeys))
	$xt->assignbyref("dsc_dispositivo_editcontrol",$control_dsc_dispositivo);
else if(array_key_exists("dsc_dispositivo", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dsc_dispositivo", ""),"field=dsc%5Fdispositivo","",MODE_VIEW);
