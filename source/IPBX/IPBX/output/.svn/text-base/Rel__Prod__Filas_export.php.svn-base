...
include("include/Rel__Prod__Filas_variables.php");
...
$pageObject->body["begin"] .="<form action=\"Rel__Prod__Filas_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel__Prod__Filas_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Prod__Filas.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Prod__Filas.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Prod__Filas.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["NivelServicoContratado"] = GetData($row,"NivelServicoContratado","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Prod__Filas.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values["Fila"] = DisplayLookupWizard("Fila",$row["Fila"],$row,"",MODE_EXPORT);
			...
			$format="";
			$values["NivelServicoContratado"] = htmlspecialchars(GetData($row,"NivelServicoContratado",$format));
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["NivelServicoContratado"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Nivel Servico Contratado").'</td>';	
	}
	else
	{
		echo "<td>"."Data "."</td>";
		...
		echo "<td>"."Nivel Servico Contratado"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["NivelServicoContratado"] = GetData($row,"NivelServicoContratado",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Aband.Contr."]);
					else
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["NivelServicoContratado"]);
					else
						echo htmlspecialchars($values["NivelServicoContratado"]);
			echo '</td>';
			echo "</tr>";
		}		
