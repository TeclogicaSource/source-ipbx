...
include("include/sip_buddies_horario_variables.php");
...
$templatefile = "sip_buddies_horario_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_ramal_horario"]=postvalue("editid1");
$savedKeys["id_ramal_horario"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_ramal_horario"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: sip_buddies_horario_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_sip_buddies_horario->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: sip_buddies_horario_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_ramal_horario"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_ramal_horario", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="sip_buddies_horario_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_ramal_horario"))
		$key[]=GetFieldIndex("id_ramal_horario");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('sip_buddies_horario_edit.php','".$skeys."','',function(){window.location.href='sip_buddies_horario_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('sip_buddies_horario_edit.php','".$skeys."','',function(){window.location.href='sip_buddies_horario_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_ramal_horario"]);
if($message)
{
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('sip_buddies_horario_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
