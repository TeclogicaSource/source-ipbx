...
include("include/blacklist_variables.php");
...
$templatefile = ( $inlineedit ) ? "blacklist_inline_edit.htm" : "blacklist_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_bl_list"]=postvalue("editid1");
$savedKeys["id_bl_list"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_dsc_bl_list_".$id);
	$type=postvalue("type_dsc_bl_list_".$id);
	if(FieldSubmitted("dsc_bl_list_".$id))
	{
		
		$value=prepare_for_db("dsc_bl_list",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "dsc_bl_list"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["dsc_bl_list"]=$value;
		
		
	}


//	processibng dsc_bl_list - end
	}
	$condition = $inlineedit;

	if($condition)
	{
	$value = postvalue("value_num_bl_list_".$id);
	$type=postvalue("type_num_bl_list_".$id);
	if(FieldSubmitted("num_bl_list_".$id))
	{
		
		$value=prepare_for_db("num_bl_list",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "num_bl_list"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["num_bl_list"]=$value;
		
		
	}


//	processibng num_bl_list - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_bl_list"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: blacklist_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_blacklist->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: blacklist_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_bl_list"]=$evalues["dsc_bl_list"];
	$data["num_bl_list"]=$evalues["num_bl_list"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_bl_list"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_bl_list", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="blacklist_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_bl_list"))
		$key[]=GetFieldIndex("id_bl_list");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('blacklist_edit.php','".$skeys."','',function(){window.location.href='blacklist_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('blacklist_edit.php','".$skeys."','',function(){window.location.href='blacklist_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_bl_list"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_bl_list"]));


//	dsc_bl_list - 

		$value="";
				$value = ProcessLargeText(GetData($data,"dsc_bl_list", ""),"field=dsc%5Fbl%5Flist".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dsc_bl_list";
				$showRawValues[] = substr($data["dsc_bl_list"],0,100);

//	num_bl_list - 

		$value="";
				$value = ProcessLargeText(GetData($data,"num_bl_list", ""),"field=num%5Fbl%5Flist".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "num_bl_list";
				$showRawValues[] = substr($data["num_bl_list"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$regextype = '';
//	control - dsc_bl_list
$control_dsc_bl_list=array();
$control_dsc_bl_list["func"]="xt_buildeditcontrol";
$control_dsc_bl_list["params"] = array();
$control_dsc_bl_list["params"]["field"]="dsc_bl_list";
$control_dsc_bl_list["params"]["value"]=@$data["dsc_bl_list"];
//	Begin Add validation
$arrValidate = array();	

$arrValidate['basicValidate'][] = "IsRequired";

$control_dsc_bl_list["params"]["validate"]=$arrValidate;
//	End Add validation
$control_dsc_bl_list["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_dsc_bl_list["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_bl_list["params"]["mode"]="inline_edit";
else
	$control_dsc_bl_list["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_bl_list", $detailKeys))	
	$xt->assign("dsc_bl_list_editcontrol",$control_dsc_bl_list);
else if(array_key_exists("dsc_bl_list",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_bl_list", ""),"field=dsc%5Fbl%5Flist","",MODE_VIEW);
		$xt->assign("dsc_bl_list_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - num_bl_list
$control_num_bl_list=array();
$control_num_bl_list["func"]="xt_buildeditcontrol";
$control_num_bl_list["params"] = array();
$control_num_bl_list["params"]["field"]="num_bl_list";
$control_num_bl_list["params"]["value"]=@$data["num_bl_list"];
//	Begin Add validation
$arrValidate = array();	

$arrValidate['basicValidate'][] = "IsRequired";

$control_num_bl_list["params"]["validate"]=$arrValidate;
//	End Add validation
$control_num_bl_list["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_num_bl_list["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_num_bl_list["params"]["mode"]="inline_edit";
else
	$control_num_bl_list["params"]["mode"]="edit";
if(!$detailKeys || !in_array("num_bl_list", $detailKeys))	
	$xt->assign("num_bl_list_editcontrol",$control_num_bl_list);
else if(array_key_exists("num_bl_list",$data))
{
				$value = ProcessLargeText(GetData($data,"num_bl_list", ""),"field=num%5Fbl%5Flist","",MODE_VIEW);
		$xt->assign("num_bl_list_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('blacklist_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
