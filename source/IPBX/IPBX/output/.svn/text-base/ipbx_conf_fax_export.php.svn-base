...
$pageObject->body["begin"] .="<form action=\"ipbx_conf_fax_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("ipbx_conf_fax_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_conf_fax.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_conf_fax.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_conf_fax.xml");
	...
	{
		$values = array();
			$values["id_conf_fax"] = GetData($row,"id_conf_fax","");
			$values["tp_reter_fax"] = GetData($row,"tp_reter_fax","");
			$values["pasta"] = GetData($row,"pasta","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_conf_fax.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["id_conf_fax"] = htmlspecialchars(GetData($row,"id_conf_fax",$format));
			$format="";
			$values["tp_reter_fax"] = htmlspecialchars(GetData($row,"tp_reter_fax",$format));
			$format="";
			$values["pasta"] = htmlspecialchars(GetData($row,"pasta",$format));
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_conf_fax"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["tp_reter_fax"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["pasta"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Manter FAX por (mês)").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Local para gravação (Exemplo: /tmp/)").'</td>';	
	}
	else
	{
		echo "<td>"."Identificação"."</td>";
		echo "<td>"."Manter FAX por (mês)"."</td>";
		echo "<td>"."Local para gravação (Exemplo: /tmp/)"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["id_conf_fax"] = GetData($row,"id_conf_fax",$format);
					
							$format="";
			
			$values["tp_reter_fax"] = GetData($row,"tp_reter_fax",$format);
					
							$format="";
			
			$values["pasta"] = GetData($row,"pasta",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["id_conf_fax"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="";
									echo htmlspecialchars($values["tp_reter_fax"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["pasta"]);
					else
						echo htmlspecialchars($values["pasta"]);
			echo '</td>';
			echo "</tr>";
		}		
