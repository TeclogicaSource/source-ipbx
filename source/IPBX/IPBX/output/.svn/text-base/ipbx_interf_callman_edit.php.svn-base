...
$templatefile = ( $inlineedit ) ? "ipbx_interf_callman_inline_edit.htm" : "ipbx_interf_callman_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ipbx_interf_callman_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_interf_callman->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_interf_callman_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_callman_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_interf_callman_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_callman_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_interf_callman_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_callman_list.php?a=return'});return false;\"");
...
$control_dtmf["params"]["value"]=@$data["dtmf"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_dtmf["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dtmf["params"]["mode"]="inline_edit";
else
	$control_dtmf["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dtmf", $detailKeys))	
	$xt->assign("dtmf_editcontrol",$control_dtmf);
else if(array_key_exists("dtmf",$data))
{
				$value = ProcessLargeText(GetData($data,"dtmf", ""),"field=dtmf","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_interf_callman_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
