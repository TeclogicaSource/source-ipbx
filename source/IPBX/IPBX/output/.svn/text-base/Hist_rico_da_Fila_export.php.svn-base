...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["Fila"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["dt_periodo"]=refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$keys["hr_periodo"]=refine($_REQUEST["mdelete3"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<3)
				continue;
			$keys=array();
			$keys["Fila"]=urldecode($arr[0]);
			$keys["dt_periodo"]=urldecode($arr[1]);
			$keys["hr_periodo"]=urldecode($arr[2]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"Hist_rico_da_Fila_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Hist_rico_da_Fila_export.htm");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Hist_rico_da_Fila.xml");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Hist_rico_da_Fila.csv");
