...
include("include/ramal_pabx_autorizado_variables.php");
...
$templatefile = ( $inlineedit ) ? "ramal_pabx_autorizado_inline_edit.htm" : "ramal_pabx_autorizado_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_ramal_pabx"]=postvalue("editid1");
$savedKeys["id_ramal_pabx"]=postvalue("editid1");
...
//	processibng ramal - end
	...
	if($condition)
	{
	$value = postvalue("value_flg_disp_".$id);
	$type=postvalue("type_flg_disp_".$id);
	if(FieldSubmitted("flg_disp_".$id))
	{
		
		$value=prepare_for_db("flg_disp",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "flg_disp"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["flg_disp"]=$value;
		
		
	}


//	processibng flg_disp - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_ramal_pabx"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ramal_pabx_autorizado_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ramal_pabx_autorizado->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ramal_pabx_autorizado_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	...
	$data["flg_disp"]=$evalues["flg_disp"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_ramal_pabx"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_ramal_pabx", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ramal_pabx_autorizado_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_ramal_pabx"))
		$key[]=GetFieldIndex("id_ramal_pabx");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ramal_pabx_autorizado_edit.php','".$skeys."','',function(){window.location.href='ramal_pabx_autorizado_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ramal_pabx_autorizado_edit.php','".$skeys."','',function(){window.location.href='ramal_pabx_autorizado_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_ramal_pabx"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_ramal_pabx"]));
...
//	flg_disp - Checkbox
		...
		$value="";
				$value = GetData($data,"flg_disp", "Checkbox");
		$showValues[] = $value;
		$showFields[] = "flg_disp";
				$showRawValues[] = substr($data["flg_disp"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
{
				$value = ProcessLargeText(GetData($data,"ramal", ""),"field=ramal","",MODE_VIEW);
...
//	control - flg_disp
$control_flg_disp=array();
$control_flg_disp["func"]="xt_buildeditcontrol";
$control_flg_disp["params"] = array();
$control_flg_disp["params"]["field"]="flg_disp";
$control_flg_disp["params"]["value"]=@$data["flg_disp"];
//	Begin Add validation
$arrValidate = array();	


$control_flg_disp["params"]["validate"]=$arrValidate;
//	End Add validation
$control_flg_disp["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_flg_disp["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_flg_disp["params"]["mode"]="inline_edit";
else
	$control_flg_disp["params"]["mode"]="edit";
if(!$detailKeys || !in_array("flg_disp", $detailKeys))	
	$xt->assign("flg_disp_editcontrol",$control_flg_disp);
else if(array_key_exists("flg_disp",$data))
{
				$value = GetData($data,"flg_disp", "Checkbox");
		$xt->assign("flg_disp_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ramal_pabx_autorizado_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
