...
$mode=SEARCH_SIMPLE;
$templatefile = "Rel__Pesquisa_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "Rel__Pesquisa_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'Rel__Pesquisa';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "Rel. Pesquisa";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("nm_ramal");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "nm_ramal";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("nm_ramal_label","<label for=\"".GetInputElementId("nm_ramal", $id)."\">","</label>");
	else 
		$xt->assign("nm_ramal_label", true);
	
	$xt->assign("nm_ramal_fieldblock", true);		
	$xt->assignbyref("nm_ramal_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("nm_ramal_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("nm_ramal_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_nm_ramal", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("nm_ramal", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("nm_ramal")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	...
	{
	$contents_block["begin"].="action=\"Rel__Pesquisa_report.php\" ";
	}
