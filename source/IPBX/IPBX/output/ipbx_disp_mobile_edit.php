...
include("include/ipbx_disp_mobile_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_disp_mobile_inline_edit.htm" : "ipbx_disp_mobile_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_idm"]=postvalue("editid1");
$savedKeys["id_idm"]=postvalue("editid1");
...
//	processibng name - end
	...
	if($condition)
	{
	$value = postvalue("value_id_dispositivo_".$id);
	$type=postvalue("type_id_dispositivo_".$id);
	if(FieldSubmitted("id_dispositivo_".$id))
	{
		
		$value=prepare_for_db("id_dispositivo",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "id_dispositivo"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["id_dispositivo"]=$value;
		
		
	}


//	processibng id_dispositivo - end
	}
	$condition = $inlineedit;

	if($condition)
	{
	$value = postvalue("value_dsc_dispositivo_".$id);
	$type=postvalue("type_dsc_dispositivo_".$id);
	if(FieldSubmitted("dsc_dispositivo_".$id))
	{
		
		$value=prepare_for_db("dsc_dispositivo",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "dsc_dispositivo"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["dsc_dispositivo"]=$value;
		
		
	}


//	processibng dsc_dispositivo - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_idm"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_disp_mobile_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_disp_mobile->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_disp_mobile_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
$readonlyfields=array();
  ...
  $readonlyfields["id_dispositivo"] = htmlspecialchars(GetData($data,"id_dispositivo", ""));

if($readevalues)
{
	$data["name"]=$evalues["name"];
	$data["dsc_dispositivo"]=$evalues["dsc_dispositivo"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_idm"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_idm", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_disp_mobile_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_idm"))
		$key[]=GetFieldIndex("id_idm");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_disp_mobile_edit.php','".$skeys."','',function(){window.location.href='ipbx_disp_mobile_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_disp_mobile_edit.php','".$skeys."','',function(){window.location.href='ipbx_disp_mobile_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_idm"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_idm"]));


//	id_dispositivo - 

		$value="";
				$value = ProcessLargeText(GetData($data,"id_dispositivo", ""),"field=id%5Fdispositivo".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "id_dispositivo";
				$showRawValues[] = substr($data["id_dispositivo"],0,100);

//	dsc_dispositivo - 

		$value="";
				$value = ProcessLargeText(GetData($data,"dsc_dispositivo", ""),"field=dsc%5Fdispositivo".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dsc_dispositivo";
				$showRawValues[] = substr($data["dsc_dispositivo"],0,100);
...
{
				$value=DisplayLookupWizard("name",$data["name"],$data,"",MODE_VIEW);
		$xt->assign("name_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - id_dispositivo
$control_id_dispositivo=array();
$control_id_dispositivo["func"]="xt_buildeditcontrol";
$control_id_dispositivo["params"] = array();
$control_id_dispositivo["params"]["field"]="id_dispositivo";
$control_id_dispositivo["params"]["value"]=@$data["id_dispositivo"];
//	Begin Add validation
$arrValidate = array();	

$control_id_dispositivo["params"]["validate"]=$arrValidate;
//	End Add validation
$control_id_dispositivo["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_id_dispositivo["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_id_dispositivo["params"]["mode"]="inline_edit";
else
	$control_id_dispositivo["params"]["mode"]="edit";
if(!$detailKeys || !in_array("id_dispositivo", $detailKeys))	
	$xt->assign("id_dispositivo_editcontrol",$control_id_dispositivo);
else if(array_key_exists("id_dispositivo",$data))
{
				$value = ProcessLargeText(GetData($data,"id_dispositivo", ""),"field=id%5Fdispositivo","",MODE_VIEW);
		$xt->assign("id_dispositivo_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - dsc_dispositivo
$control_dsc_dispositivo=array();
$control_dsc_dispositivo["func"]="xt_buildeditcontrol";
$control_dsc_dispositivo["params"] = array();
$control_dsc_dispositivo["params"]["field"]="dsc_dispositivo";
$control_dsc_dispositivo["params"]["value"]=@$data["dsc_dispositivo"];
//	Begin Add validation
$arrValidate = array();	

$arrValidate['basicValidate'][] = "IsRequired";

$control_dsc_dispositivo["params"]["validate"]=$arrValidate;
//	End Add validation
$control_dsc_dispositivo["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_dsc_dispositivo["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_dispositivo["params"]["mode"]="inline_edit";
else
	$control_dsc_dispositivo["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_dispositivo", $detailKeys))	
	$xt->assign("dsc_dispositivo_editcontrol",$control_dsc_dispositivo);
else if(array_key_exists("dsc_dispositivo",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_dispositivo", ""),"field=dsc%5Fdispositivo","",MODE_VIEW);
		$xt->assign("dsc_dispositivo_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_disp_mobile_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
