...
include("include/ipbx_horario_desv_variables.php");
...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_horario_desv_inline_add.htm";
else
	$templatefile = "ipbx_horario_desv_add.htm";
...
{
	$ids = $id;
	$mKeys["ipbx_horario_desv_ramais"] = GetMasterKeysByDetailTable("ipbx_horario_desv_ramais", $strTableName);
	$dpParams['strTableNames'][] = "ipbx_horario_desv_ramais";
	$dpParams['ids'][] = ++$ids;
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			'mTableName':'".jsreplace($strTableName)."',
			'mShortTableName':'ipbx_horario_desv', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_horario_desv_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_horario_desv_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_horario_desv_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_horario_desv_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_horario_desv_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_horario_desv_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
{
	$LookupSQL="";
	if(postvalue("table")=="ipbx_ramais" && postvalue("field")=="id_desvio") 
	{
	$linkfield = "id_desvio";
	...
	if(postvalue("table")=="personal_info" && postvalue("field")=="id_desvio") 
	{
	$linkfield = "id_desvio";
	$dispfield = "dsc_desvio";
	$LookupSQL = "select ";
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$masterquery="mastertable=ipbx%5Fhorario%5Fdesv";
	$masterquery.="&masterkey1=".rawurlencode($data["id_desvio"]);
	$showDetailKeys["ipbx_horario_desv_ramais"]=$masterquery;
