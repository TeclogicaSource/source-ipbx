...
include("include/ipbx_usuarios_variables.php");
...
$templatefile = "ipbx_usuarios_edit.htm";
...
$savedKeys["id_usuario"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_usuario"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_usuarios_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_usuarios->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_usuarios_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_usuario"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_usuario", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_usuarios_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_usuario"))
		$key[]=GetFieldIndex("id_usuario");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_usuarios_edit.php','".$skeys."','',function(){window.location.href='ipbx_usuarios_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_usuarios_edit.php','".$skeys."','',function(){window.location.href='ipbx_usuarios_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_usuario"]);
if($message)
{
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_usuarios_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
