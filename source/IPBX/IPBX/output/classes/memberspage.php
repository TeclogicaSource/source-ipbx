...
{	
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			...
			foreach(postvalue("username") as $user)
			{
				...
				$sql="delete from `ipbx_ugmembers` where UserName='".db_addslashes($user)."'";
				...
				if(count($_POST["cb_".$useridx]))
				{
					...
					foreach(@$_POST["cb_".$useridx] as $g)
					{
						if($g<0)
						{
							if($user==$_SESSION["UserID"] && $g==-1)
								continue;
							$sql="insert into `ipbx_ugmembers` (UserName,GroupID) values ('".htmlspecialchars($user)."',".$g.")";
							db_exec($sql,$this->conn);
						}
						else
						{
					...
					if($glist!="")
					{
						$sql="insert into `ipbx_ugmembers` (UserName,GroupID) select '".htmlspecialchars($user)."',GroupID from `ipbx_uggroups` where GroupID in (".$glist.")";
						db_exec($sql,$this->conn);
					}
				}
			}
	...
	{							
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize))
		{
			...
			$row["begin"]="<input type=hidden name=\"username[]\" value=\"".htmlspecialchars($data["name"])."\">";
			$row["username"]=htmlspecialchars($data["name"]);
			$row["user"]=$data["name"];
			$usercheckbox_attrs.=" name=\"user_".htmlspecialchars($data["name"])."\"";
	...
	{
		//	select members list		
		$trs = db_query("select UserName,GroupID from `ipbx_ugmembers` order by UserName,GroupID",$this->conn);
	...
	function fillGroups() 
	{
		...
		$trs = db_query("select GroupID,Label from `ipbx_uggroups` order by Label",$this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
