...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_desv_prefix_inline_add.htm";
else
	$templatefile = "ipbx_desv_prefix_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_desv_prefix', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_desv_prefix_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_desv_prefix_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_desv_prefix_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_desv_prefix"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_desv_prefix"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["dsc_grp_pref"]=@$avalues["dsc_grp_pref"];
}
//for basic files
$includes="";
...
if($inlineadd!=ADD_INLINE)
{
	...
	$xt->assign("dsc_grp_pref_fieldblock",true);
	$xt->assign("dsc_grp_pref_label",true);
	if(isEnableSection508())
		$xt->assign_section("dsc_grp_pref_label","<label for=\"".GetInputElementId("dsc_grp_pref", $id)."\">","</label>");
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_desv_prefix_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_desv_prefix_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_desv_prefix_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
{
	$LookupSQL="";
	if(postvalue("table")=="cc_menu_atend_item" && postvalue("field")=="prefixo") 
	{
	$linkfield = "id_desv_prefix";
	$dispfield = "dsc_grp_pref";
	$LookupSQL = "select ";
	$LookupSQL .= "`id_desv_prefix`";
		$LookupSQL .= ",`dsc_grp_pref`";
	}
	if($LookupSQL)
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_desv_prefix"]);
...
$control_dsc_grp_pref["params"]["value"]=@$defvalues["dsc_grp_pref"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dsc_grp_pref["params"]["mode"]="inline_add";
else
	$control_dsc_grp_pref["params"]["mode"]="add";

if(!$detailKeys || !in_array("dsc_grp_pref", $detailKeys))
	$xt->assignbyref("dsc_grp_pref_editcontrol",$control_dsc_grp_pref);
else if(array_key_exists("dsc_grp_pref", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dsc_grp_pref", ""),"field=dsc%5Fgrp%5Fpref","",MODE_VIEW);
