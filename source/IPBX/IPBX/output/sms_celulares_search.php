...
$mode=SEARCH_SIMPLE;
$templatefile = "sms_celulares_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "sms_celulares_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'sms_celulares';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "sms_celulares";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("id_grupo");
	...
	{
		$firstFieldParams['fName'] = "id_grupo";
	...
	$xt->assignbyref("id_grupo_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("id_grupo_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("id_grupo_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_id_grupo", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("id_grupo", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("id_grupo")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("dsc_nome");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "dsc_nome";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("dsc_nome_label","<label for=\"".GetInputElementId("dsc_nome", $id)."\">","</label>");
	else 
		$xt->assign("dsc_nome_label", true);
	
	$xt->assign("dsc_nome_fieldblock", true);		
	$xt->assignbyref("dsc_nome_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("dsc_nome_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("dsc_nome_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_dsc_nome", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("dsc_nome", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("dsc_nome")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	...
	{
	$contents_block["begin"].="action=\"sms_celulares_list.php\" ";
	}
