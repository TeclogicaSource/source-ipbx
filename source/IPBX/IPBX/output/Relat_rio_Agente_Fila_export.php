...
$pageObject->body["begin"] .="<form action=\"Relat_rio_Agente_Fila_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Relat_rio_Agente_Fila_export.htm");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Relat_rio_Agente_Fila.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["Horário de Atendimento"] = GetData($row,"Horário de Atendimento","");
			...
			$values["Chave Única"] = GetData($row,"Chave Única","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Relat_rio_Agente_Fila.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="";
			$values["Horário de Atendimento"] = htmlspecialchars(GetData($row,"Horário de Atendimento",$format));
			$format="";
			...
			$format="";
			$values["Chave Única"] = htmlspecialchars(GetData($row,"Chave Única",$format));
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Horário de Atendimento"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Chave Única"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("HorÃ¡rio de Atendimento").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Chave Ãšnica").'</td>';	
	}
	else
	{
		...
		echo "<td>"."Telefone"."</td>";
		echo "<td>"."HorÃ¡rio de Atendimento"."</td>";
		echo "<td>"."Evento"."</td>";
		echo "<td>"."Fila"."</td>";
		echo "<td>"."Agente"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["Horário de Atendimento"] = GetData($row,"Horário de Atendimento",$format);
			...
			$values["Chave Única"] = GetData($row,"Chave Única",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["Horário de Atendimento"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Chave Única"]);
					else
						echo htmlspecialchars($values["Chave Única"]);
			echo '</td>';
			echo "</tr>";
		}		
