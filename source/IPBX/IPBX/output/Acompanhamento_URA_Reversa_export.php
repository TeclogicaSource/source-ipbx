...
$pageObject->body["begin"] .="<form action=\"Acompanhamento_URA_Reversa_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Acompanhamento_URA_Reversa_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Acompanhamento_URA_Reversa.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Acompanhamento_URA_Reversa.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Acompanhamento_URA_Reversa.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["id_msg"] = GetData($row,"id_msg","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Acompanhamento_URA_Reversa.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="";
			$values["id_msg"] = htmlspecialchars(GetData($row,"id_msg",$format));
			$format="Datetime";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_msg"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Ura Rev").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Número Telefone").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Mensagem").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Data Criado").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Número de Tentat.").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Status").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Resposta Usuário").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Data Resposta").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Cliente").'</td>';	
	}
	else
	{
		echo "<td>"."Id Ura Rev"."</td>";
		echo "<td>"."Número Telefone"."</td>";
		echo "<td>"."Mensagem"."</td>";
		echo "<td>"."Data Criado"."</td>";
		echo "<td>"."Número de Tentat."."</td>";
		echo "<td>"."Status"."</td>";
		echo "<td>"."Resposta Usuário"."</td>";
		echo "<td>"."Data Resposta"."</td>";
		echo "<td>"."Cliente"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["id_msg"] = GetData($row,"id_msg",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["id_msg"]);
			echo '</td>';
							echo '<td>';
