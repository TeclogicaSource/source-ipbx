...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_interf_sip_generica_inline_add.htm";
else
	$templatefile = "ipbx_interf_sip_generica_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_interf_sip_generica', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	processing ip_host - start
	...
	if(!($value===false))
	{
		...
		if(0 && "ip_host"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["ip_host"]=$value;
	}
	}
//	processibng ip_host - end
//	processing codec - start
...
//	processing flg_id_cham_parc - start
	...
	if(!($value===false))
	{
		...
		if(0 && "flg_id_cham_parc"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["flg_id_cham_parc"]=$value;
	}
	}
...
//	processing flg_logon_remoto - start
	...
	if(!($value===false))
	{
		...
		if(0 && "flg_logon_remoto"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["flg_logon_remoto"]=$value;
	}
	}
//	processibng flg_logon_remoto - end
//	processing registro - start
    
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
	$value = postvalue("value_registro_".$id);
	$type=postvalue("type_registro_".$id);
	if (FieldSubmitted("registro_".$id))
	{
		$value=prepare_for_db("registro",$value,$type);
	}
	else
		$value=false;
	if(!($value===false))
	{
		...
		if(0 && "registro"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["registro"]=$value;
	}
	}
//	processibng registro - end
//	processing pers_params - start
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_interf_sip_generica_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_interf_sip_generica_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_interf_sip_generica_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_interf_sip_generica_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_sip_generica_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_sip_generica_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
//	registro - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"registro", ""),"field=registro".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "registro";
				$showRawValues[] = substr($data["registro"],0,100);
	}	
//	pers_params - 
	$display = false;
	if($inlineadd==ADD_INLINE)
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"pers_params", ""),"field=pers%5Fparams".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "pers_params";
				$showRawValues[] = substr($data["pers_params"],0,100);
	}	
...
$control_ip_host["params"]["value"]=@$defvalues["ip_host"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_ip_host["params"]["mode"]="inline_add";
else
	$control_ip_host["params"]["mode"]="add";

if(!$detailKeys || !in_array("ip_host", $detailKeys))
	$xt->assignbyref("ip_host_editcontrol",$control_ip_host);
else if(array_key_exists("ip_host", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"ip_host", ""),"field=ip%5Fhost","",MODE_VIEW);
...
$control_codec["params"]["value"]=@$defvalues["codec"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_codec["params"]["mode"]="inline_add";
else
	$control_codec["params"]["mode"]="add";

if(!$detailKeys || !in_array("codec", $detailKeys))
	$xt->assignbyref("codec_editcontrol",$control_codec);
else if(array_key_exists("codec", $defvalues))
{
				$value=DisplayLookupWizard("codec",$defvalues["codec"],$defvalues,"",MODE_VIEW);
...
$control_flg_id_cham_parc["params"]["value"]=@$defvalues["flg_id_cham_parc"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_flg_id_cham_parc["params"]["mode"]="inline_add";
else
	$control_flg_id_cham_parc["params"]["mode"]="add";

if(!$detailKeys || !in_array("flg_id_cham_parc", $detailKeys))
	$xt->assignbyref("flg_id_cham_parc_editcontrol",$control_flg_id_cham_parc);
else if(array_key_exists("flg_id_cham_parc", $defvalues))
{
				$value = GetData($defvalues,"flg_id_cham_parc", "Checkbox");
...
$control_flg_logon_remoto["params"]["value"]=@$defvalues["flg_logon_remoto"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_flg_logon_remoto["params"]["mode"]="inline_add";
else
	$control_flg_logon_remoto["params"]["mode"]="add";

if(!$detailKeys || !in_array("flg_logon_remoto", $detailKeys))
	$xt->assignbyref("flg_logon_remoto_editcontrol",$control_flg_logon_remoto);
else if(array_key_exists("flg_logon_remoto", $defvalues))
{
				$value = GetData($defvalues,"flg_logon_remoto", "Checkbox");
		$xt->assign("flg_logon_remoto_editcontrol",$value);
}
// add prevent submit on enter js if only one text record
//	control - registro
$control_registro=array();
$control_registro["func"]="xt_buildeditcontrol";
$control_registro["params"] = array();
$control_registro["params"]["field"]="registro";
$control_registro["params"]["value"]=@$defvalues["registro"];

//	Begin Add validation
$arrValidate = array();	
$control_registro["params"]["validate"]=$arrValidate;
//	End Add validation

$control_registro["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_registro["params"]["mode"]="inline_add";
else
	$control_registro["params"]["mode"]="add";

if(!$detailKeys || !in_array("registro", $detailKeys))
	$xt->assignbyref("registro_editcontrol",$control_registro);
else if(array_key_exists("registro", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"registro", ""),"field=registro","",MODE_VIEW);
		$xt->assign("registro_editcontrol",$value);
}
// add prevent submit on enter js if only one text record
//	control - pers_params
...
$control_pers_params["params"]["value"]=@$defvalues["pers_params"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_pers_params["params"]["mode"]="inline_add";
else
	$control_pers_params["params"]["mode"]="add";

if(!$detailKeys || !in_array("pers_params", $detailKeys))
	$xt->assignbyref("pers_params_editcontrol",$control_pers_params);
else if(array_key_exists("pers_params", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"pers_params", ""),"field=pers%5Fparams","",MODE_VIEW);
