...
if($inlineadd==ADD_INLINE)
	$templatefile = "troncos_inline_add.htm";
else
	$templatefile = "troncos_add.htm";
...
//	processing mcdu_inicio - start
	...
	if(!($value===false))
	{
		...
		if(0 && "mcdu_inicio"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["mcdu_inicio"]=$value;
	}
	}
//	processibng mcdu_inicio - end
//	processing mcdu_fim - start
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'troncos_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'troncos_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: troncos_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="troncos_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="troncos_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
//	mcdu_inicio - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"mcdu_inicio", ""),"field=mcdu%5Finicio".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "mcdu_inicio";
				$showRawValues[] = substr($data["mcdu_inicio"],0,100);
	}	
//	mcdu_fim - 
	$display = false;
	if($inlineadd==ADD_MASTER)
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"mcdu_fim", ""),"field=mcdu%5Ffim".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "mcdu_fim";
				$showRawValues[] = substr($data["mcdu_fim"],0,100);
	}	
//	tp_toques - 
	$display = false;
	if($inlineadd==ADD_MASTER)
...
{
				$value=DisplayLookupWizard("id_interf",$defvalues["id_interf"],$defvalues,"",MODE_VIEW);
...
$control_mcdu_inicio["params"]["value"]=@$defvalues["mcdu_inicio"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_mcdu_inicio["params"]["mode"]="inline_add";
else
	$control_mcdu_inicio["params"]["mode"]="add";

if(!$detailKeys || !in_array("mcdu_inicio", $detailKeys))
	$xt->assignbyref("mcdu_inicio_editcontrol",$control_mcdu_inicio);
else if(array_key_exists("mcdu_inicio", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"mcdu_inicio", ""),"field=mcdu%5Finicio","",MODE_VIEW);
...
$control_mcdu_fim["params"]["value"]=@$defvalues["mcdu_fim"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_mcdu_fim["params"]["mode"]="inline_add";
else
	$control_mcdu_fim["params"]["mode"]="add";

if(!$detailKeys || !in_array("mcdu_fim", $detailKeys))
	$xt->assignbyref("mcdu_fim_editcontrol",$control_mcdu_fim);
else if(array_key_exists("mcdu_fim", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"mcdu_fim", ""),"field=mcdu%5Ffim","",MODE_VIEW);
...
{
				$value = GetData($defvalues,"flg_grava_ent", "Checkbox");
...
{
				$value = GetData($defvalues,"flg_grava_sai", "Checkbox");
...
{
				$value = GetData($defvalues,"flg_pabx_remoto", "Checkbox");
...
{
				$value = GetData($defvalues,"flg_cel_local", "Checkbox");
...
{
				$value = GetData($defvalues,"flg_local", "Checkbox");
...
{
				$value = GetData($defvalues,"flg_fixo_ddd", "Checkbox");
...
{
				$value = GetData($defvalues,"flg_cel_ddd", "Checkbox");
...
{
				$value = GetData($defvalues,"flg_ddi", "Checkbox");
