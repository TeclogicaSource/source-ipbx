...
include("include/Rel_Chamada_Detalhada_variables.php");
...
$mode=SEARCH_SIMPLE;
$templatefile = "Rel_Chamada_Detalhada_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "Rel_Chamada_Detalhada_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'Rel_Chamada_Detalhada';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "Rel. Cham Detalhada";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("Terminado");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Terminado";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Terminado_label","<label for=\"".GetInputElementId("Terminado", $id)."\">","</label>");
	else 
		$xt->assign("Terminado_label", true);
	
	$xt->assign("Terminado_fieldblock", true);		
	$xt->assignbyref("Terminado_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Terminado_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Terminado_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Terminado", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("Terminado", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("Terminado")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("Obs.");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("Obs__label","<label for=\"".GetInputElementId("Obs.", $id)."\">","</label>");
	else 
		$xt->assign("Obs__label", true);
	
	$xt->assign("Obs__fieldblock", true);		
	$xt->assignbyref("Obs__editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Obs__notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Obs__editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Obs_", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("Obs.", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("Obs.")."', ".$pageObject->id.", ".$ctrlsMap.");";
	...
	{
	$contents_block["begin"].="action=\"Rel_Chamada_Detalhada_report.php\" ";
	}
