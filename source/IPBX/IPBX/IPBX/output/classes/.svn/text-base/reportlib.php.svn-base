...
class SQLStatement
{
	...
	function SQLStatement($sql, $order, $pagesize, $connection, &$searchClauseObj, &$params)
	{
        ...
        {	
			...
			{
				$this->_aggregates []= 'MAX('.cached_ffn($this->fieldsArr[$i]['name']).') as '.cached_ffn(GoodFieldName($this->fieldsArr[$i]['name']."MAX"));
            }
            if ($this->fieldsArr[$i]['totalMin']){		
				$this->_aggregates []= 'MIN('.cached_ffn($this->fieldsArr[$i]['name']).') as '.cached_ffn(GoodFieldName($this->fieldsArr[$i]['name']."MIN"));
            }
            if ($this->fieldsArr[$i]['totalAvg']){	
	            if (!IsDateFieldType(GetFieldType($this->fieldsArr[$i]['name'], $this->tName)))
	            {  
					$this->_aggregates []= 'AVG('.cached_ffn($this->fieldsArr[$i]['name']).') as '.cached_ffn(GoodFieldName($this->fieldsArr[$i]['name']."AVG"));
					$this->_aggregates []= 'COUNT('.cached_ffn($this->fieldsArr[$i]['name']).') as '.cached_ffn(GoodFieldName($this->fieldsArr[$i]['name']."NAVG"));
	            }
			}
			if ($this->fieldsArr[$i]['totalSum']){
				if (!IsDateFieldType(GetFieldType($this->fieldsArr[$i]['name'], $this->tName)))
	            {
	          		$this->_aggregates []= 'SUM('.cached_ffn($this->fieldsArr[$i]['name']).') as '.cached_ffn(GoodFieldName($this->fieldsArr[$i]['name']."SUM"));
	            }
			}
        }
...
class ReportLogic extends Summarable
{
	...
	function getReport($from = 0)
	{
		...
		if(!$this->_recordBasedRequest)
		{
			...
			if (event_exists('BeforeQueryReport'))
			{
				$hwhere = $hsql['where'];
	...
	function getFormattedRow($value)
	{
		...
		{		
			...
			if ($this->fieldsArr[$i]['viewFormat'] == FORMAT_DATABASE_IMAGE)
			{
				...
				{
					$val = "Dados Binários longos demais, Não pode ser exibido";
				}
