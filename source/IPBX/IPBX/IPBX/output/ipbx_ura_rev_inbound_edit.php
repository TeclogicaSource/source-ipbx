...
include("include/ipbx_ura_rev_inbound_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_ura_rev_inbound_inline_edit.htm" : "ipbx_ura_rev_inbound_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_ura_rev_inbount"]=postvalue("editid1");
$savedKeys["id_ura_rev_inbount"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_ura_rev_inbount"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_ura_rev_inbound_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_ura_rev_inbound->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_ura_rev_inbound_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_ura_rev_inbount"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_ura_rev_inbount", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_ura_rev_inbound_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_ura_rev_inbount"))
		$key[]=GetFieldIndex("id_ura_rev_inbount");
...
//create a line where and order for the two queries
	if($lenArr>0 and count($key) and !$_SESSION[$strTableName."_noNextPrev"])
	{
		...
		if($where_next!="" and $order_next!="" and $where_prev!="" and $order_prev!="")
		{
			...
			{
					$next[1]=$row_next["id_ura_rev_inbount"];
			}
			
			$res_prev=db_query($sql_prev,$conn);	
			if($row_prev=db_fetch_array($res_prev))
			{
					$prev[1]=$row_prev["id_ura_rev_inbount"];
			}
		}
	}
}
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_ura_rev_inbound_edit.php','".$skeys."','',function(){window.location.href='ipbx_ura_rev_inbound_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_ura_rev_inbound_edit.php','".$skeys."','',function(){window.location.href='ipbx_ura_rev_inbound_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_ura_rev_inbount"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_ura_rev_inbount"]));
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_ura_rev_inbound_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
