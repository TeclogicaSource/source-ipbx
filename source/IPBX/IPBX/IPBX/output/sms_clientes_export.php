...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_cliente"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["id_cliente"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"sms_clientes_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("sms_clientes_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=sms_clientes.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=sms_clientes.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=sms_clientes.xml");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=sms_clientes.csv");
