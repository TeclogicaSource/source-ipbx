...
include("include/ipbx_conf_fax_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_conf_fax_inline_edit.htm" : "ipbx_conf_fax_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_conf_fax"]=postvalue("editid1");
$savedKeys["id_conf_fax"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_tp_reter_fax_".$id);
	$type=postvalue("type_tp_reter_fax_".$id);
	if(FieldSubmitted("tp_reter_fax_".$id))
	{
		
		$value=prepare_for_db("tp_reter_fax",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "tp_reter_fax"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["tp_reter_fax"]=$value;
		
		
	}


//	processibng tp_reter_fax - end
	}
	$condition = $inlineedit;

	if($condition)
	{
	$value = postvalue("value_pasta_".$id);
	$type=postvalue("type_pasta_".$id);
	if(FieldSubmitted("pasta_".$id))
	{
		
		$value=prepare_for_db("pasta",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "pasta"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["pasta"]=$value;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_conf_fax"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_conf_fax_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_conf_fax->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_conf_fax_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["tp_reter_fax"]=$evalues["tp_reter_fax"];
	$data["pasta"]=$evalues["pasta"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_conf_fax"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_conf_fax", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_conf_fax_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_conf_fax"))
		$key[]=GetFieldIndex("id_conf_fax");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_conf_fax_edit.php','".$skeys."','',function(){window.location.href='ipbx_conf_fax_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_conf_fax_edit.php','".$skeys."','',function(){window.location.href='ipbx_conf_fax_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_conf_fax"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_conf_fax"]));


//	tp_reter_fax - 

		$value="";
				$value = ProcessLargeText(GetData($data,"tp_reter_fax", ""),"field=tp%5Freter%5Ffax".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "tp_reter_fax";
				$showRawValues[] = substr($data["tp_reter_fax"],0,100);

//	pasta - 

		$value="";
				$value = ProcessLargeText(GetData($data,"pasta", ""),"field=pasta".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "pasta";
				$showRawValues[] = substr($data["pasta"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$regexmessage='';
$regextype = '';
//	control - tp_reter_fax
$control_tp_reter_fax=array();
$control_tp_reter_fax["func"]="xt_buildeditcontrol";
$control_tp_reter_fax["params"] = array();
$control_tp_reter_fax["params"]["field"]="tp_reter_fax";
$control_tp_reter_fax["params"]["value"]=@$data["tp_reter_fax"];
...
$control_tp_reter_fax["params"]["validate"]=$arrValidate;
//	End Add validation
$control_tp_reter_fax["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_tp_reter_fax["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_tp_reter_fax["params"]["mode"]="inline_edit";
else
	$control_tp_reter_fax["params"]["mode"]="edit";
if(!$detailKeys || !in_array("tp_reter_fax", $detailKeys))	
	$xt->assign("tp_reter_fax_editcontrol",$control_tp_reter_fax);
else if(array_key_exists("tp_reter_fax",$data))
{
				$value = ProcessLargeText(GetData($data,"tp_reter_fax", ""),"field=tp%5Freter%5Ffax","",MODE_VIEW);
		$xt->assign("tp_reter_fax_editcontrol",$value);
}
...
//	control - pasta
$control_pasta=array();
$control_pasta["func"]="xt_buildeditcontrol";
$control_pasta["params"] = array();
$control_pasta["params"]["field"]="pasta";
$control_pasta["params"]["value"]=@$data["pasta"];
//	Begin Add validation
$arrValidate = array();	

$control_pasta["params"]["validate"]=$arrValidate;
//	End Add validation
$control_pasta["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_pasta["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_pasta["params"]["mode"]="inline_edit";
else
	$control_pasta["params"]["mode"]="edit";
if(!$detailKeys || !in_array("pasta", $detailKeys))	
	$xt->assign("pasta_editcontrol",$control_pasta);
else if(array_key_exists("pasta",$data))
{
				$value = ProcessLargeText(GetData($data,"pasta", ""),"field=pasta","",MODE_VIEW);
		$xt->assign("pasta_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_conf_fax_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
