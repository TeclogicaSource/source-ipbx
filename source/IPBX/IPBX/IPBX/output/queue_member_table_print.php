...
include("include/queue_member_table_variables.php");
...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["uniqueid"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
//	fill $rowinfo array
	...
	while($data && ($all || $recno<=$PageSize))
	{
		...
		for($col=1;$data && ($all || $recno<=$PageSize) && $col<=1;$col++)
		{
			...
			$records++;
			$keylink="";
			$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["uniqueid"]));


//	membername - 
			$value="";
				$value = ProcessLargeText(GetData($data,"membername", ""),"field=membername".$keylink,"",MODE_PRINT);
			$record["membername_value"]=$value;

//	queue_name - 
			$value="";
				$value = ProcessLargeText(GetData($data,"queue_name", ""),"field=queue%5Fname".$keylink,"",MODE_PRINT);
			$record["queue_name_value"]=$value;
...
//	penalty - 
			$value="";
				$value = ProcessLargeText(GetData($data,"penalty", ""),"field=penalty".$keylink,"",MODE_PRINT);
			$record["penalty_value"]=$value;

//	paused - Checkbox
			$value="";
				$value = GetData($data,"paused", "Checkbox");
			$record["paused_value"]=$value;
			if($col<$colsonpage)
...
$xt->assign("membername_fieldheadercolumn",true);
$xt->assign("membername_fieldheader",true);
$xt->assign("membername_fieldcolumn",true);
$xt->assign("membername_fieldfootercolumn",true);
$xt->assign("queue_name_fieldheadercolumn",true);
$xt->assign("queue_name_fieldheader",true);
$xt->assign("queue_name_fieldcolumn",true);
$xt->assign("queue_name_fieldfootercolumn",true);
...
$xt->assign("penalty_fieldheadercolumn",true);
$xt->assign("penalty_fieldheader",true);
$xt->assign("penalty_fieldcolumn",true);
$xt->assign("penalty_fieldfootercolumn",true);
$xt->assign("paused_fieldheadercolumn",true);
$xt->assign("paused_fieldheader",true);
$xt->assign("paused_fieldcolumn",true);
$xt->assign("paused_fieldfootercolumn",true);
...
$templatefile = "queue_member_table_print.htm";
