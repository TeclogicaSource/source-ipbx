...
$templatefile = ( $inlineedit ) ? "interf_khomp_e1_inline_edit.htm" : "interf_khomp_e1_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_k_e1"]=postvalue("editid1");
$savedKeys["id_k_e1"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_k_e1"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: interf_khomp_e1_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_interf_khomp_e1->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: interf_khomp_e1_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_k_e1"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_k_e1", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="interf_khomp_e1_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_k_e1"))
		$key[]=GetFieldIndex("id_k_e1");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('interf_khomp_e1_edit.php','".$skeys."','',function(){window.location.href='interf_khomp_e1_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('interf_khomp_e1_edit.php','".$skeys."','',function(){window.location.href='interf_khomp_e1_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_k_e1"]);
if($message)
{
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('interf_khomp_e1_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
