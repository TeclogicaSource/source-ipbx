...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_tarifa_vc2_inline_add.htm";
else
	$templatefile = "ipbx_tarifa_vc2_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_tarifa_vc2', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_tarifa_vc2_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_tarifa_vc2_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_tarifa_vc2_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_tarifa_vc2_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_tarifa_vc2_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_tarifa_vc2_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
$control_prefixo["params"]["value"]=@$defvalues["prefixo"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_prefixo["params"]["mode"]="inline_add";
else
	$control_prefixo["params"]["mode"]="add";

if(!$detailKeys || !in_array("prefixo", $detailKeys))
	$xt->assignbyref("prefixo_editcontrol",$control_prefixo);
else if(array_key_exists("prefixo", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"prefixo", ""),"field=prefixo","",MODE_VIEW);
