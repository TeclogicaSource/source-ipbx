...
include("include/ipbx_interf_callman_variables.php");
...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_interf_callman_inline_add.htm";
else
	$templatefile = "ipbx_interf_callman_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_interf_callman', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
//	processing dtmf - start
    
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
	$value = postvalue("value_dtmf_".$id);
	$type=postvalue("type_dtmf_".$id);
	if (FieldSubmitted("dtmf_".$id))
	{
		$value=prepare_for_db("dtmf",$value,$type);
	}
	else
		$value=false;
	if(!($value===false))
	{
		...
		if(0 && "dtmf"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["dtmf"]=$value;
	}
	}
//	processibng dtmf - end
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_interf_callman_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_interf_callman_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_interf_callman_".$pageObject->getPageType().".php");
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_interf_callman_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_callman_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_callman_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
//	dtmf - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"dtmf", ""),"field=dtmf".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dtmf";
				$showRawValues[] = substr($data["dtmf"],0,100);
	}	
//	id_tp_interf - 
	$display = false;
	if($inlineadd==ADD_INLINE)
...
//	control - dtmf
$control_dtmf=array();
$control_dtmf["func"]="xt_buildeditcontrol";
$control_dtmf["params"] = array();
$control_dtmf["params"]["field"]="dtmf";
$control_dtmf["params"]["value"]=@$defvalues["dtmf"];

//	Begin Add validation
$arrValidate = array();	
$control_dtmf["params"]["validate"]=$arrValidate;
//	End Add validation

$control_dtmf["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dtmf["params"]["mode"]="inline_add";
else
	$control_dtmf["params"]["mode"]="add";

if(!$detailKeys || !in_array("dtmf", $detailKeys))
	$xt->assignbyref("dtmf_editcontrol",$control_dtmf);
else if(array_key_exists("dtmf", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dtmf", ""),"field=dtmf","",MODE_VIEW);
		$xt->assign("dtmf_editcontrol",$value);
}
// add prevent submit on enter js if only one text record
