...
if ( $rowcount ) {
	...
	//	id - 
		    $value="";
				$value=DisplayLookupWizard("id",$data["id"],$data,$keylink,MODE_PRINT);
			$row["id_value"]=$value;
	//	dsc_plano - 
		    $value="";
				$value = ProcessLargeText(GetData($data,"dsc_plano", ""),"field=dsc%5Fplano".$keylink,"",MODE_PRINT);
			$row["dsc_plano_value"]=$value;
	//	hr_inicio - Time
		    $value="";
...
$xt->display("sip_users_horario_detailspreview.htm");
if($mode!="inline")
