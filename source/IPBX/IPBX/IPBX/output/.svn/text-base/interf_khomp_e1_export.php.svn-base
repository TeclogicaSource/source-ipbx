...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_k_e1"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"interf_khomp_e1_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("interf_khomp_e1_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=interf_khomp_e1.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=interf_khomp_e1.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=interf_khomp_e1.xml");
	...
	{
		$values = array();
			$values["id_k_e1"] = GetData($row,"id_k_e1","");
			$values["dsc_interf"] = GetData($row,"dsc_interf","");
			$values["id_contrato"] = GetData($row,"id_contrato","");
			$values["board"] = GetData($row,"board","");
			$values["link"] = GetData($row,"link","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=interf_khomp_e1.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["id_k_e1"] = htmlspecialchars(GetData($row,"id_k_e1",$format));
			$format="";
			...
			$format="";
			$values["board"] = htmlspecialchars(GetData($row,"board",$format));
			$format="";
			$values["link"] = htmlspecialchars(GetData($row,"link",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_k_e1"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["board"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["link"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id K E1").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Dsc Interf").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Contrato").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Board").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Link").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Central").'</td>';	
	}
	else
	{
		echo "<td>"."Id K E1"."</td>";
		echo "<td>"."Dsc Interf"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["id_k_e1"] = GetData($row,"id_k_e1",$format);
			...
			$values["board"] = GetData($row,"board",$format);
					
							$format="";
			
			$values["link"] = GetData($row,"link",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["id_k_e1"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
			...
			echo '</td>';
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["board"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="";
									echo htmlspecialchars($values["link"]);
			echo '</td>';
							echo '<td>';
