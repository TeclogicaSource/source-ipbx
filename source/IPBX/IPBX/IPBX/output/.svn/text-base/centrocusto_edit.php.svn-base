...
$templatefile = "centrocusto_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["idt_custo"]=postvalue("editid1");
$savedKeys["idt_custo"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idt_custo"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: centrocusto_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_centrocusto->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: centrocusto_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_centro_cust"]=$evalues["dsc_centro_cust"];
	$data["flg_ativado"]=$evalues["flg_ativado"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["idt_custo"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"idt_custo", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="centrocusto_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("idt_custo"))
		$key[]=GetFieldIndex("idt_custo");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('centrocusto_edit.php','".$skeys."','',function(){window.location.href='centrocusto_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('centrocusto_edit.php','".$skeys."','',function(){window.location.href='centrocusto_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["idt_custo"]);
if($message)
{
...
$control_dsc_centro_cust["params"]["value"]=@$data["dsc_centro_cust"];
...
$control_dsc_centro_cust["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_centro_cust["params"]["mode"]="inline_edit";
else
	$control_dsc_centro_cust["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_centro_cust", $detailKeys))	
	$xt->assign("dsc_centro_cust_editcontrol",$control_dsc_centro_cust);
else if(array_key_exists("dsc_centro_cust",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_centro_cust", ""),"field=dsc%5Fcentro%5Fcust","",MODE_VIEW);
...
$control_flg_ativado["params"]["value"]=@$data["flg_ativado"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_flg_ativado["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_flg_ativado["params"]["mode"]="inline_edit";
else
	$control_flg_ativado["params"]["mode"]="edit";
if(!$detailKeys || !in_array("flg_ativado", $detailKeys))	
	$xt->assign("flg_ativado_editcontrol",$control_flg_ativado);
else if(array_key_exists("flg_ativado",$data))
{
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('centrocusto_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
