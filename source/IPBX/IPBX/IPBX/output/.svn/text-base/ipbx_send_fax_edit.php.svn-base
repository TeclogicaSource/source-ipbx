...
$templatefile = "ipbx_send_fax_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_send_fax"]=postvalue("editid1");
$savedKeys["id_send_fax"]=postvalue("editid1");
...
//	processibng nm_telefone - end
	...
	if(FieldSubmitted("arq_fax_".$id))
	{
		...
		{		
			$value = prepare_file($value,"arq_fax",$type,$fileNameForPrepareFunc ,$id);
		}
		else if(substr($type,0,6)=="upload")
		{		
			$value=prepare_upload("arq_fax",$type,$fileNameForPrepareFunc,$value,"" ,$id);
		}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_send_fax"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_send_fax_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_send_fax->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_send_fax_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["nm_telefone"]=$evalues["nm_telefone"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_send_fax"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_send_fax", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_send_fax_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_send_fax"))
		$key[]=GetFieldIndex("id_send_fax");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_send_fax_edit.php','".$skeys."','',function(){window.location.href='ipbx_send_fax_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_send_fax_edit.php','".$skeys."','',function(){window.location.href='ipbx_send_fax_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_send_fax"]);
if($message)
{
...
$control_nm_telefone["params"]["value"]=@$data["nm_telefone"];
...
$control_nm_telefone["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_nm_telefone["params"]["mode"]="inline_edit";
else
	$control_nm_telefone["params"]["mode"]="edit";
if(!$detailKeys || !in_array("nm_telefone", $detailKeys))	
	$xt->assign("nm_telefone_editcontrol",$control_nm_telefone);
else if(array_key_exists("nm_telefone",$data))
{
				$value = ProcessLargeText(GetData($data,"nm_telefone", ""),"field=nm%5Ftelefone","",MODE_VIEW);
...
$control_arq_fax["params"]["value"]=@$data["arq_fax"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_arq_fax["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_arq_fax["params"]["mode"]="inline_edit";
else
	$control_arq_fax["params"]["mode"]="edit";
if(!$detailKeys || !in_array("arq_fax", $detailKeys))	
	$xt->assign("arq_fax_editcontrol",$control_arq_fax);
else if(array_key_exists("arq_fax",$data))
{
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_send_fax_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
