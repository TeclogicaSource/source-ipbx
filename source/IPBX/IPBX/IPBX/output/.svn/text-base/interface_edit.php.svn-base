...
include("include/interface_variables.php");
...
$templatefile = ( $inlineedit ) ? "interface_inline_edit.htm" : "interface_edit.htm";
...
//	processibng dsc_interf - end
	...
	if($condition)
	{
	$value = postvalue("value_placa_".$id);
	$type=postvalue("type_placa_".$id);
	if(FieldSubmitted("placa_".$id))
	{
		
		$value=prepare_for_db("placa",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "placa"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["placa"]=$value;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: interface_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_interface->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: interface_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="interface_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('interface_edit.php','".$skeys."','',function(){window.location.href='interface_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('interface_edit.php','".$skeys."','',function(){window.location.href='interface_list.php?a=return'});return false;\"");
...
//	placa - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"placa", ""),"field=placa".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "placa";
				$showRawValues[] = substr($data["placa"],0,100);
...
//	control - placa
$control_placa=array();
$control_placa["func"]="xt_buildeditcontrol";
$control_placa["params"] = array();
$control_placa["params"]["field"]="placa";
$control_placa["params"]["value"]=@$data["placa"];
//	Begin Add validation
$arrValidate = array();	


$control_placa["params"]["validate"]=$arrValidate;
//	End Add validation
$control_placa["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_placa["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_placa["params"]["mode"]="inline_edit";
else
	$control_placa["params"]["mode"]="edit";
if(!$detailKeys || !in_array("placa", $detailKeys))	
	$xt->assign("placa_editcontrol",$control_placa);
else if(array_key_exists("placa",$data))
{
				$value = ProcessLargeText(GetData($data,"placa", ""),"field=placa","",MODE_VIEW);
		$xt->assign("placa_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('interface_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
