...
include("include/sigame_variables.php");
...
$templatefile = ( $inlineedit ) ? "sigame_inline_edit.htm" : "sigame_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["idt_siga"]=postvalue("editid1");
$savedKeys["idt_siga"]=postvalue("editid1");
...
//	processibng Ramal - end
	...
	if(0 && "Destino"=="" && $url_page=="admin_users_")
			$value=md5($value);
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idt_siga"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: sigame_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: sigame_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["idt_siga"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"idt_siga", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="sigame_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("idt_siga"))
		$key[]=GetFieldIndex("idt_siga");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('sigame_edit.php','".$skeys."','',function(){window.location.href='sigame_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('sigame_edit.php','".$skeys."','',function(){window.location.href='sigame_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["idt_siga"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["idt_siga"]));


//	idt_siga - 

		$value="";
				$value = ProcessLargeText(GetData($data,"idt_siga", ""),"field=idt%5Fsiga".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "idt_siga";
				$showRawValues[] = substr($data["idt_siga"],0,100);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('sigame_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
