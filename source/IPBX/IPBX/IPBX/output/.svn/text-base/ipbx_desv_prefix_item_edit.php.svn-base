...
$templatefile = ( $inlineedit ) ? "ipbx_desv_prefix_item_inline_edit.htm" : "ipbx_desv_prefix_item_edit.htm";
...
$savedKeys["id_desv_pref_item"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_id_desv_prefix_".$id);
	$type=postvalue("type_id_desv_prefix_".$id);
	if(FieldSubmitted("id_desv_prefix_".$id))
	{
		
		$value=prepare_for_db("id_desv_prefix",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "id_desv_prefix"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["id_desv_prefix"]=$value;
		
		
	}


//	processibng id_desv_prefix - end
	}
	$condition = $inlineedit;

	if($condition)
	{
	$value = postvalue("value_prefixo_".$id);
	$type=postvalue("type_prefixo_".$id);
	if(FieldSubmitted("prefixo_".$id))
	{
		
		$value=prepare_for_db("prefixo",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "prefixo"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["prefixo"]=$value;
		
		
	}


//	processibng prefixo - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_desv_pref_item"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_desv_prefix_item_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_desv_prefix_item->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_desv_prefix_item_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["id_desv_prefix"]=$evalues["id_desv_prefix"];
	$data["prefixo"]=$evalues["prefixo"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_desv_pref_item"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_desv_pref_item", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_desv_prefix_item_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_desv_prefix_item_edit.php','".$skeys."','',function(){window.location.href='ipbx_desv_prefix_item_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_desv_prefix_item_edit.php','".$skeys."','',function(){window.location.href='ipbx_desv_prefix_item_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_desv_pref_item"]);
if($message)
{
...
//	id_desv_prefix - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"id_desv_prefix", ""),"field=id%5Fdesv%5Fprefix".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "id_desv_prefix";
				$showRawValues[] = substr($data["id_desv_prefix"],0,100);

//	prefixo - 

		$value="";
				$value = ProcessLargeText(GetData($data,"prefixo", ""),"field=prefixo".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "prefixo";
				$showRawValues[] = substr($data["prefixo"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$regexmessage='';
$regextype = '';
//	control - id_desv_prefix
$control_id_desv_prefix=array();
$control_id_desv_prefix["func"]="xt_buildeditcontrol";
$control_id_desv_prefix["params"] = array();
$control_id_desv_prefix["params"]["field"]="id_desv_prefix";
$control_id_desv_prefix["params"]["value"]=@$data["id_desv_prefix"];
...
$control_id_desv_prefix["params"]["validate"]=$arrValidate;
//	End Add validation
$control_id_desv_prefix["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_id_desv_prefix["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_id_desv_prefix["params"]["mode"]="inline_edit";
else
	$control_id_desv_prefix["params"]["mode"]="edit";
if(!$detailKeys || !in_array("id_desv_prefix", $detailKeys))	
	$xt->assign("id_desv_prefix_editcontrol",$control_id_desv_prefix);
else if(array_key_exists("id_desv_prefix",$data))
{
				$value = ProcessLargeText(GetData($data,"id_desv_prefix", ""),"field=id%5Fdesv%5Fprefix","",MODE_VIEW);
		$xt->assign("id_desv_prefix_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - prefixo
$control_prefixo=array();
$control_prefixo["func"]="xt_buildeditcontrol";
$control_prefixo["params"] = array();
$control_prefixo["params"]["field"]="prefixo";
$control_prefixo["params"]["value"]=@$data["prefixo"];
//	Begin Add validation
$arrValidate = array();	

$control_prefixo["params"]["validate"]=$arrValidate;
//	End Add validation
$control_prefixo["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_prefixo["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_prefixo["params"]["mode"]="inline_edit";
else
	$control_prefixo["params"]["mode"]="edit";
if(!$detailKeys || !in_array("prefixo", $detailKeys))	
	$xt->assign("prefixo_editcontrol",$control_prefixo);
else if(array_key_exists("prefixo",$data))
{
				$value = ProcessLargeText(GetData($data,"prefixo", ""),"field=prefixo","",MODE_VIEW);
		$xt->assign("prefixo_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_desv_prefix_item_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
