...
$templatefile = "cc_receptivo_filas_horario_desv_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: cc_receptivo_filas_horario_desv_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_cc_receptivo_filas_horario_desv->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: cc_receptivo_filas_horario_desv_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_desvio"]=$evalues["dsc_desvio"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="cc_receptivo_filas_horario_desv_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('cc_receptivo_filas_horario_desv_edit.php','".$skeys."','',function(){window.location.href='cc_receptivo_filas_horario_desv_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('cc_receptivo_filas_horario_desv_edit.php','".$skeys."','',function(){window.location.href='cc_receptivo_filas_horario_desv_list.php?a=return'});return false;\"");
...
$control_dsc_desvio["params"]["value"]=@$data["dsc_desvio"];
...
$control_dsc_desvio["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_desvio["params"]["mode"]="inline_edit";
else
	$control_dsc_desvio["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_desvio", $detailKeys))	
	$xt->assign("dsc_desvio_editcontrol",$control_dsc_desvio);
else if(array_key_exists("dsc_desvio",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_desvio", ""),"field=dsc%5Fdesvio","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('cc_receptivo_filas_horario_desv_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
