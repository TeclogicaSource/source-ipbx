...
include("include/Graf__Agentes_variables.php");
...
$mode=SEARCH_SIMPLE;
$templatefile = "Graf__Agentes_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "Graf__Agentes_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'Graf__Agentes';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "Graf. Agentes";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("Filas");
	...
	if(isEnableSection508())
		$xt->assign_section("Filas_label","<label for=\"".GetInputElementId("Filas", $id)."\">","</label>");
	else 
		$xt->assign("Filas_label", true);
	
	$xt->assign("Filas_fieldblock", true);		
	$xt->assignbyref("Filas_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Filas_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Filas_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Filas", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("Filas", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("Filas")."', ".$pageObject->id.", ".$ctrlsMap.");";
	...
	{
	$contents_block["begin"].="action=\"Graf__Agentes_chart.php\" ";
	}
