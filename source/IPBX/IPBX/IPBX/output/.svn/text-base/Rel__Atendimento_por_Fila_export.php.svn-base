...
$pageObject->body["begin"] .="<form action=\"Rel__Atendimento_por_Fila_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel__Atendimento_por_Fila_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Atendimento_por_Fila.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Atendimento_por_Fila.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Atendimento_por_Fila.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["hr_atendimento"] = GetData($row,"hr_atendimento","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Atendimento_por_Fila.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="Time";
			$values["hr_atendimento"] = htmlspecialchars(GetData($row,"hr_atendimento",$format));
			$format="Time";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["hr_atendimento"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["hr_atendimento"] = GetData($row,"hr_atendimento",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format="Time";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["hr_atendimento"]);
					else
						echo htmlspecialchars($values["hr_atendimento"]);
			echo '</td>';
							echo '<td>';
