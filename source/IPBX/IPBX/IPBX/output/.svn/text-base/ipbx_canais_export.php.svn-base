...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_canal"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"ipbx_canais_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("ipbx_canais_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_canais.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_canais.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_canais.xml");
	...
	{
		$values = array();
			$values["id_canal"] = GetData($row,"id_canal","");
			$values["canal"] = GetData($row,"canal","");
			$values["flg_utilizado"] = GetData($row,"flg_utilizado","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_canais.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["id_canal"] = htmlspecialchars(GetData($row,"id_canal",$format));
			$format="";
			$values["canal"] = htmlspecialchars(GetData($row,"canal",$format));
			$format="";
			$values["flg_utilizado"] = htmlspecialchars(GetData($row,"flg_utilizado",$format));
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_canal"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["canal"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["flg_utilizado"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Canal").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Canal").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Flg Utilizado").'</td>';	
	}
	else
	{
		echo "<td>"."Id Canal"."</td>";
		echo "<td>"."Canal"."</td>";
		echo "<td>"."Flg Utilizado"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["id_canal"] = GetData($row,"id_canal",$format);
					
							$format="";
			
			$values["canal"] = GetData($row,"canal",$format);
					
							$format="";
			
			$values["flg_utilizado"] = GetData($row,"flg_utilizado",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["id_canal"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["canal"]);
					else
						echo htmlspecialchars($values["canal"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="";
									echo htmlspecialchars($values["flg_utilizado"]);
			echo '</td>';
			echo "</tr>";
		}		
