...
$templatefile = "ipbx_grupo_captura_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_captura"]=postvalue("editid1");
$savedKeys["id_captura"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_captura"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_grupo_captura_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_grupo_captura->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_grupo_captura_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_grupo"]=$evalues["dsc_grupo"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_captura"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_captura", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_grupo_captura_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_captura"))
		$key[]=GetFieldIndex("id_captura");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_grupo_captura_edit.php','".$skeys."','',function(){window.location.href='ipbx_grupo_captura_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_grupo_captura_edit.php','".$skeys."','',function(){window.location.href='ipbx_grupo_captura_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_captura"]);
if($message)
{
...
$control_dsc_grupo["params"]["value"]=@$data["dsc_grupo"];
...
$control_dsc_grupo["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_grupo["params"]["mode"]="inline_edit";
else
	$control_dsc_grupo["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_grupo", $detailKeys))	
	$xt->assign("dsc_grupo_editcontrol",$control_dsc_grupo);
else if(array_key_exists("dsc_grupo",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_grupo", ""),"field=dsc%5Fgrupo","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_grupo_captura_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
