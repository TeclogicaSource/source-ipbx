...
include("include/ipbx_send_fax_variables.php");
...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_send_fax_inline_add.htm";
else
	$templatefile = "ipbx_send_fax_add.htm";
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_send_fax_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_send_fax_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_send_fax_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_send_fax"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_send_fax"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["nm_telefone"]=@$avalues["nm_telefone"];
}
//for basic files
$includes="";
...
if($inlineadd!=ADD_INLINE)
{
	...
	$xt->assign("arq_fax_fieldblock",true);
	$xt->assign("arq_fax_label",true);
	if(isEnableSection508())
		$xt->assign_section("arq_fax_label","<label for=\"".GetInputElementId("arq_fax", $id)."\">","</label>");
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_send_fax_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_send_fax_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_send_fax_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_send_fax"]);

	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_send_fax"]));
...
//	id_send_fax - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"id_send_fax", ""),"field=id%5Fsend%5Ffax".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "id_send_fax";
				$showRawValues[] = substr($data["id_send_fax"],0,100);
	}	
//	nm_telefone - 
	$display = false;
