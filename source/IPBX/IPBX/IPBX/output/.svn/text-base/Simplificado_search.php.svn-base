...
include("include/Simplificado_variables.php");
...
$mode=SEARCH_SIMPLE;
$templatefile = "Simplificado_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "Simplificado_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'Simplificado';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "Rel. Simplificado - Centro de custo";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("dsc_centro_cust");
	...
	{
		$firstFieldParams['fName'] = "dsc_centro_cust";
	...
	$xt->assignbyref("dsc_centro_cust_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("dsc_centro_cust_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("dsc_centro_cust_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_dsc_centro_cust", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("dsc_centro_cust", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("dsc_centro_cust")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("Mes");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("Mes_label","<label for=\"".GetInputElementId("Mes", $id)."\">","</label>");
	else 
		$xt->assign("Mes_label", true);
	
	$xt->assign("Mes_fieldblock", true);		
	$xt->assignbyref("Mes_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Mes_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Mes_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Mes", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("Mes", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("Mes")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("Ano");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("Ano_label","<label for=\"".GetInputElementId("Ano", $id)."\">","</label>");
	else 
		$xt->assign("Ano_label", true);
	
	$xt->assign("Ano_fieldblock", true);		
	$xt->assignbyref("Ano_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Ano_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Ano_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Ano", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("Ano", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("Ano")."', ".$pageObject->id.", ".$ctrlsMap.");";
	...
	{
	$contents_block["begin"].="action=\"Simplificado_report.php\" ";
	}
