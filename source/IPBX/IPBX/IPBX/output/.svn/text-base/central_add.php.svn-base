...
if($inlineadd==ADD_INLINE)
	$templatefile = "central_inline_add.htm";
else
	$templatefile = "central_add.htm";
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'central_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'central_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: central_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_central"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_central"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='central_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="central_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="central_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$masterquery="mastertable=central";
	$masterquery.="&masterkey1=".rawurlencode($data["id_central"]);
	$showDetailKeys["ipbx_interf_vono"]=$masterquery;
	$masterquery="mastertable=central";
	$masterquery.="&masterkey1=".rawurlencode($data["id_central"]);
	$showDetailKeys["ipbx_interf_fxs"]=$masterquery;
	$masterquery="mastertable=central";
	$masterquery.="&masterkey1=".rawurlencode($data["id_central"]);
	$showDetailKeys["ipbx_interf_e1"]=$masterquery;
	$masterquery="mastertable=central";
	$masterquery.="&masterkey1=".rawurlencode($data["id_central"]);
	$showDetailKeys["ipbx_interf_gsm"]=$masterquery;
	$masterquery="mastertable=central";
	$masterquery.="&masterkey1=".rawurlencode($data["id_central"]);
	$showDetailKeys["ipbx_interf_mslync"]=$masterquery;
	$masterquery="mastertable=central";
	$masterquery.="&masterkey1=".rawurlencode($data["id_central"]);
	$showDetailKeys["ipbx_interf_sip"]=$masterquery;
	$masterquery="mastertable=central";
	$masterquery.="&masterkey1=".rawurlencode($data["id_central"]);
	$showDetailKeys["ipbx_interf_voxip"]=$masterquery;
	$masterquery="mastertable=central";
	$masterquery.="&masterkey1=".rawurlencode($data["id_central"]);
	$showDetailKeys["ipbx_interf_callman"]=$masterquery;
	$masterquery="mastertable=central";
	$masterquery.="&masterkey1=".rawurlencode($data["id_central"]);
	$showDetailKeys["ipbx_interf_fxo"]=$masterquery;
	$masterquery="mastertable=central";
	$masterquery.="&masterkey1=".rawurlencode($data["id_central"]);
	$showDetailKeys["ipbx_interf_sip_generica"]=$masterquery;
