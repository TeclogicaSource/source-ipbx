...
$pageObject->body["begin"] .="<form action=\"horario_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("horario_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=horario.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=horario.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=horario.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["dsc_horario"] = GetData($row,"dsc_horario","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=horario.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="";
			$values["dsc_horario"] = htmlspecialchars(GetData($row,"dsc_horario",$format));
			$format="Time";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["dsc_horario"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Horario Inicio").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Horario Fim").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["dsc_horario"] = GetData($row,"dsc_horario",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["dsc_horario"]);
					else
						echo htmlspecialchars($values["dsc_horario"]);
			echo '</td>';
							echo '<td>';
