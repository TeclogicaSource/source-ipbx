...
if ( isset( $_POST['str_xml'] ))
{
	$arr = my_json_decode(postvalue('str_xml'));
		if($arr["table_type"]=="db")
		{
			$_SESSION["webobject"]["db-based"]=true;
			$_SESSION["webcharts"]["db-based"]=true;
			$_SESSION["webreports"]["db-based"]=true;
		}
		if($arr["table_type"]=="project")
		{
			$_SESSION["webobject"]["db-based"]=false;
			$_SESSION["webcharts"]["db-based"]=false;
			$_SESSION["webreports"]["db-based"]=false;
		}
}
...
if ( isset( $_POST['str_xml'] ) && isset( $_POST['web'] ) && !isset( $_POST['save'] ) )
{
	...
	{
        if(is_new_wrobject() && (array_key_exists("table_relations", $arr) || array_key_exists("group_by_condition", $arr)))
		{
	...
	{
        if(is_new_wrobject() && array_key_exists("table_relations", $arr))
		{
...
//	calc order by clause
				...
				for ($i=0; $i < count($root['group_by_condition'])-1; $i++) 
				{
					...
					WRSplitFieldName($arr['field_opt'],$table_name,$field_name);
					$fld_name = AddTableWrappers($table_name).".".AddFieldWrappers($field_name);
					$arr_order[(int)($arr["sort_order"])]=array("field"=>$fld_name,"dir"=>$arr["sort_dir"]);
				}
				if(count($arr_order))
				{
