...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["data"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["hora"]=refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$keys["acao"]=refine($_REQUEST["mdelete3"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<3)
				continue;
			$keys=array();
			$keys["data"]=urldecode($arr[0]);
			$keys["hora"]=urldecode($arr[1]);
			$keys["acao"]=urldecode($arr[2]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"Acesso_de_agentes_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Acesso_de_agentes_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Acesso_de_agentes.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Acesso_de_agentes.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Acesso_de_agentes.xml");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Acesso_de_agentes.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="";
			$values["acao"] = htmlspecialchars(GetData($row,"acao",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["acao"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Acao").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["acao"] = GetData($row,"acao",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["acao"]);
					else
						echo htmlspecialchars($values["acao"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
