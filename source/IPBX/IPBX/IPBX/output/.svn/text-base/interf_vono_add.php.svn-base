...
if($inlineadd==ADD_INLINE)
	$templatefile = "interf_vono_inline_add.htm";
else
	$templatefile = "interf_vono_add.htm";
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'interf_vono_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'interf_vono_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: interf_vono_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_interf_vono"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_interf_vono"]=postvalue("editid1");
	}
...
if($readavalues)
{
	...
	$defvalues["codec"]=@$avalues["codec"];
	$defvalues["usuario"]=@$avalues["usuario"];
	$defvalues["senha"]=@$avalues["senha"];
	$defvalues["ip_host"]=@$avalues["ip_host"];
}
//for basic files
$includes="";
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='interf_vono_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="interf_vono_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="interf_vono_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_interf_vono"]);
...
//	codec - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"codec", ""),"field=codec".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "codec";
...
{
				$value = ProcessLargeText(GetData($defvalues,"id_contrato", ""),"field=id%5Fcontrato","",MODE_VIEW);
...
{
				$value = ProcessLargeText(GetData($defvalues,"codec", ""),"field=codec","",MODE_VIEW);
