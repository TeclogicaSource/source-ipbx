...
include("include/plano_discagem_regiao_variables.php");
...
$templatefile = ( $inlineedit ) ? "plano_discagem_regiao_inline_edit.htm" : "plano_discagem_regiao_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_regiao"]=postvalue("editid1");
$savedKeys["id_regiao"]=postvalue("editid1");
...
{
	$ids = $id;
	$mKeys["planos_discagem"] = GetMasterKeysByDetailTable("planos_discagem", $strTableName);
	$dpParams['strTableNames'][] = "planos_discagem";
	$dpParams['ids'][] = ++$ids;
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_regiao_".$id);
	$type=postvalue("type_regiao_".$id);
	if(FieldSubmitted("regiao_".$id))
	{
		
		$value=prepare_for_db("regiao",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "regiao"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["regiao"]=$value;
		
		
	}


//	processibng regiao - end
	}
	$condition = 1;

	if($condition)
	{
	$value = postvalue("value_id_tronco_".$id);
	$type=postvalue("type_id_tronco_".$id);
	if(FieldSubmitted("id_tronco_".$id))
	{
		
		$value=prepare_for_db("id_tronco",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "id_tronco"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["id_tronco"]=$value;
		
		
	}


//	processibng id_tronco - end
	}
	$condition = 1;

	if($condition)
	{
	$value = postvalue("value_tp_destino_".$id);
	$type=postvalue("type_tp_destino_".$id);
	if(FieldSubmitted("tp_destino_".$id))
	{
		
		$value=prepare_for_db("tp_destino",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "tp_destino"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["tp_destino"]=$value;
		
		
	}


//	processibng tp_destino - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_regiao"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: plano_discagem_regiao_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_plano_discagem_regiao->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: plano_discagem_regiao_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["regiao"]=$evalues["regiao"];
	$data["id_tronco"]=$evalues["id_tronco"];
	$data["tp_destino"]=$evalues["tp_destino"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	if(isEnableSection508())
		$xt->assign_section("regiao_label","<label for=\"".GetInputElementId("regiao", $id)."\">","</label>");
	$xt->assign("id_tronco_fieldblock",true);
	$xt->assign("id_tronco_label",true);
	if(isEnableSection508())
		$xt->assign_section("id_tronco_label","<label for=\"".GetInputElementId("id_tronco", $id)."\">","</label>");
	$xt->assign("tp_destino_fieldblock",true);
	$xt->assign("tp_destino_label",true);
	if(isEnableSection508())
		$xt->assign_section("tp_destino_label","<label for=\"".GetInputElementId("tp_destino", $id)."\">","</label>");
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_regiao"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_regiao", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="plano_discagem_regiao_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_regiao"))
		$key[]=GetFieldIndex("id_regiao");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('plano_discagem_regiao_edit.php','".$skeys."','',function(){window.location.href='plano_discagem_regiao_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('plano_discagem_regiao_edit.php','".$skeys."','',function(){window.location.href='plano_discagem_regiao_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_regiao"]);
if($message)
{
...
//	detail tables
	$masterquery="mastertable=plano%5Fdiscagem%5Fregiao";
	$masterquery.="&masterkey1=".rawurlencode($data["id_regiao"]);
	$showDetailKeys["planos_discagem"]=$masterquery;
...
//	tp_destino - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"tp_destino", ""),"field=tp%5Fdestino".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "tp_destino";
				$showRawValues[] = substr($data["tp_destino"],0,100);

//	regiao - 

		$value="";
				$value = ProcessLargeText(GetData($data,"regiao", ""),"field=regiao".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "regiao";
				$showRawValues[] = substr($data["regiao"],0,100);

//	id_tronco - 

		$value="";
				$value=DisplayLookupWizard("id_tronco",$data["id_tronco"],$data,$keylink,MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "id_tronco";
				$showRawValues[] = substr($data["id_tronco"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$regextype = '';
//	control - regiao
$control_regiao=array();
$control_regiao["func"]="xt_buildeditcontrol";
$control_regiao["params"] = array();
$control_regiao["params"]["field"]="regiao";
$control_regiao["params"]["value"]=@$data["regiao"];
//	Begin Add validation
$arrValidate = array();	

$control_regiao["params"]["validate"]=$arrValidate;
//	End Add validation
$control_regiao["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_regiao["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_regiao["params"]["mode"]="inline_edit";
else
	$control_regiao["params"]["mode"]="edit";
if(!$detailKeys || !in_array("regiao", $detailKeys))	
	$xt->assign("regiao_editcontrol",$control_regiao);
else if(array_key_exists("regiao",$data))
{
				$value = ProcessLargeText(GetData($data,"regiao", ""),"field=regiao","",MODE_VIEW);
		$xt->assign("regiao_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - id_tronco
$control_id_tronco=array();
$control_id_tronco["func"]="xt_buildeditcontrol";
$control_id_tronco["params"] = array();
$control_id_tronco["params"]["field"]="id_tronco";
$control_id_tronco["params"]["value"]=@$data["id_tronco"];
//	Begin Add validation
$arrValidate = array();	


$control_id_tronco["params"]["validate"]=$arrValidate;
//	End Add validation
$control_id_tronco["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_id_tronco["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_id_tronco["params"]["mode"]="inline_edit";
else
	$control_id_tronco["params"]["mode"]="edit";
if(!$detailKeys || !in_array("id_tronco", $detailKeys))	
	$xt->assign("id_tronco_editcontrol",$control_id_tronco);
else if(array_key_exists("id_tronco",$data))
{
				$value=DisplayLookupWizard("id_tronco",$data["id_tronco"],$data,"",MODE_VIEW);
		$xt->assign("id_tronco_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - tp_destino
$control_tp_destino=array();
$control_tp_destino["func"]="xt_buildeditcontrol";
$control_tp_destino["params"] = array();
$control_tp_destino["params"]["field"]="tp_destino";
$control_tp_destino["params"]["value"]=@$data["tp_destino"];
//	Begin Add validation
$arrValidate = array();	

$control_tp_destino["params"]["validate"]=$arrValidate;
//	End Add validation
$control_tp_destino["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_tp_destino["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_tp_destino["params"]["mode"]="inline_edit";
else
	$control_tp_destino["params"]["mode"]="edit";
if(!$detailKeys || !in_array("tp_destino", $detailKeys))	
	$xt->assign("tp_destino_editcontrol",$control_tp_destino);
else if(array_key_exists("tp_destino",$data))
{
				$value = ProcessLargeText(GetData($data,"tp_destino", ""),"field=tp%5Fdestino","",MODE_VIEW);
		$xt->assign("tp_destino_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('plano_discagem_regiao_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
