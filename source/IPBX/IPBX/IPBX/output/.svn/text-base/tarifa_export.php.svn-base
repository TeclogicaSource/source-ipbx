...
$pageObject->body["begin"] .="<form action=\"tarifa_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("tarifa_export.htm");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=tarifa.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=tarifa.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["vlr_unid"] = GetData($row,"vlr_unid","");
			$values["cad_inicial"] = GetData($row,"cad_inicial","");
			$values["cad_frac"] = GetData($row,"cad_frac","");
			...
			$values["IDT_TARIFA"] = GetData($row,"IDT_TARIFA","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=tarifa.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="Currency";
			$values["vlr_unid"] = htmlspecialchars(GetData($row,"vlr_unid",$format));
			$format="Number";
			$values["cad_inicial"] = htmlspecialchars($row["cad_inicial"]);
			$format="Number";
			$values["cad_frac"] = htmlspecialchars($row["cad_frac"]);
			$format=FORMAT_NONE;
			...
			$format="";
			$values["IDT_TARIFA"] = htmlspecialchars(GetData($row,"IDT_TARIFA",$format));
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["vlr_unid"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["cad_inicial"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["cad_frac"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["IDT_TARIFA"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Valor Unitário").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Cadencia Inicial").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Cadencia Fracionaria").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Segunda Feira").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Terça Feira").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("IDT TARIFA").'</td>';	
	}
	else
	{
		...
		echo "<td>"."Cadencia Inicial"."</td>";
		echo "<td>"."Cadencia Fracionaria"."</td>";
		echo "<td>"."Segunda Feira"."</td>";
		echo "<td>"."Terça Feira"."</td>";
		echo "<td>"."Quarta Feira"."</td>";
		echo "<td>"."Quinta Feira"."</td>";
		echo "<td>"."Sexta Feira"."</td>";
		echo "<td>"."Sabado"."</td>";
		echo "<td>"."Domingo"."</td>";
		echo "<td>"."IDT TARIFA"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["vlr_unid"] = GetData($row,"vlr_unid",$format);
					
							$format="Number";
			
			$values["cad_inicial"] = GetData($row,"cad_inicial",$format);
					
							$format="Number";
			
			$values["cad_frac"] = GetData($row,"cad_frac",$format);
			...
			$values["IDT_TARIFA"] = GetData($row,"IDT_TARIFA",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format="Currency";
									echo htmlspecialchars($values["vlr_unid"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="Number";
									echo htmlspecialchars($values["cad_inicial"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="Number";
									echo htmlspecialchars($values["cad_frac"]);
			echo '</td>';
							echo '<td>';
			...
			echo '</td>';
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["IDT_TARIFA"]);
			echo '</td>';
			echo "</tr>";
		}		
