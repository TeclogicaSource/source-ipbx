...
include("include/Recebidas_variables.php");
...
$pageObject->body["begin"] .="<form action=\"Recebidas_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Recebidas_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Recebidas.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Recebidas.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Recebidas.xml");
	...
	{
		$values = array();
			$values["Período"] = GetData($row,"Período","");
			$values["calldate"] = GetData($row,"calldate","");
			$values["clid"] = GetData($row,"clid","");
			$values["src"] = GetData($row,"src","");
			$values["dst"] = GetData($row,"dst","");
			$values["duration"] = GetData($row,"duration","");
			$values["billsec"] = GetData($row,"billsec","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Recebidas.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="Short Date";
			$values["Período"] = htmlspecialchars(GetData($row,"Período",$format));
			$format="Short Date";
			$values["calldate"] = htmlspecialchars(GetData($row,"calldate",$format));
			$format="";
			$values["clid"] = htmlspecialchars(GetData($row,"clid",$format));
			$format="";
			$values["src"] = htmlspecialchars(GetData($row,"src",$format));
			$format="";
			$values["dst"] = htmlspecialchars(GetData($row,"dst",$format));
			$format="Time";
			$values["duration"] = htmlspecialchars(GetData($row,"duration",$format));
			$format="Time";
			$values["billsec"] = htmlspecialchars(GetData($row,"billsec",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Período"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["calldate"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["clid"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["duration"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["billsec"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Período").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Data e Hora").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Usuário").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Origem").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Destino").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Duração").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Segundos").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Centro de custo").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Ultimo Estado").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["Período"] = GetData($row,"Período",$format);
			...
			$values["clid"] = GetData($row,"clid",$format);
			...
			$values["duration"] = GetData($row,"duration",$format);
					
							$format="Time";
			
			$values["billsec"] = GetData($row,"billsec",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="Short Date";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["Período"]);
					else
						echo htmlspecialchars($values["Período"]);
			echo '</td>';
							echo '<td>';
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["clid"]);
					else
						echo htmlspecialchars($values["clid"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
			...
			echo '</td>';
							echo '<td>';
									...
									$format="Time";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["duration"]);
					else
						echo htmlspecialchars($values["duration"]);
			echo '</td>';
							echo '<td>';
									...
									$format="Time";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["billsec"]);
					else
						echo htmlspecialchars($values["billsec"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
