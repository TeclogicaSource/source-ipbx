...
include("include/ipbx_interf_e1_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_interf_e1_inline_edit.htm" : "ipbx_interf_e1_edit.htm";
...
//	processibng board - end
	...
	if($condition)
	{
	$value = postvalue("value_link_".$id);
	$type=postvalue("type_link_".$id);
	if(FieldSubmitted("link_".$id))
	{
	...
	if(0 && "link"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["link"]=$value;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ipbx_interf_e1_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_interf_e1->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_interf_e1_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	...
	$data["board"]=$evalues["board"];
	$data["link"]=$evalues["link"];
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_e1_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_interf_e1_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_e1_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_interf_e1_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_e1_list.php?a=return'});return false;\"");
...
//	link - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"link", ""),"field=link".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "link";
				$showRawValues[] = substr($data["link"],0,100);
...
$control_board["params"]["value"]=@$data["board"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_board["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_board["params"]["mode"]="inline_edit";
else
	$control_board["params"]["mode"]="edit";
if(!$detailKeys || !in_array("board", $detailKeys))	
	$xt->assign("board_editcontrol",$control_board);
else if(array_key_exists("board",$data))
{
				$value = ProcessLargeText(GetData($data,"board", ""),"field=board","",MODE_VIEW);
...
//	control - link
$control_link=array();
$control_link["func"]="xt_buildeditcontrol";
$control_link["params"] = array();
$control_link["params"]["field"]="link";
$control_link["params"]["value"]=@$data["link"];
//	Begin Add validation
$arrValidate = array();	

$arrValidate['basicValidate'][] = "IsRequired";

$control_link["params"]["validate"]=$arrValidate;
//	End Add validation
$control_link["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_link["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_link["params"]["mode"]="inline_edit";
else
	$control_link["params"]["mode"]="edit";
if(!$detailKeys || !in_array("link", $detailKeys))	
	$xt->assign("link_editcontrol",$control_link);
else if(array_key_exists("link",$data))
{
				$value = ProcessLargeText(GetData($data,"link", ""),"field=link","",MODE_VIEW);
		$xt->assign("link_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_interf_e1_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
