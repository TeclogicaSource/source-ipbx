...
$templatefile = "ipbx_sincronismo_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_sinc"]=postvalue("editid1");
$savedKeys["id_sinc"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_sinc"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_sincronismo_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_sincronismo->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_sincronismo_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
$readonlyfields=array();
  ...
  $readonlyfields["ult_sincronismo"] = htmlspecialchars(GetData($data,"ult_sincronismo", "Datetime"));

if($readevalues)
{
	$data["usuario"]=$evalues["usuario"];
	$data["senha"]=$evalues["senha"];
	$data["trafego"]=$evalues["trafego"];
	$data["ip_server"]=$evalues["ip_server"];
	$data["flg_ativo"]=$evalues["flg_ativo"];
	$data["flg_fax"]=$evalues["flg_fax"];
	$data["flg_gravacoes"]=$evalues["flg_gravacoes"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_sinc"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_sinc", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_sincronismo_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_sinc"))
		$key[]=GetFieldIndex("id_sinc");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_sincronismo_edit.php','".$skeys."','',function(){window.location.href='ipbx_sincronismo_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_sincronismo_edit.php','".$skeys."','',function(){window.location.href='ipbx_sincronismo_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_sinc"]);
if($message)
{
...
$control_trafego["params"]["value"]=@$data["trafego"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_trafego["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_trafego["params"]["mode"]="inline_edit";
else
	$control_trafego["params"]["mode"]="edit";
if(!$detailKeys || !in_array("trafego", $detailKeys))	
	$xt->assign("trafego_editcontrol",$control_trafego);
else if(array_key_exists("trafego",$data))
{
				$value = ProcessLargeText(GetData($data,"trafego", ""),"field=trafego","",MODE_VIEW);
...
$control_flg_gravacoes["params"]["value"]=@$data["flg_gravacoes"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_flg_gravacoes["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_flg_gravacoes["params"]["mode"]="inline_edit";
else
	$control_flg_gravacoes["params"]["mode"]="edit";
if(!$detailKeys || !in_array("flg_gravacoes", $detailKeys))	
	$xt->assign("flg_gravacoes_editcontrol",$control_flg_gravacoes);
else if(array_key_exists("flg_gravacoes",$data))
{
...
$control_ult_sincronismo["params"]["value"]=@$data["ult_sincronismo"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_ult_sincronismo["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_ult_sincronismo["params"]["mode"]="inline_edit";
else
	$control_ult_sincronismo["params"]["mode"]="edit";
if(!$detailKeys || !in_array("ult_sincronismo", $detailKeys))	
	$xt->assign("ult_sincronismo_editcontrol",$control_ult_sincronismo);
else if(array_key_exists("ult_sincronismo",$data))
{
				$value = ProcessLargeText(GetData($data,"ult_sincronismo", "Datetime"),"field=ult%5Fsincronismo","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_sincronismo_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
