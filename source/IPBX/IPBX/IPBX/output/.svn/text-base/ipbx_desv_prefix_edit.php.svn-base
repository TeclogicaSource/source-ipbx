...
include("include/ipbx_desv_prefix_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_desv_prefix_inline_edit.htm" : "ipbx_desv_prefix_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_desv_prefix"]=postvalue("editid1");
$savedKeys["id_desv_prefix"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_dsc_grp_pref_".$id);
	$type=postvalue("type_dsc_grp_pref_".$id);
	if(FieldSubmitted("dsc_grp_pref_".$id))
	{
		
		$value=prepare_for_db("dsc_grp_pref",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "dsc_grp_pref"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["dsc_grp_pref"]=$value;
		
		
	}


//	processibng dsc_grp_pref - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_desv_prefix"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_desv_prefix_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_desv_prefix->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_desv_prefix_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_grp_pref"]=$evalues["dsc_grp_pref"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_desv_prefix"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_desv_prefix", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_desv_prefix_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_desv_prefix"))
		$key[]=GetFieldIndex("id_desv_prefix");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_desv_prefix_edit.php','".$skeys."','',function(){window.location.href='ipbx_desv_prefix_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_desv_prefix_edit.php','".$skeys."','',function(){window.location.href='ipbx_desv_prefix_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_desv_prefix"]);
if($message)
{
...
//	detail tables
	$masterquery="mastertable=ipbx%5Fdesv%5Fprefix";
	$masterquery.="&masterkey1=".rawurlencode($data["id_desv_prefix"]);
	$showDetailKeys["ipbx_desv_prefix_item"]=$masterquery;
...
//	dsc_grp_pref - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"dsc_grp_pref", ""),"field=dsc%5Fgrp%5Fpref".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dsc_grp_pref";
				$showRawValues[] = substr($data["dsc_grp_pref"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$regexmessage='';
$regextype = '';
//	control - dsc_grp_pref
$control_dsc_grp_pref=array();
$control_dsc_grp_pref["func"]="xt_buildeditcontrol";
$control_dsc_grp_pref["params"] = array();
$control_dsc_grp_pref["params"]["field"]="dsc_grp_pref";
$control_dsc_grp_pref["params"]["value"]=@$data["dsc_grp_pref"];
//	Begin Add validation
$arrValidate = array();	

$control_dsc_grp_pref["params"]["validate"]=$arrValidate;
//	End Add validation
$control_dsc_grp_pref["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_dsc_grp_pref["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_grp_pref["params"]["mode"]="inline_edit";
else
	$control_dsc_grp_pref["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_grp_pref", $detailKeys))	
	$xt->assign("dsc_grp_pref_editcontrol",$control_dsc_grp_pref);
else if(array_key_exists("dsc_grp_pref",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_grp_pref", ""),"field=dsc%5Fgrp%5Fpref","",MODE_VIEW);
		$xt->assign("dsc_grp_pref_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_desv_prefix_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
