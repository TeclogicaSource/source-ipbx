...
include("include/sms_grupo_variables.php");
...
$templatefile = ( $inlineedit ) ? "sms_grupo_inline_edit.htm" : "sms_grupo_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_grupo"]=postvalue("editid1");
$savedKeys["id_grupo"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_grupo"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: sms_grupo_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_sms_grupo->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: sms_grupo_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_grupo"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_grupo", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="sms_grupo_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_grupo"))
		$key[]=GetFieldIndex("id_grupo");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('sms_grupo_edit.php','".$skeys."','',function(){window.location.href='sms_grupo_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('sms_grupo_edit.php','".$skeys."','',function(){window.location.href='sms_grupo_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_grupo"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_grupo"]));
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('sms_grupo_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
