...
include("include/ipbx_grupo_captura_variables.php");
...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_grupo_captura_inline_add.htm";
else
	$templatefile = "ipbx_grupo_captura_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_grupo_captura', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
if(@$_POST["a"]=="added")
{
	...
	$files_move=array();
	$files_save=array();
//	processing dsc_grupo - start
    
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
	$value = postvalue("value_dsc_grupo_".$id);
	$type=postvalue("type_dsc_grupo_".$id);
	if (FieldSubmitted("dsc_grupo_".$id))
	{
		$value=prepare_for_db("dsc_grupo",$value,$type);
	}
	else
		$value=false;
	if(!($value===false))
	{
		...
		if(0 && "dsc_grupo"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["dsc_grupo"]=$value;
	}
	}
//	processibng dsc_grupo - end
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_grupo_captura_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_grupo_captura_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_grupo_captura_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_captura"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_captura"]=postvalue("editid1");
	}
...
if($readavalues)
{
	$defvalues["dsc_grupo"]=@$avalues["dsc_grupo"];
}
//for basic files
$includes="";
...
if($inlineadd!=ADD_INLINE)
{
	...
	$xt->assign("dsc_grupo_fieldblock",true);
	$xt->assign("dsc_grupo_label",true);
	if(isEnableSection508())
		$xt->assign_section("dsc_grupo_label","<label for=\"".GetInputElementId("dsc_grupo", $id)."\">","</label>");
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_grupo_captura_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_grupo_captura_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_grupo_captura_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
{
	$LookupSQL="";
	if(postvalue("table")=="ipbx_ramais" && postvalue("field")=="pickupgroup") 
	{
	$linkfield = "id_captura";
	$dispfield = "dsc_grupo";
	$LookupSQL = "select ";
	$LookupSQL .= "`id_captura`";
		$LookupSQL .= ",`dsc_grupo`";
	}
	if(postvalue("table")=="personal_info" && postvalue("field")=="pickupgroup") 
	{
	$linkfield = "id_captura";
	$dispfield = "dsc_grupo";
	$LookupSQL = "select ";
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_captura"]);

	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_captura"]));
...
//	id_captura - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"id_captura", ""),"field=id%5Fcaptura".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "id_captura";
				$showRawValues[] = substr($data["id_captura"],0,100);
	}	
//	dsc_grupo - 
	$display = false;
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"dsc_grupo", ""),"field=dsc%5Fgrupo".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dsc_grupo";
				$showRawValues[] = substr($data["dsc_grupo"],0,100);
	}	
...
$regextype = '';
//	control - dsc_grupo
$control_dsc_grupo=array();
$control_dsc_grupo["func"]="xt_buildeditcontrol";
$control_dsc_grupo["params"] = array();
$control_dsc_grupo["params"]["field"]="dsc_grupo";
$control_dsc_grupo["params"]["value"]=@$defvalues["dsc_grupo"];

//	Begin Add validation
$arrValidate = array();	
$control_dsc_grupo["params"]["validate"]=$arrValidate;
//	End Add validation

$control_dsc_grupo["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dsc_grupo["params"]["mode"]="inline_add";
else
	$control_dsc_grupo["params"]["mode"]="add";

if(!$detailKeys || !in_array("dsc_grupo", $detailKeys))
	$xt->assignbyref("dsc_grupo_editcontrol",$control_dsc_grupo);
else if(array_key_exists("dsc_grupo", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dsc_grupo", ""),"field=dsc%5Fgrupo","",MODE_VIEW);
		$xt->assign("dsc_grupo_editcontrol",$value);
}
