...
include("include/Graf__Perfil_Tempo_de_espera_variables.php");
...
$mode=SEARCH_SIMPLE;
$templatefile = "Graf__Perfil_Tempo_de_espera_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "Graf__Perfil_Tempo_de_espera_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'Graf__Perfil_Tempo_de_espera';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "Graf. Perfil Tempo de espera";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("Periodo");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "Periodo";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("Periodo_label","<label for=\"".GetInputElementId("Periodo", $id)."\">","</label>");
	else 
		$xt->assign("Periodo_label", true);
	
	$xt->assign("Periodo_fieldblock", true);		
	$xt->assignbyref("Periodo_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("Periodo_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("Periodo_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_Periodo", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("Periodo", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("Periodo")."', ".$pageObject->id.", ".$ctrlsMap.");";
	...
	{
	$contents_block["begin"].="action=\"Graf__Perfil_Tempo_de_espera_chart.php\" ";
	}
