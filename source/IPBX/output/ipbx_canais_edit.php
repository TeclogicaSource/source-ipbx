...
$templatefile = ( $inlineedit ) ? "ipbx_canais_inline_edit.htm" : "ipbx_canais_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_canal"]=postvalue("editid1");
$savedKeys["id_canal"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_canal"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_canais_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_canais->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_canais_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_canal"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_canal", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_canais_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_canal"))
		$key[]=GetFieldIndex("id_canal");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_canais_edit.php','".$skeys."','',function(){window.location.href='ipbx_canais_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_canais_edit.php','".$skeys."','',function(){window.location.href='ipbx_canais_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_canal"]);
if($message)
{
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_canais_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
