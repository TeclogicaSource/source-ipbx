...
$templatefile = "ipbx_provisionamento_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_provis"]=postvalue("editid1");
$savedKeys["id_provis"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_provis"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_provisionamento_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_provisionamento->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_provisionamento_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_aparelho"]=$evalues["dsc_aparelho"];
	$data["nome_arq"]=$evalues["nome_arq"];
	$data["ipservidor"]=$evalues["ipservidor"];
	$data["ipntp"]=$evalues["ipntp"];
	$data["txt_provis"]=$evalues["txt_provis"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_provis"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_provis", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_provisionamento_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_provis"))
		$key[]=GetFieldIndex("id_provis");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_provisionamento_edit.php','".$skeys."','',function(){window.location.href='ipbx_provisionamento_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_provisionamento_edit.php','".$skeys."','',function(){window.location.href='ipbx_provisionamento_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_provis"]);
if($message)
{
...
$control_dsc_aparelho["params"]["value"]=@$data["dsc_aparelho"];
...
$control_dsc_aparelho["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_aparelho["params"]["mode"]="inline_edit";
else
	$control_dsc_aparelho["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_aparelho", $detailKeys))	
	$xt->assign("dsc_aparelho_editcontrol",$control_dsc_aparelho);
else if(array_key_exists("dsc_aparelho",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_aparelho", ""),"field=dsc%5Faparelho","",MODE_VIEW);
...
$control_nome_arq["params"]["value"]=@$data["nome_arq"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_nome_arq["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_nome_arq["params"]["mode"]="inline_edit";
else
	$control_nome_arq["params"]["mode"]="edit";
if(!$detailKeys || !in_array("nome_arq", $detailKeys))	
	$xt->assign("nome_arq_editcontrol",$control_nome_arq);
else if(array_key_exists("nome_arq",$data))
{
				$value = ProcessLargeText(GetData($data,"nome_arq", ""),"field=nome%5Farq","",MODE_VIEW);
...
$control_ipservidor["params"]["value"]=@$data["ipservidor"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_ipservidor["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_ipservidor["params"]["mode"]="inline_edit";
else
	$control_ipservidor["params"]["mode"]="edit";
if(!$detailKeys || !in_array("ipservidor", $detailKeys))	
	$xt->assign("ipservidor_editcontrol",$control_ipservidor);
else if(array_key_exists("ipservidor",$data))
{
				$value = ProcessLargeText(GetData($data,"ipservidor", ""),"field=ipservidor","",MODE_VIEW);
...
$control_ipntp["params"]["value"]=@$data["ipntp"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_ipntp["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_ipntp["params"]["mode"]="inline_edit";
else
	$control_ipntp["params"]["mode"]="edit";
if(!$detailKeys || !in_array("ipntp", $detailKeys))	
	$xt->assign("ipntp_editcontrol",$control_ipntp);
else if(array_key_exists("ipntp",$data))
{
				$value = ProcessLargeText(GetData($data,"ipntp", ""),"field=ipntp","",MODE_VIEW);
...
$control_txt_provis["params"]["value"]=@$data["txt_provis"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_txt_provis["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_txt_provis["params"]["mode"]="inline_edit";
else
	$control_txt_provis["params"]["mode"]="edit";
if(!$detailKeys || !in_array("txt_provis", $detailKeys))	
	$xt->assign("txt_provis_editcontrol",$control_txt_provis);
else if(array_key_exists("txt_provis",$data))
{
				$value = ProcessLargeText(GetData($data,"txt_provis", ""),"field=txt%5Fprovis","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_provisionamento_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
