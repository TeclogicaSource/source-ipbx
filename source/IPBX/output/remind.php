...
if (@$_POST["btnSubmit"] == "Remind")
{
	...
	{	
		...
		if($tosearch)
		{
			$strSQL="select ".AddFieldWrappers($cUserNameField).",".AddFieldWrappers($cPasswordField).",".AddFieldWrappers($cEmailField)." from `sip_users` where ".$sWhere;
			...
			if($data=db_fetch_numarray($rs))
			{
				...
				$message.="Você pediu para lembrar seu nome de usuário e sua senha em"." ".$url."\r\n";
				$message.="Nome do Usuário".": ".$data[0]."\r\n";
				$message.="Senha".": ".$password."\r\n";
				runner_mail(array('to' => $data[2], 'subject' => "Lembrete de Senha", 'body' => $message));
				$reminded=true;
		...
		{
			if($strSearchBy!="email")
				$strMessage="Usuário"." <i>".$strUsername."</i> "."não é registrado";
			else
				$strMessage="Este email não existe em nosso cadastro";
		}
