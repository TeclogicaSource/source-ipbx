...
include("include/sms_celulares_variables.php");
...
$templatefile = ( $inlineedit ) ? "sms_celulares_inline_edit.htm" : "sms_celulares_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_celular"]=postvalue("editid1");
$savedKeys["id_celular"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_dsc_nome_".$id);
	$type=postvalue("type_dsc_nome_".$id);
	if(FieldSubmitted("dsc_nome_".$id))
	{
		
		$value=prepare_for_db("dsc_nome",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "dsc_nome"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["dsc_nome"]=$value;
		
		
	}


//	processibng dsc_nome - end
	}
	$condition = $inlineedit;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_celular"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: sms_celulares_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_sms_celulares->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: sms_celulares_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_nome"]=$evalues["dsc_nome"];
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_celular"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_celular", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="sms_celulares_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_celular"))
		$key[]=GetFieldIndex("id_celular");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('sms_celulares_edit.php','".$skeys."','',function(){window.location.href='sms_celulares_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('sms_celulares_edit.php','".$skeys."','',function(){window.location.href='sms_celulares_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_celular"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_celular"]));
...
//	dsc_nome - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"dsc_nome", ""),"field=dsc%5Fnome".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dsc_nome";
				$showRawValues[] = substr($data["dsc_nome"],0,100);
...
$regextype = '';
//	control - dsc_nome
$control_dsc_nome=array();
$control_dsc_nome["func"]="xt_buildeditcontrol";
$control_dsc_nome["params"] = array();
$control_dsc_nome["params"]["field"]="dsc_nome";
$control_dsc_nome["params"]["value"]=@$data["dsc_nome"];
//	Begin Add validation
$arrValidate = array();	

$control_dsc_nome["params"]["validate"]=$arrValidate;
//	End Add validation
$control_dsc_nome["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_dsc_nome["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_nome["params"]["mode"]="inline_edit";
else
	$control_dsc_nome["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_nome", $detailKeys))	
	$xt->assign("dsc_nome_editcontrol",$control_dsc_nome);
else if(array_key_exists("dsc_nome",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_nome", ""),"field=dsc%5Fnome","",MODE_VIEW);
		$xt->assign("dsc_nome_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('sms_celulares_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
