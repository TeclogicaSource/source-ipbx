...
include("include/cc_agentes_fila_variables.php");
...
$templatefile = ( $inlineedit ) ? "cc_agentes_fila_inline_edit.htm" : "cc_agentes_fila_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["idt_agentes_fila"]=postvalue("editid1");
$savedKeys["idt_agentes_fila"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_id_filas_".$id);
	$type=postvalue("type_id_filas_".$id);
	if(FieldSubmitted("id_filas_".$id))
	{
		
		$value=prepare_for_db("id_filas",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "id_filas"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["id_filas"]=$value;
		
		
	}


//	processibng id_filas - end
	}
	$condition = $inlineedit;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idt_agentes_fila"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: cc_agentes_fila_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_cc_agentes_fila->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: cc_agentes_fila_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["id_filas"]=$evalues["id_filas"];
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["idt_agentes_fila"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"idt_agentes_fila", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="cc_agentes_fila_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("idt_agentes_fila"))
		$key[]=GetFieldIndex("idt_agentes_fila");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('cc_agentes_fila_edit.php','".$skeys."','',function(){window.location.href='cc_agentes_fila_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('cc_agentes_fila_edit.php','".$skeys."','',function(){window.location.href='cc_agentes_fila_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["idt_agentes_fila"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["idt_agentes_fila"]));


//	id_filas - 

		$value="";
				$value=DisplayLookupWizard("id_filas",$data["id_filas"],$data,$keylink,MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "id_filas";
				$showRawValues[] = substr($data["id_filas"],0,100);
...
$regextype = '';
//	control - id_filas
$control_id_filas=array();
$control_id_filas["func"]="xt_buildeditcontrol";
$control_id_filas["params"] = array();
$control_id_filas["params"]["field"]="id_filas";
$control_id_filas["params"]["value"]=@$data["id_filas"];
//	Begin Add validation
$arrValidate = array();	

$arrValidate['basicValidate'][] = "IsRequired";

$control_id_filas["params"]["validate"]=$arrValidate;
//	End Add validation
$control_id_filas["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_id_filas["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_id_filas["params"]["mode"]="inline_edit";
else
	$control_id_filas["params"]["mode"]="edit";
if(!$detailKeys || !in_array("id_filas", $detailKeys))	
	$xt->assign("id_filas_editcontrol",$control_id_filas);
else if(array_key_exists("id_filas",$data))
{
				$value=DisplayLookupWizard("id_filas",$data["id_filas"],$data,"",MODE_VIEW);
		$xt->assign("id_filas_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('cc_agentes_fila_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
