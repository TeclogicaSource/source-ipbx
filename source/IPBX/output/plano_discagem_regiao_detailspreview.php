...
if($mastertable=="planos_discagem")
{
	$where ="";
		$where.= GetFullFieldName("id_plano")."=".make_db_value("id_plano",$_SESSION[$strTableName."_masterkey1"]);
}
...
if ( $rowcount ) {
	...
	//	regiao - 
			...
			$row["regiao_value"]=$value;
	//	id_tronco - 
		    $value="";
				$value=DisplayLookupWizard("id_tronco",$data["id_tronco"],$data,$keylink,MODE_PRINT);
			$row["id_tronco_value"]=$value;
	//	id_plano - 
		    $value="";
				$value=DisplayLookupWizard("id_plano",$data["id_plano"],$data,$keylink,MODE_PRINT);
			$row["id_plano_value"]=$value;
	//	tp_destino - 
		    $value="";
				$value = ProcessLargeText(GetData($data,"tp_destino", ""),"field=tp%5Fdestino".$keylink,"",MODE_PRINT);
			$row["tp_destino_value"]=$value;
	$rowinfo[]=$row;
	}
	$xt->assign_loopsection("details_row",$rowinfo);
} else {
}
$xt->display("plano_discagem_regiao_detailspreview.htm");
if($mode!="inline")
