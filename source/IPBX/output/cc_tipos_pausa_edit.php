...
include("include/cc_tipos_pausa_variables.php");
...
$templatefile = ( $inlineedit ) ? "cc_tipos_pausa_inline_edit.htm" : "cc_tipos_pausa_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["idt_pausa"]=postvalue("editid1");
$savedKeys["idt_pausa"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_dsc_pausa_".$id);
	$type=postvalue("type_dsc_pausa_".$id);
	if(FieldSubmitted("dsc_pausa_".$id))
	{
		
		$value=prepare_for_db("dsc_pausa",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "dsc_pausa"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["dsc_pausa"]=$value;
		
		
	}


//	processibng dsc_pausa - end
	}
	$condition = $inlineedit;
...
//	processibng flg_ativo - end
	...
	if($condition)
	{
	$value = postvalue("value_abreviado_".$id);
	$type=postvalue("type_abreviado_".$id);
	if(FieldSubmitted("abreviado_".$id))
	{
		
		$value=prepare_for_db("abreviado",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "abreviado"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["abreviado"]=$value;
		
		
	}


//	processibng abreviado - end
	}
	$condition = $inlineedit;

	if($condition)
	{
	$value = postvalue("value_flg_produtiva_".$id);
	$type=postvalue("type_flg_produtiva_".$id);
	if(FieldSubmitted("flg_produtiva_".$id))
	{
		
		$value=prepare_for_db("flg_produtiva",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "flg_produtiva"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["flg_produtiva"]=$value;
		
		
	}


//	processibng flg_produtiva - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idt_pausa"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: cc_tipos_pausa_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_cc_tipos_pausa->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: cc_tipos_pausa_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_pausa"]=$evalues["dsc_pausa"];
	$data["flg_ativo"]=$evalues["flg_ativo"];
	$data["abreviado"]=$evalues["abreviado"];
	$data["flg_produtiva"]=$evalues["flg_produtiva"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["idt_pausa"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"idt_pausa", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="cc_tipos_pausa_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("idt_pausa"))
		$key[]=GetFieldIndex("idt_pausa");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('cc_tipos_pausa_edit.php','".$skeys."','',function(){window.location.href='cc_tipos_pausa_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('cc_tipos_pausa_edit.php','".$skeys."','',function(){window.location.href='cc_tipos_pausa_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["idt_pausa"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["idt_pausa"]));


//	dsc_pausa - 

		$value="";
				$value = ProcessLargeText(GetData($data,"dsc_pausa", ""),"field=dsc%5Fpausa".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dsc_pausa";
				$showRawValues[] = substr($data["dsc_pausa"],0,100);

//	abreviado - 

		$value="";
				$value = ProcessLargeText(GetData($data,"abreviado", ""),"field=abreviado".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "abreviado";
				$showRawValues[] = substr($data["abreviado"],0,100);
...
//	flg_produtiva - Checkbox

		$value="";
				$value = GetData($data,"flg_produtiva", "Checkbox");
		$showValues[] = $value;
		$showFields[] = "flg_produtiva";
				$showRawValues[] = substr($data["flg_produtiva"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$regextype = '';
//	control - dsc_pausa
$control_dsc_pausa=array();
$control_dsc_pausa["func"]="xt_buildeditcontrol";
$control_dsc_pausa["params"] = array();
$control_dsc_pausa["params"]["field"]="dsc_pausa";
$control_dsc_pausa["params"]["value"]=@$data["dsc_pausa"];
//	Begin Add validation
$arrValidate = array();	

$control_dsc_pausa["params"]["validate"]=$arrValidate;
//	End Add validation
$control_dsc_pausa["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_dsc_pausa["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_pausa["params"]["mode"]="inline_edit";
else
	$control_dsc_pausa["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_pausa", $detailKeys))	
	$xt->assign("dsc_pausa_editcontrol",$control_dsc_pausa);
else if(array_key_exists("dsc_pausa",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_pausa", ""),"field=dsc%5Fpausa","",MODE_VIEW);
		$xt->assign("dsc_pausa_editcontrol",$value);
}
...
// add prevent submit on enter js if only one text record
//	control - abreviado
$control_abreviado=array();
$control_abreviado["func"]="xt_buildeditcontrol";
$control_abreviado["params"] = array();
$control_abreviado["params"]["field"]="abreviado";
$control_abreviado["params"]["value"]=@$data["abreviado"];
//	Begin Add validation
$arrValidate = array();	

$control_abreviado["params"]["validate"]=$arrValidate;
//	End Add validation
$control_abreviado["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_abreviado["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_abreviado["params"]["mode"]="inline_edit";
else
	$control_abreviado["params"]["mode"]="edit";
if(!$detailKeys || !in_array("abreviado", $detailKeys))	
	$xt->assign("abreviado_editcontrol",$control_abreviado);
else if(array_key_exists("abreviado",$data))
{
				$value = ProcessLargeText(GetData($data,"abreviado", ""),"field=abreviado","",MODE_VIEW);
		$xt->assign("abreviado_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - flg_produtiva
$control_flg_produtiva=array();
$control_flg_produtiva["func"]="xt_buildeditcontrol";
$control_flg_produtiva["params"] = array();
$control_flg_produtiva["params"]["field"]="flg_produtiva";
$control_flg_produtiva["params"]["value"]=@$data["flg_produtiva"];
//	Begin Add validation
$arrValidate = array();	


$control_flg_produtiva["params"]["validate"]=$arrValidate;
//	End Add validation
$control_flg_produtiva["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_flg_produtiva["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_flg_produtiva["params"]["mode"]="inline_edit";
else
	$control_flg_produtiva["params"]["mode"]="edit";
if(!$detailKeys || !in_array("flg_produtiva", $detailKeys))	
	$xt->assign("flg_produtiva_editcontrol",$control_flg_produtiva);
else if(array_key_exists("flg_produtiva",$data))
{
				$value = GetData($data,"flg_produtiva", "Checkbox");
		$xt->assign("flg_produtiva_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('cc_tipos_pausa_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
