...
$mode=SEARCH_SIMPLE;
$templatefile = "ipbx_canais_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "ipbx_canais_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'ipbx_canais';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "ipbx_canais";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("rdz_interf");
	...
	{
		$firstFieldParams['fName'] = "rdz_interf";
	...
	if(isEnableSection508())
		$xt->assign_section("rdz_interf_label","<label for=\"".GetInputElementId("rdz_interf", $id)."\">","</label>");
	else 
		$xt->assign("rdz_interf_label", true);
	
	$xt->assign("rdz_interf_fieldblock", true);		
	$xt->assignbyref("rdz_interf_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("rdz_interf_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("rdz_interf_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_rdz_interf", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("rdz_interf", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("rdz_interf")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("flg_disp");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "flg_disp";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("flg_disp_label","<label for=\"".GetInputElementId("flg_disp", $id)."\">","</label>");
	else 
		$xt->assign("flg_disp_label", true);
	
	$xt->assign("flg_disp_fieldblock", true);		
	$xt->assignbyref("flg_disp_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("flg_disp_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("flg_disp_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_flg_disp", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("flg_disp", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("flg_disp")."', ".$pageObject->id.", ".$ctrlsMap.");";
	...
	{
	$contents_block["begin"].="action=\"ipbx_canais_list.php\" ";
	}
