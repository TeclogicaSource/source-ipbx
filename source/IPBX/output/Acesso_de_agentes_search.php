...
include("include/Acesso_de_agentes_variables.php");
...
$mode=SEARCH_SIMPLE;
$templatefile = "Acesso_de_agentes_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "Acesso_de_agentes_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'Acesso_de_agentes';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "Rel. Acesso de agentes";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("hora");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("hora_label","<label for=\"".GetInputElementId("hora", $id)."\">","</label>");
	else 
		$xt->assign("hora_label", true);
	
	$xt->assign("hora_fieldblock", true);		
	$xt->assignbyref("hora_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("hora_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("hora_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_hora", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("hora", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("hora")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	...
	{
	$contents_block["begin"].="action=\"Acesso_de_agentes_report.php\" ";
	}
