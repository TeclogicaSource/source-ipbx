...
if($inlineadd==ADD_INLINE)
	$templatefile = "blacklist_inline_add.htm";
else
	$templatefile = "blacklist_add.htm";
...
//	processing dsc_bl_list - start
	...
	if(!($value===false))
	{
		...
		if(0 && "dsc_bl_list"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["dsc_bl_list"]=$value;
	}
	}
//	processibng dsc_bl_list - end
//	processing num_bl_list - start
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'blacklist_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'blacklist_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: blacklist_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_bl_list"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_bl_list"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='blacklist_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="blacklist_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="blacklist_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_bl_list"]);
...
$control_dsc_bl_list["params"]["value"]=@$defvalues["dsc_bl_list"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dsc_bl_list["params"]["mode"]="inline_add";
else
	$control_dsc_bl_list["params"]["mode"]="add";

if(!$detailKeys || !in_array("dsc_bl_list", $detailKeys))
	$xt->assignbyref("dsc_bl_list_editcontrol",$control_dsc_bl_list);
else if(array_key_exists("dsc_bl_list", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dsc_bl_list", ""),"field=dsc%5Fbl%5Flist","",MODE_VIEW);
...
$control_num_bl_list["params"]["value"]=@$defvalues["num_bl_list"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_num_bl_list["params"]["mode"]="inline_add";
else
	$control_num_bl_list["params"]["mode"]="add";

if(!$detailKeys || !in_array("num_bl_list", $detailKeys))
	$xt->assignbyref("num_bl_list_editcontrol",$control_num_bl_list);
else if(array_key_exists("num_bl_list", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"num_bl_list", ""),"field=num%5Fbl%5Flist","",MODE_VIEW);
