...
$mode=SEARCH_SIMPLE;
$templatefile = "parametros_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "parametros_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'parametros';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "parametros";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("valor");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("valor_label","<label for=\"".GetInputElementId("valor", $id)."\">","</label>");
	else 
		$xt->assign("valor_label", true);
	
	$xt->assign("valor_fieldblock", true);		
	$xt->assignbyref("valor_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("valor_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("valor_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_valor", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("valor", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("valor")."', ".$pageObject->id.", ".$ctrlsMap.");";
	...
	{
	$contents_block["begin"].="action=\"parametros_list.php\" ";
	}
