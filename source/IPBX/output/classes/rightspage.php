...
class RightsPage extends ListPage  
{
	...
	function getRights() 
	{
		$trs = db_query("select GroupID,TableName,AccessMask from `ipbx_ugrights` order by GroupID", $this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			foreach(postvalue("table") as $table)
			{
				...
				$sql="delete from `ipbx_ugrights` where TableName='".db_addslashes($table)."'";
				...
				foreach($this->groupsArr as $grp)
				{
					...
					if(@$_POST["cbadm_".$gtable."_".$grp[0]])
						$mask.="M";
					if($mask)
						db_exec("insert into `ipbx_ugrights` (TableName,GroupID,AccessMask) values ('".db_addslashes($table)."',".$grp[0].",'".$mask."')",$this->conn);
				}
			}
		}
	}
...
*/		
	...
	function addCommonJs() {
		...
		$this->AddJSCode("window.TEXT_AA_ADD_NEW_GROUP = \""."Adicionar novo Grupo"."\";
			window.TEXT_AA_RENAMEGROUP = \""."Renomear Grupo"."\";
			window.renameidx=-1;
			...
			window.deletegroup = function ()
			{
				...
				if(!confirm('".jsreplace("Deseja de fato eliminar o grupo")." \\''+disp+'\\'?')) 
					return;
				$.get('ug_group.php',
					{	
						rndval: Math.random(),
