...
$pageObject->body["begin"] .="<form action=\"Simplificado_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Simplificado_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Simplificado.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Simplificado.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Simplificado.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["dsc_centro_cust"] = DisplayLookupWizard("dsc_centro_cust",$row["dsc_centro_cust"],$row,"",MODE_EXPORT);		
			...
			$values["dsc_interf"] = DisplayLookupWizard("dsc_interf",$row["dsc_interf"],$row,"",MODE_EXPORT);		
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Simplificado.csv");
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Mês").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Ano").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Interface de saída").'</td>';	
	}
	else
	{
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["idt_custo"] = GetData($row,"idt_custo",$format);
					$values["dsc_centro_cust"] = "";	
			if(strlen($row["dsc_centro_cust"]))
			{
				$values["dsc_centro_cust"] = DisplayLookupWizard("dsc_centro_cust", $row["dsc_centro_cust"], $row,"",MODE_EXPORT);
			}
