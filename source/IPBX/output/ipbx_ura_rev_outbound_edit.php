...
include("include/ipbx_ura_rev_outbound_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_ura_rev_outbound_inline_edit.htm" : "ipbx_ura_rev_outbound_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_ura_rev_outbound"]=postvalue("editid1");
$savedKeys["id_ura_rev_outbound"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_arquivo_".$id);
	$type=postvalue("type_arquivo_".$id);
	if(FieldSubmitted("arquivo_".$id))
	{
		
			$fileNameForPrepareFunc = postvalue("filename_arquivo_".$id);
		if($fileNameForPrepareFunc)
			$value = $fileNameForPrepareFunc;
		if(substr($type,0,4)=="file")
		{		
			$value = prepare_file($value,"arquivo",$type,$fileNameForPrepareFunc ,$id);
		}
		else if(substr($type,0,6)=="upload")
		{		
			$value=prepare_upload("arquivo",$type,$fileNameForPrepareFunc,$value,"" ,$id);
		}
	...
	if(0 && "arquivo"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["arquivo"]=$value;
		
		
	}


//	processibng arquivo - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_ura_rev_outbound"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_ura_rev_outbound_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_ura_rev_outbound->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_ura_rev_outbound_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_ura_rev_outbound"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_ura_rev_outbound", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_ura_rev_outbound_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_ura_rev_outbound"))
		$key[]=GetFieldIndex("id_ura_rev_outbound");
...
//create a line where and order for the two queries
	if($lenArr>0 and count($key) and !$_SESSION[$strTableName."_noNextPrev"])
	{
		...
		if($where_next!="" and $order_next!="" and $where_prev!="" and $order_prev!="")
		{
			...
			{
					$next[1]=$row_next["id_ura_rev_outbound"];
			}
			
			$res_prev=db_query($sql_prev,$conn);	
			if($row_prev=db_fetch_array($res_prev))
			{
					$prev[1]=$row_prev["id_ura_rev_outbound"];
			}
		}
	}
}
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_ura_rev_outbound_edit.php','".$skeys."','',function(){window.location.href='ipbx_ura_rev_outbound_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_ura_rev_outbound_edit.php','".$skeys."','',function(){window.location.href='ipbx_ura_rev_outbound_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_ura_rev_outbound"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_ura_rev_outbound"]));
...
//	arquivo - Document Download
		...
		$showValues[] = $value;
		$showFields[] = "arquivo";
				$showRawValues[] = substr($data["arquivo"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$regextype = '';
//	control - arquivo
$control_arquivo=array();
$control_arquivo["func"]="xt_buildeditcontrol";
$control_arquivo["params"] = array();
$control_arquivo["params"]["field"]="arquivo";
$control_arquivo["params"]["value"]=@$data["arquivo"];
//	Begin Add validation
$arrValidate = array();	

$arrValidate['basicValidate'][] = "IsRequired";

$control_arquivo["params"]["validate"]=$arrValidate;
//	End Add validation
$control_arquivo["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_arquivo["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_arquivo["params"]["mode"]="inline_edit";
else
	$control_arquivo["params"]["mode"]="edit";
if(!$detailKeys || !in_array("arquivo", $detailKeys))	
	$xt->assign("arquivo_editcontrol",$control_arquivo);
else if(array_key_exists("arquivo",$data))
{
				$value = GetData($data,"arquivo", "Document Download");
		$xt->assign("arquivo_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_ura_rev_outbound_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
