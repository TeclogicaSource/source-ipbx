...
$templatefile = "ipbx_pesquisa_ura_rev_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_pesquisa"]=postvalue("editid1");
$savedKeys["id_pesquisa"]=postvalue("editid1");
...
{
	$ids = $id;
	$mKeys["ipbx_ura_rev_msg"] = GetMasterKeysByDetailTable("ipbx_ura_rev_msg", $strTableName);
	$dpParams['strTableNames'][] = "ipbx_ura_rev_msg";
	$dpParams['ids'][] = ++$ids;
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_dsc_pesquisa_".$id);
	$type=postvalue("type_dsc_pesquisa_".$id);
	if(FieldSubmitted("dsc_pesquisa_".$id))
	{
		
		$value=prepare_for_db("dsc_pesquisa",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "dsc_pesquisa"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["dsc_pesquisa"]=$value;
		
		
	}


//	processibng dsc_pesquisa - end
	}
	$condition = !$inlineedit;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_pesquisa"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_pesquisa_ura_rev_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_pesquisa_ura_rev->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_pesquisa_ura_rev_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["dsc_pesquisa"]=$evalues["dsc_pesquisa"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_pesquisa"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_pesquisa", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_pesquisa_ura_rev_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_pesquisa"))
		$key[]=GetFieldIndex("id_pesquisa");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_pesquisa_ura_rev_edit.php','".$skeys."','',function(){window.location.href='ipbx_pesquisa_ura_rev_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_pesquisa_ura_rev_edit.php','".$skeys."','',function(){window.location.href='ipbx_pesquisa_ura_rev_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_pesquisa"]);
if($message)
{
...
$regexmessage='';
$regextype = '';
//	control - dsc_pesquisa
$control_dsc_pesquisa=array();
$control_dsc_pesquisa["func"]="xt_buildeditcontrol";
$control_dsc_pesquisa["params"] = array();
$control_dsc_pesquisa["params"]["field"]="dsc_pesquisa";
$control_dsc_pesquisa["params"]["value"]=@$data["dsc_pesquisa"];
//	Begin Add validation
$arrValidate = array();	

$arrValidate['basicValidate'][] = "IsRequired";

$control_dsc_pesquisa["params"]["validate"]=$arrValidate;
//	End Add validation
$control_dsc_pesquisa["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_dsc_pesquisa["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_pesquisa["params"]["mode"]="inline_edit";
else
	$control_dsc_pesquisa["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_pesquisa", $detailKeys))	
	$xt->assign("dsc_pesquisa_editcontrol",$control_dsc_pesquisa);
else if(array_key_exists("dsc_pesquisa",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_pesquisa", ""),"field=dsc%5Fpesquisa","",MODE_VIEW);
		$xt->assign("dsc_pesquisa_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_pesquisa_ura_rev_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
