...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_ipbx_ura_rev_msg"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["id_ipbx_ura_rev_msg"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"ipbx_ura_rev_msg_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("ipbx_ura_rev_msg_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_ura_rev_msg.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_ura_rev_msg.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_ura_rev_msg.xml");
	...
	{
		$values = array();
			$values["id_ipbx_ura_rev_msg"] = GetData($row,"id_ipbx_ura_rev_msg","");
			$values["arq_audio"] = GetData($row,"arq_audio","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_ura_rev_msg.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["id_ipbx_ura_rev_msg"] = htmlspecialchars(GetData($row,"id_ipbx_ura_rev_msg",$format));
			$format=FORMAT_NONE;
			$values["arq_audio"] = htmlspecialchars(GetData($row,"arq_audio",$format));
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_ipbx_ura_rev_msg"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["arq_audio"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Identificador").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Pergunta (Arquivo de áudio)").'</td>';	
	}
	else
	{
		echo "<td>"."Identificador"."</td>";
		echo "<td>"."Pergunta (Arquivo de áudio)"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["id_ipbx_ura_rev_msg"] = GetData($row,"id_ipbx_ura_rev_msg",$format);
					
							$format=FORMAT_NONE;
			
			$values["arq_audio"] = GetData($row,"arq_audio",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["id_ipbx_ura_rev_msg"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									$format=FORMAT_NONE;
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["arq_audio"]);
					else
						echo htmlspecialchars($values["arq_audio"]);
			echo '</td>';
			echo "</tr>";
		}		
