...
$templatefile = ( $inlineedit ) ? "ipbx_plano_disc_inline_edit.htm" : "ipbx_plano_disc_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_plano"]=postvalue("editid1");
$savedKeys["id_plano"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_plano"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_plano_disc_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_plano_disc->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_plano_disc_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["padrao"]=$evalues["padrao"];
	$data["contexto"]=$evalues["contexto"];
	$data["rota"]=$evalues["rota"];
	$data["id_interf_1"]=$evalues["id_interf_1"];
	$data["add_rota_1"]=$evalues["add_rota_1"];
	$data["id_interf_2"]=$evalues["id_interf_2"];
	$data["add_rota_2"]=$evalues["add_rota_2"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_plano"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_plano", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_plano_disc_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_plano"))
		$key[]=GetFieldIndex("id_plano");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_plano_disc_edit.php','".$skeys."','',function(){window.location.href='ipbx_plano_disc_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_plano_disc_edit.php','".$skeys."','',function(){window.location.href='ipbx_plano_disc_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_plano"]);
if($message)
{
...
$control_padrao["params"]["value"]=@$data["padrao"];
...
$control_padrao["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_padrao["params"]["mode"]="inline_edit";
else
	$control_padrao["params"]["mode"]="edit";
if(!$detailKeys || !in_array("padrao", $detailKeys))	
	$xt->assign("padrao_editcontrol",$control_padrao);
else if(array_key_exists("padrao",$data))
{
				$value = ProcessLargeText(GetData($data,"padrao", ""),"field=padrao","",MODE_VIEW);
...
$control_contexto["params"]["value"]=@$data["contexto"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_contexto["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_contexto["params"]["mode"]="inline_edit";
else
	$control_contexto["params"]["mode"]="edit";
if(!$detailKeys || !in_array("contexto", $detailKeys))	
	$xt->assign("contexto_editcontrol",$control_contexto);
else if(array_key_exists("contexto",$data))
{
				$value = ProcessLargeText(GetData($data,"contexto", ""),"field=contexto","",MODE_VIEW);
...
$control_rota["params"]["value"]=@$data["rota"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_rota["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_rota["params"]["mode"]="inline_edit";
else
	$control_rota["params"]["mode"]="edit";
if(!$detailKeys || !in_array("rota", $detailKeys))	
	$xt->assign("rota_editcontrol",$control_rota);
else if(array_key_exists("rota",$data))
{
				$value = ProcessLargeText(GetData($data,"rota", ""),"field=rota","",MODE_VIEW);
...
$control_id_interf_1["params"]["value"]=@$data["id_interf_1"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_id_interf_1["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_id_interf_1["params"]["mode"]="inline_edit";
else
	$control_id_interf_1["params"]["mode"]="edit";
if(!$detailKeys || !in_array("id_interf_1", $detailKeys))	
	$xt->assign("id_interf_1_editcontrol",$control_id_interf_1);
else if(array_key_exists("id_interf_1",$data))
{
				$value=DisplayLookupWizard("id_interf_1",$data["id_interf_1"],$data,"",MODE_VIEW);
...
$control_add_rota_1["params"]["value"]=@$data["add_rota_1"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_add_rota_1["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_add_rota_1["params"]["mode"]="inline_edit";
else
	$control_add_rota_1["params"]["mode"]="edit";
if(!$detailKeys || !in_array("add_rota_1", $detailKeys))	
	$xt->assign("add_rota_1_editcontrol",$control_add_rota_1);
else if(array_key_exists("add_rota_1",$data))
{
				$value = ProcessLargeText(GetData($data,"add_rota_1", ""),"field=add%5Frota%5F1","",MODE_VIEW);
...
$control_id_interf_2["params"]["value"]=@$data["id_interf_2"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_id_interf_2["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_id_interf_2["params"]["mode"]="inline_edit";
else
	$control_id_interf_2["params"]["mode"]="edit";
if(!$detailKeys || !in_array("id_interf_2", $detailKeys))	
	$xt->assign("id_interf_2_editcontrol",$control_id_interf_2);
else if(array_key_exists("id_interf_2",$data))
{
				$value=DisplayLookupWizard("id_interf_2",$data["id_interf_2"],$data,"",MODE_VIEW);
...
$control_add_rota_2["params"]["value"]=@$data["add_rota_2"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_add_rota_2["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_add_rota_2["params"]["mode"]="inline_edit";
else
	$control_add_rota_2["params"]["mode"]="edit";
if(!$detailKeys || !in_array("add_rota_2", $detailKeys))	
	$xt->assign("add_rota_2_editcontrol",$control_add_rota_2);
else if(array_key_exists("add_rota_2",$data))
{
				$value = ProcessLargeText(GetData($data,"add_rota_2", ""),"field=add%5Frota%5F2","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_plano_disc_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
