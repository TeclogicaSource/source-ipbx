...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_interf_voxip_inline_add.htm";
else
	$templatefile = "ipbx_interf_voxip_add.htm";
...
if($isShowDetailTables && ($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER))
{
	...
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			...
			'mShortTableName':'ipbx_interf_voxip', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
if(@$_POST["a"]=="added")
{
	...
	$files_move=array();
	$files_save=array();
//	processing qtd_cham_por_ramal - start
	...
	if(!($value===false))
	{
		...
		if(0 && "qtd_cham_por_ramal"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["qtd_cham_por_ramal"]=$value;
	}
	}
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_interf_voxip_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_interf_voxip_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_interf_voxip_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["qtd_cham_por_ramal"]=@$avalues["qtd_cham_por_ramal"];
}
//for basic files
$includes="";
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_interf_voxip_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_voxip_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_voxip_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
$control_qtd_cham_por_ramal["params"]["value"]=@$defvalues["qtd_cham_por_ramal"];
...
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_qtd_cham_por_ramal["params"]["mode"]="inline_add";
else
	$control_qtd_cham_por_ramal["params"]["mode"]="add";

if(!$detailKeys || !in_array("qtd_cham_por_ramal", $detailKeys))
	$xt->assignbyref("qtd_cham_por_ramal_editcontrol",$control_qtd_cham_por_ramal);
else if(array_key_exists("qtd_cham_por_ramal", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"qtd_cham_por_ramal", ""),"field=qtd%5Fcham%5Fpor%5Framal","",MODE_VIEW);
