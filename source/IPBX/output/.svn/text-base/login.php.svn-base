...
$logacc=true;
	...
	if($auditObj)
	{
		...
		{
			$logacc=false;
			$message=mysprintf("Acesso negado po %s minutos",array($auditObj->LoginAccess()));
		}
	}
...
//   	 username and password are stored in the database
	...
	if(NeedQuotes($cPasswordFieldType))
		...
		$strSQL = "select * from `ipbx_ramais` where ".AddFieldWrappers($cUserNameField).
