...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["idt_tronco"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["idt_tronco"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"pabx_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("pabx_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=pabx.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=pabx.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=pabx.xml");
	...
	{
		$values = array();
			$values["ramal"] = DisplayLookupWizard("ramal",$row["ramal"],$row,"",MODE_EXPORT);		
			...
			$values["id_horario"] = DisplayLookupWizard("id_horario",$row["id_horario"],$row,"",MODE_EXPORT);		
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=pabx.csv");
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Contexto").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Horario").'</td>';	
	}
	else
	{
		echo "<td>"."Ramal"."</td>";
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["id_horario"]);
				else
