...
$templatefile = ( $inlineedit ) ? "interf_ms_lync_inline_edit.htm" : "interf_ms_lync_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_ms_lync"]=postvalue("editid1");
$savedKeys["id_ms_lync"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_ms_lync"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: interf_ms_lync_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_interf_ms_lync->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: interf_ms_lync_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_ms_lync"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_ms_lync", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="interf_ms_lync_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_ms_lync"))
		$key[]=GetFieldIndex("id_ms_lync");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('interf_ms_lync_edit.php','".$skeys."','',function(){window.location.href='interf_ms_lync_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('interf_ms_lync_edit.php','".$skeys."','',function(){window.location.href='interf_ms_lync_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_ms_lync"]);
if($message)
{
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('interf_ms_lync_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
