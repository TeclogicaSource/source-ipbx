...
include("include/ipbx_ura_rev_conf_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_ura_rev_conf_inline_edit.htm" : "ipbx_ura_rev_conf_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_ipbx_ura_rev"]=postvalue("editid1");
$savedKeys["id_ipbx_ura_rev"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_mx_tent_".$id);
	$type=postvalue("type_mx_tent_".$id);
	if(FieldSubmitted("mx_tent_".$id))
	{
		
		$value=prepare_for_db("mx_tent",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "mx_tent"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["mx_tent"]=$value;
		
		
	}


//	processibng mx_tent - end
	}
	$condition = $inlineedit;

	if($condition)
	{
	$value = postvalue("value_tp_entre_tent_".$id);
	$type=postvalue("type_tp_entre_tent_".$id);
	if(FieldSubmitted("tp_entre_tent_".$id))
	{
		
		$value=prepare_for_db("tp_entre_tent",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "tp_entre_tent"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["tp_entre_tent"]=$value;
		
		
	}


//	processibng tp_entre_tent - end
	}
	$condition = $inlineedit;
...
//	processibng hr_fim - end
	...
	if($condition)
	{
	$value = postvalue("value_mx_chamada_".$id);
	$type=postvalue("type_mx_chamada_".$id);
	if(FieldSubmitted("mx_chamada_".$id))
	{
		
		$value=prepare_for_db("mx_chamada",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "mx_chamada"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["mx_chamada"]=$value;
		
		
	}


//	processibng mx_chamada - end
	}
	$condition = $inlineedit;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_ipbx_ura_rev"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_ura_rev_conf_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_ura_rev_conf->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_ura_rev_conf_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	$data["mx_tent"]=$evalues["mx_tent"];
	$data["tp_entre_tent"]=$evalues["tp_entre_tent"];
	$data["hr_inicio"]=$evalues["hr_inicio"];
	$data["hr_fim"]=$evalues["hr_fim"];
	$data["mx_chamada"]=$evalues["mx_chamada"];
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_ipbx_ura_rev"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_ipbx_ura_rev", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_ura_rev_conf_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_ipbx_ura_rev"))
		$key[]=GetFieldIndex("id_ipbx_ura_rev");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_ura_rev_conf_edit.php','".$skeys."','',function(){window.location.href='ipbx_ura_rev_conf_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_ura_rev_conf_edit.php','".$skeys."','',function(){window.location.href='ipbx_ura_rev_conf_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_ipbx_ura_rev"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_ipbx_ura_rev"]));


//	mx_tent - 

		$value="";
				$value = ProcessLargeText(GetData($data,"mx_tent", ""),"field=mx%5Ftent".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "mx_tent";
				$showRawValues[] = substr($data["mx_tent"],0,100);

//	tp_entre_tent - 

		$value="";
				$value = ProcessLargeText(GetData($data,"tp_entre_tent", ""),"field=tp%5Fentre%5Ftent".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "tp_entre_tent";
				$showRawValues[] = substr($data["tp_entre_tent"],0,100);
...
//	mx_chamada - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"mx_chamada", ""),"field=mx%5Fchamada".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "mx_chamada";
				$showRawValues[] = substr($data["mx_chamada"],0,100);
...
$regextype = '';
//	control - mx_tent
$control_mx_tent=array();
$control_mx_tent["func"]="xt_buildeditcontrol";
$control_mx_tent["params"] = array();
$control_mx_tent["params"]["field"]="mx_tent";
$control_mx_tent["params"]["value"]=@$data["mx_tent"];
...
$control_mx_tent["params"]["validate"]=$arrValidate;
//	End Add validation
$control_mx_tent["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_mx_tent["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_mx_tent["params"]["mode"]="inline_edit";
else
	$control_mx_tent["params"]["mode"]="edit";
if(!$detailKeys || !in_array("mx_tent", $detailKeys))	
	$xt->assign("mx_tent_editcontrol",$control_mx_tent);
else if(array_key_exists("mx_tent",$data))
{
				$value = ProcessLargeText(GetData($data,"mx_tent", ""),"field=mx%5Ftent","",MODE_VIEW);
		$xt->assign("mx_tent_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - tp_entre_tent
$control_tp_entre_tent=array();
$control_tp_entre_tent["func"]="xt_buildeditcontrol";
$control_tp_entre_tent["params"] = array();
$control_tp_entre_tent["params"]["field"]="tp_entre_tent";
$control_tp_entre_tent["params"]["value"]=@$data["tp_entre_tent"];
//	Begin Add validation
$arrValidate = array();	
$validatetype = getJsValidatorName("Number");
$arrValidate['basicValidate'][] = $validatetype;
...
$control_tp_entre_tent["params"]["validate"]=$arrValidate;
//	End Add validation
$control_tp_entre_tent["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_tp_entre_tent["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_tp_entre_tent["params"]["mode"]="inline_edit";
else
	$control_tp_entre_tent["params"]["mode"]="edit";
if(!$detailKeys || !in_array("tp_entre_tent", $detailKeys))	
	$xt->assign("tp_entre_tent_editcontrol",$control_tp_entre_tent);
else if(array_key_exists("tp_entre_tent",$data))
{
				$value = ProcessLargeText(GetData($data,"tp_entre_tent", ""),"field=tp%5Fentre%5Ftent","",MODE_VIEW);
		$xt->assign("tp_entre_tent_editcontrol",$value);
}
...
// add prevent submit on enter js if only one text record
//	control - mx_chamada
$control_mx_chamada=array();
$control_mx_chamada["func"]="xt_buildeditcontrol";
$control_mx_chamada["params"] = array();
$control_mx_chamada["params"]["field"]="mx_chamada";
$control_mx_chamada["params"]["value"]=@$data["mx_chamada"];
//	Begin Add validation
$arrValidate = array();	
$validatetype = getJsValidatorName("Number");
$arrValidate['basicValidate'][] = $validatetype;
...
$control_mx_chamada["params"]["validate"]=$arrValidate;
//	End Add validation
$control_mx_chamada["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_mx_chamada["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_mx_chamada["params"]["mode"]="inline_edit";
else
	$control_mx_chamada["params"]["mode"]="edit";
if(!$detailKeys || !in_array("mx_chamada", $detailKeys))	
	$xt->assign("mx_chamada_editcontrol",$control_mx_chamada);
else if(array_key_exists("mx_chamada",$data))
{
				$value = ProcessLargeText(GetData($data,"mx_chamada", ""),"field=mx%5Fchamada","",MODE_VIEW);
		$xt->assign("mx_chamada_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_ura_rev_conf_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
