...
include("include/parametros_variables.php");
...
$templatefile = ( $inlineedit ) ? "parametros_inline_edit.htm" : "parametros_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_param"]=postvalue("editid1");
$savedKeys["id_param"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_valor_".$id);
	$type=postvalue("type_valor_".$id);
	if(FieldSubmitted("valor_".$id))
	{
		
		$value=prepare_for_db("valor",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "valor"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["valor"]=$value;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_param"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: parametros_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_parametros->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: parametros_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_param"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_param", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="parametros_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_param"))
		$key[]=GetFieldIndex("id_param");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('parametros_edit.php','".$skeys."','',function(){window.location.href='parametros_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('parametros_edit.php','".$skeys."','',function(){window.location.href='parametros_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_param"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_param"]));
...
//	valor - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"valor", ""),"field=valor".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "valor";
				$showRawValues[] = substr($data["valor"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
//	control - valor
$control_valor=array();
$control_valor["func"]="xt_buildeditcontrol";
$control_valor["params"] = array();
$control_valor["params"]["field"]="valor";
$control_valor["params"]["value"]=@$data["valor"];
//	Begin Add validation
$arrValidate = array();	

$arrValidate['basicValidate'][] = "IsRequired";

$control_valor["params"]["validate"]=$arrValidate;
//	End Add validation
$control_valor["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_valor["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_valor["params"]["mode"]="inline_edit";
else
	$control_valor["params"]["mode"]="edit";
if(!$detailKeys || !in_array("valor", $detailKeys))	
	$xt->assign("valor_editcontrol",$control_valor);
else if(array_key_exists("valor",$data))
{
				$value = ProcessLargeText(GetData($data,"valor", ""),"field=valor","",MODE_VIEW);
		$xt->assign("valor_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('parametros_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
