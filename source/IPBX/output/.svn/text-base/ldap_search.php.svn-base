...
$mode=SEARCH_SIMPLE;
$templatefile = "ldap_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "ldap_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'ldap';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "ldap";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("chave");
	...
	$xt->assignbyref("chave_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("chave_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("chave_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_chave", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("chave", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("chave")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("nm_usuario");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "nm_usuario";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("nm_usuario_label","<label for=\"".GetInputElementId("nm_usuario", $id)."\">","</label>");
	else 
		$xt->assign("nm_usuario_label", true);
	
	$xt->assign("nm_usuario_fieldblock", true);		
	$xt->assignbyref("nm_usuario_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("nm_usuario_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("nm_usuario_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_nm_usuario", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("nm_usuario", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("nm_usuario")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	...
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("email")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("name2");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("name2_label","<label for=\"".GetInputElementId("name2", $id)."\">","</label>");
	else 
		$xt->assign("name2_label", true);
	
	$xt->assign("name2_fieldblock", true);		
	$xt->assignbyref("name2_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("name2_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("name2_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_name2", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("name2", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("name2")."', ".$pageObject->id.", ".$ctrlsMap.");";
	...
	{
	$contents_block["begin"].="action=\"ldap_list.php\" ";
	}
