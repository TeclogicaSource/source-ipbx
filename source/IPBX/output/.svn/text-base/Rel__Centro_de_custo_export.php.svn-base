...
$pageObject->body["begin"] .="<form action=\"Rel__Centro_de_custo_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel__Centro_de_custo_export.htm");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Centro_de_custo.xml");
	...
	{
		$values = array();
			$values["Minutos"] = GetData($row,"Minutos","");
			$values["dt"] = GetData($row,"dt","");
			$values["Mes"] = DisplayLookupWizard("Mes",$row["Mes"],$row,"",MODE_EXPORT);		
			$values["Ano"] = DisplayLookupWizard("Ano",$row["Ano"],$row,"",MODE_EXPORT);		
			$values["dsc_centro_cust"] = GetData($row,"dsc_centro_cust","");
			$values["Total MES"] = GetData($row,"Total MES","");
			$values["Percentual"] = GetData($row,"Percentual","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Centro_de_custo.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="Number";
			$values["Minutos"] = htmlspecialchars($row["Minutos"]);
			$format="";
			$values["dt"] = htmlspecialchars(GetData($row,"dt",$format));
		$values["Mes"] = DisplayLookupWizard("Mes",$row["Mes"],$row,"",MODE_EXPORT);
		$values["Ano"] = DisplayLookupWizard("Ano",$row["Ano"],$row,"",MODE_EXPORT);
			$format="";
			$values["dsc_centro_cust"] = htmlspecialchars(GetData($row,"dsc_centro_cust",$format));
			$format="Number";
			$values["Total MES"] = htmlspecialchars($row["Total MES"]);
			$format="Number";
			$values["Percentual"] = htmlspecialchars($row["Percentual"]);
			$format="Number";
			$values["custo"] = htmlspecialchars($row["custo"]);
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Minutos"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["dt"]).'"';
			if($outstr!="")
				$outstr.=",";
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["dsc_centro_cust"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Total MES"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Percentual"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Minutos").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Data").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Mes de referência").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Ano de referência").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Centro de Custo").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Total MES").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Percentual (%)").'</td>';	
	...
	else
	{
		echo "<td>"."Minutos"."</td>";
		echo "<td>"."Data"."</td>";
		echo "<td>"."Mes de referência"."</td>";
		echo "<td>"."Ano de referência"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["Minutos"] = GetData($row,"Minutos",$format);
					
							$format="";
			
			$values["dt"] = GetData($row,"dt",$format);
					$values["Mes"] = "";	
			if(strlen($row["Mes"]))
			{
				$values["Mes"] = DisplayLookupWizard("Mes", $row["Mes"], $row,"",MODE_EXPORT);
			...
			$values["dsc_centro_cust"] = GetData($row,"dsc_centro_cust",$format);
					
							$format="Number";
			
			$values["Total MES"] = GetData($row,"Total MES",$format);
					
							$format="Number";
			
			$values["Percentual"] = GetData($row,"Percentual",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="Number";
									echo htmlspecialchars($values["Minutos"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["dt"]);
					else
						echo htmlspecialchars($values["dt"]);
			echo '</td>';
							echo '<td>';
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["dsc_centro_cust"]);
					else
						echo htmlspecialchars($values["dsc_centro_cust"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="Number";
									echo htmlspecialchars($values["Total MES"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="Number";
									echo htmlspecialchars($values["Percentual"]);
			echo '</td>';
							echo '<td>';
