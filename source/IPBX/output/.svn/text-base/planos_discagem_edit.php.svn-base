...
include("include/planos_discagem_variables.php");
...
$templatefile = ( $inlineedit ) ? "planos_discagem_inline_edit.htm" : "planos_discagem_edit.htm";
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_DDD_".$id);
	$type=postvalue("type_DDD_".$id);
	if(FieldSubmitted("DDD_".$id))
	{
		
		$value=prepare_for_db("DDD",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "DDD"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["DDD"]=$value;
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: planos_discagem_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_planos_discagem->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: planos_discagem_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$xt->assign("DDD_fieldblock",true);
	$xt->assign("DDD_label",true);
	if(isEnableSection508())
		$xt->assign_section("DDD_label","<label for=\"".GetInputElementId("DDD", $id)."\">","</label>");
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="planos_discagem_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('planos_discagem_edit.php','".$skeys."','',function(){window.location.href='planos_discagem_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('planos_discagem_edit.php','".$skeys."','',function(){window.location.href='planos_discagem_list.php?a=return'});return false;\"");
...
//	DDD - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"DDD", ""),"field=DDD".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "DDD";
				$showRawValues[] = substr($data["DDD"],0,100);
...
//	control - DDD
$control_DDD=array();
$control_DDD["func"]="xt_buildeditcontrol";
$control_DDD["params"] = array();
$control_DDD["params"]["field"]="DDD";
$control_DDD["params"]["value"]=@$data["DDD"];
//	Begin Add validation
$arrValidate = array();	

$control_DDD["params"]["validate"]=$arrValidate;
//	End Add validation
$control_DDD["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_DDD["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_DDD["params"]["mode"]="inline_edit";
else
	$control_DDD["params"]["mode"]="edit";
if(!$detailKeys || !in_array("DDD", $detailKeys))	
	$xt->assign("DDD_editcontrol",$control_DDD);
else if(array_key_exists("DDD",$data))
{
				$value = ProcessLargeText(GetData($data,"DDD", ""),"field=DDD","",MODE_VIEW);
		$xt->assign("DDD_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('planos_discagem_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
