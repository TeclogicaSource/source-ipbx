...
$mode=SEARCH_SIMPLE;
$templatefile = "ipbx_dt_especifica_feriado_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode=SEARCH_LOAD_CONTROL;
	$templatefile = "ipbx_dt_especifica_feriado_inline_search.htm";
}	
...
$params['xt'] = &$xt;
$params['shortTableName'] = 'ipbx_dt_especifica_feriado';
$params['origTName'] = $strOriginalTableName;
$params['dataSourceTable'] = "ipbx_dt_especifica_feriado";
...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("dsc_data");
	...
	{
		$firstFieldParams['fName'] = "dsc_data";
	...
	if(isEnableSection508())
		$xt->assign_section("dsc_data_label","<label for=\"".GetInputElementId("dsc_data", $id)."\">","</label>");
	else 
		$xt->assign("dsc_data_label", true);
	
	$xt->assign("dsc_data_fieldblock", true);		
	$xt->assignbyref("dsc_data_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("dsc_data_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("dsc_data_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_dsc_data", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("dsc_data", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("dsc_data")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("dt_especifica");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "dt_especifica";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("dt_especifica_label","<label for=\"".GetInputElementId("dt_especifica", $id)."\">","</label>");
	else 
		$xt->assign("dt_especifica_label", true);
	
	$xt->assign("dt_especifica_fieldblock", true);		
	$xt->assignbyref("dt_especifica_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("dt_especifica_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("dt_especifica_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_dt_especifica", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("dt_especifica", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("dt_especifica")."', ".$pageObject->id.", ".$ctrlsMap.");";
	...
	{
	$contents_block["begin"].="action=\"ipbx_dt_especifica_feriado_list.php\" ";
	}
