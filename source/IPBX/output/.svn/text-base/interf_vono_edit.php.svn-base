...
$templatefile = ( $inlineedit ) ? "interf_vono_inline_edit.htm" : "interf_vono_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_interf_vono"]=postvalue("editid1");
$savedKeys["id_interf_vono"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_interf_vono"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: interf_vono_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_interf_vono->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: interf_vono_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_interf_vono"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_interf_vono", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="interf_vono_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_interf_vono"))
		$key[]=GetFieldIndex("id_interf_vono");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('interf_vono_edit.php','".$skeys."','',function(){window.location.href='interf_vono_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('interf_vono_edit.php','".$skeys."','',function(){window.location.href='interf_vono_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_interf_vono"]);
if($message)
{
...
{
				$value = ProcessLargeText(GetData($data,"codec", ""),"field=codec","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('interf_vono_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
