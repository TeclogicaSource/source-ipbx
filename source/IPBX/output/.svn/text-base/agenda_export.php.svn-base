...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["idt_agenda"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["idt_agenda"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"agenda_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("agenda_export.htm");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=agenda.xml");
	...
	{
		$values = array();
			$values["empresa"] = GetData($row,"empresa","");
			$values["flg_show_mobile"] = GetData($row,"flg_show_mobile","");
			$values["solicitante"] = GetData($row,"solicitante","");
			$values["Nome"] = GetData($row,"Nome","");
			$values["Numero"] = GetData($row,"Numero","");
			$values["idt_custo"] = DisplayLookupWizard("idt_custo",$row["idt_custo"],$row,"",MODE_EXPORT);		
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["empresa"] = htmlspecialchars(GetData($row,"empresa",$format));
			$format=FORMAT_NONE;
			$values["flg_show_mobile"] = htmlspecialchars(GetData($row,"flg_show_mobile",$format));
			$format="";
			$values["solicitante"] = htmlspecialchars(GetData($row,"solicitante",$format));
			$format="";
			$values["Nome"] = htmlspecialchars(GetData($row,"Nome",$format));
			$format="";
			$values["Numero"] = htmlspecialchars(GetData($row,"Numero",$format));
		$values["idt_custo"] = DisplayLookupWizard("idt_custo",$row["idt_custo"],$row,"",MODE_EXPORT);
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["empresa"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["flg_show_mobile"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["solicitante"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Nome"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Numero"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["idt_custo"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Empresa").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Adicionar a lista de contatos (iPBX Mobile)").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Solicitante").'</td>';	
	...
	{
		echo "<td>"."Empresa"."</td>";
		echo "<td>"."Adicionar a lista de contatos (iPBX Mobile)"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["empresa"] = GetData($row,"empresa",$format);
					
							$format=FORMAT_NONE;
			
			$values["flg_show_mobile"] = GetData($row,"flg_show_mobile",$format);
					
							$format="";
			
			$values["solicitante"] = GetData($row,"solicitante",$format);
					
							$format="";
			
			$values["Nome"] = GetData($row,"Nome",$format);
					
							$format="";
			
			$values["Numero"] = GetData($row,"Numero",$format);
					$values["idt_custo"] = "";	
			if(strlen($row["idt_custo"]))
			{
				$values["idt_custo"] = DisplayLookupWizard("idt_custo", $row["idt_custo"], $row,"",MODE_EXPORT);
			}
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["empresa"]);
					else
						echo htmlspecialchars($values["empresa"]);
			echo '</td>';
							echo '<td>';
						
			
									$format=FORMAT_NONE;
									echo htmlspecialchars($values["flg_show_mobile"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["solicitante"]);
					else
						echo htmlspecialchars($values["solicitante"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
			...
			echo '</td>';
					...
					echo '<td>';
						...
						echo htmlspecialchars($values["Numero"]);
			echo '</td>';
							echo '<td>';
						
				
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["idt_custo"]);
				else
					echo htmlspecialchars($values["idt_custo"]);
