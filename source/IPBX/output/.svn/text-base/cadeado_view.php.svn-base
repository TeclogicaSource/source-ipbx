...
$fieldsArr = array();
$arr = array();
$arr['fName'] = "idt_cadeado";
$arr['viewFormat'] = ViewFormat("idt_cadeado", $strTableName);
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "Ramal";
...
//idt_cadeado - 
	$value="";
	$value = ProcessLargeText(GetData($data,"idt_cadeado", ""),"","",MODE_VIEW);
	$xt->assign("idt_cadeado_value",$value);
	$xt->assign("idt_cadeado_fieldblock",true);
////////////////////////////////////////////
//Ramal - 
	$value="";
	$value = ProcessLargeText(GetData($data,"Ramal", ""),"","",MODE_VIEW);
...
$jsKeysObj = 'window.recKeysObj = {';
	$jsKeysObj .= "'".jsreplace("idt_cadeado")."': '".(jsreplace(@$data["idt_cadeado"]))."', ";
...
//Begin show Next Prev button
	...
	if(count($next))
    {
		...
		$xt->assign("nextbutton_attrs","onclick=\"window.location.href='cadeado_view.php?".$nextlink."'\"");
	}
	else 
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","onclick=\"window.location.href='cadeado_view.php?".$prevlink."'\"");
	}
    else 
