...
$templatefile = ( $inlineedit ) ? "ipbx_call_back_inline_edit.htm" : "ipbx_call_back_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_call_back"]=postvalue("editid1");
$savedKeys["id_call_back"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_call_back"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: ipbx_call_back_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_call_back->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_call_back_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	...
	$data["interface"]=$evalues["interface"];
	$data["nm_externo"]=$evalues["nm_externo"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_call_back"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_call_back", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_call_back_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_call_back"))
		$key[]=GetFieldIndex("id_call_back");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_call_back_edit.php','".$skeys."','',function(){window.location.href='ipbx_call_back_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_call_back_edit.php','".$skeys."','',function(){window.location.href='ipbx_call_back_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_call_back"]);
if($message)
{
...
$control_interface["params"]["value"]=@$data["interface"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_interface["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_interface["params"]["mode"]="inline_edit";
else
	$control_interface["params"]["mode"]="edit";
if(!$detailKeys || !in_array("interface", $detailKeys))	
	$xt->assign("interface_editcontrol",$control_interface);
else if(array_key_exists("interface",$data))
{
				$value=DisplayLookupWizard("interface",$data["interface"],$data,"",MODE_VIEW);
...
$control_nm_externo["params"]["value"]=@$data["nm_externo"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_nm_externo["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_nm_externo["params"]["mode"]="inline_edit";
else
	$control_nm_externo["params"]["mode"]="edit";
if(!$detailKeys || !in_array("nm_externo", $detailKeys))	
	$xt->assign("nm_externo_editcontrol",$control_nm_externo);
else if(array_key_exists("nm_externo",$data))
{
				$value = ProcessLargeText(GetData($data,"nm_externo", ""),"field=nm%5Fexterno","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_call_back_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
