...
//	process selection
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_conf_fax"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["id_conf_fax"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
//	id_conf_fax - 
			$value="";
				$value = ProcessLargeText(GetData($data,"id_conf_fax", ""),"field=id%5Fconf%5Ffax".$keylink,"",MODE_PRINT);
			$record["id_conf_fax_value"]=$value;

//	tp_reter_fax - 
			$value="";
				$value = ProcessLargeText(GetData($data,"tp_reter_fax", ""),"field=tp%5Freter%5Ffax".$keylink,"",MODE_PRINT);
			$record["tp_reter_fax_value"]=$value;

//	pasta - 
			$value="";
				$value = ProcessLargeText(GetData($data,"pasta", ""),"field=pasta".$keylink,"",MODE_PRINT);
			$record["pasta_value"]=$value;
			if($col<$colsonpage)
...
$xt->assign("id_conf_fax_fieldheadercolumn",true);
$xt->assign("id_conf_fax_fieldheader",true);
$xt->assign("id_conf_fax_fieldcolumn",true);
$xt->assign("id_conf_fax_fieldfootercolumn",true);
$xt->assign("tp_reter_fax_fieldheadercolumn",true);
$xt->assign("tp_reter_fax_fieldheader",true);
$xt->assign("tp_reter_fax_fieldcolumn",true);
$xt->assign("tp_reter_fax_fieldfootercolumn",true);
$xt->assign("pasta_fieldheadercolumn",true);
$xt->assign("pasta_fieldheader",true);
$xt->assign("pasta_fieldcolumn",true);
$xt->assign("pasta_fieldfootercolumn",true);
...
$templatefile = "ipbx_conf_fax_print.htm";
