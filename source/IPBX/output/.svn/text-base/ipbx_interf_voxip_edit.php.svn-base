...
include("include/ipbx_interf_voxip_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_interf_voxip_inline_edit.htm" : "ipbx_interf_voxip_edit.htm";
...
//	processibng id_chamada - end
	...
	if($condition)
	{
	$value = postvalue("value_flg_id_cham_parc_".$id);
	$type=postvalue("type_flg_id_cham_parc_".$id);
	if(FieldSubmitted("flg_id_cham_parc_".$id))
	{
		
		$value=prepare_for_db("flg_id_cham_parc",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "flg_id_cham_parc"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["flg_id_cham_parc"]=$value;
		
		
	}


//	processibng flg_id_cham_parc - end
	}
	$condition = $inlineedit;
...
//	processibng cd_operadora - end
	...
	if($condition)
	{
	$value = postvalue("value_qtd_cham_por_ramal_".$id);
	$type=postvalue("type_qtd_cham_por_ramal_".$id);
	if(FieldSubmitted("qtd_cham_por_ramal_".$id))
	{
		
		$value=prepare_for_db("qtd_cham_por_ramal",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "qtd_cham_por_ramal"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["qtd_cham_por_ramal"]=$value;
		
		
	}


//	processibng qtd_cham_por_ramal - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ipbx_interf_voxip_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_interf_voxip->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_interf_voxip_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	...
	$data["id_chamada"]=$evalues["id_chamada"];
	$data["flg_id_cham_parc"]=$evalues["flg_id_cham_parc"];
	$data["cd_operadora"]=$evalues["cd_operadora"];
	$data["qtd_cham_por_ramal"]=$evalues["qtd_cham_por_ramal"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$xt->assign("qtd_cham_por_ramal_fieldblock",true);
	$xt->assign("qtd_cham_por_ramal_label",true);
	if(isEnableSection508())
		$xt->assign_section("qtd_cham_por_ramal_label","<label for=\"".GetInputElementId("qtd_cham_por_ramal", $id)."\">","</label>");
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_voxip_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_interf_voxip_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_voxip_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_interf_voxip_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_voxip_list.php?a=return'});return false;\"");
...
//	qtd_cham_por_ramal - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"qtd_cham_por_ramal", ""),"field=qtd%5Fcham%5Fpor%5Framal".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "qtd_cham_por_ramal";
				$showRawValues[] = substr($data["qtd_cham_por_ramal"],0,100);
...
//	flg_id_cham_parc - Checkbox
		...
		$showValues[] = $value;
		$showFields[] = "flg_id_cham_parc";
				$showRawValues[] = substr($data["flg_id_cham_parc"],0,100);
...
$control_id_chamada["params"]["value"]=@$data["id_chamada"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_id_chamada["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_id_chamada["params"]["mode"]="inline_edit";
else
	$control_id_chamada["params"]["mode"]="edit";
if(!$detailKeys || !in_array("id_chamada", $detailKeys))	
	$xt->assign("id_chamada_editcontrol",$control_id_chamada);
else if(array_key_exists("id_chamada",$data))
{
				$value = ProcessLargeText(GetData($data,"id_chamada", ""),"field=id%5Fchamada","",MODE_VIEW);
		$xt->assign("id_chamada_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - flg_id_cham_parc
$control_flg_id_cham_parc=array();
$control_flg_id_cham_parc["func"]="xt_buildeditcontrol";
$control_flg_id_cham_parc["params"] = array();
$control_flg_id_cham_parc["params"]["field"]="flg_id_cham_parc";
$control_flg_id_cham_parc["params"]["value"]=@$data["flg_id_cham_parc"];
//	Begin Add validation
$arrValidate = array();	


$control_flg_id_cham_parc["params"]["validate"]=$arrValidate;
//	End Add validation
$control_flg_id_cham_parc["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_flg_id_cham_parc["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_flg_id_cham_parc["params"]["mode"]="inline_edit";
else
	$control_flg_id_cham_parc["params"]["mode"]="edit";
if(!$detailKeys || !in_array("flg_id_cham_parc", $detailKeys))	
	$xt->assign("flg_id_cham_parc_editcontrol",$control_flg_id_cham_parc);
else if(array_key_exists("flg_id_cham_parc",$data))
{
				$value = GetData($data,"flg_id_cham_parc", "Checkbox");
		$xt->assign("flg_id_cham_parc_editcontrol",$value);
}
...
$control_cd_operadora["params"]["value"]=@$data["cd_operadora"];
//	Begin Add validation
$arrValidate = array();	
$validatetype = getJsValidatorName("Number");
$arrValidate['basicValidate'][] = $validatetype;
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_cd_operadora["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_cd_operadora["params"]["mode"]="inline_edit";
else
	$control_cd_operadora["params"]["mode"]="edit";
if(!$detailKeys || !in_array("cd_operadora", $detailKeys))	
	$xt->assign("cd_operadora_editcontrol",$control_cd_operadora);
else if(array_key_exists("cd_operadora",$data))
{
				$value = ProcessLargeText(GetData($data,"cd_operadora", ""),"field=cd%5Foperadora","",MODE_VIEW);
		$xt->assign("cd_operadora_editcontrol",$value);
}


// add prevent submit on enter js if only one text record
//	control - qtd_cham_por_ramal
$control_qtd_cham_por_ramal=array();
$control_qtd_cham_por_ramal["func"]="xt_buildeditcontrol";
$control_qtd_cham_por_ramal["params"] = array();
$control_qtd_cham_por_ramal["params"]["field"]="qtd_cham_por_ramal";
$control_qtd_cham_por_ramal["params"]["value"]=@$data["qtd_cham_por_ramal"];
//	Begin Add validation
$arrValidate = array();	
$validatetype = getJsValidatorName("Number");
$arrValidate['basicValidate'][] = $validatetype;
...
$control_qtd_cham_por_ramal["params"]["validate"]=$arrValidate;
//	End Add validation
$control_qtd_cham_por_ramal["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_qtd_cham_por_ramal["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_qtd_cham_por_ramal["params"]["mode"]="inline_edit";
else
	$control_qtd_cham_por_ramal["params"]["mode"]="edit";
if(!$detailKeys || !in_array("qtd_cham_por_ramal", $detailKeys))	
	$xt->assign("qtd_cham_por_ramal_editcontrol",$control_qtd_cham_por_ramal);
else if(array_key_exists("qtd_cham_por_ramal",$data))
{
				$value = ProcessLargeText(GetData($data,"qtd_cham_por_ramal", ""),"field=qtd%5Fcham%5Fpor%5Framal","",MODE_VIEW);
		$xt->assign("qtd_cham_por_ramal_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_interf_voxip_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
