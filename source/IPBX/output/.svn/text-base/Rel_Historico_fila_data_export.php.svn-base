...
$pageObject->body["begin"] .="<form action=\"Rel_Historico_fila_data_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel_Historico_fila_data_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Rel_Historico_fila_data.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Rel_Historico_fila_data.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel_Historico_fila_data.xml");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel_Historico_fila_data.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="Number";
			$values["Agent.Disp"] = htmlspecialchars($row["Agent.Disp"]);
			$format="Number";
			$values["AgentesPausa"] = htmlspecialchars($row["AgentesPausa"]);
			$format="";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Agent.Disp").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Agentes Pausa").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Chamadas Atendidas").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Chamadas Abandonadas").'</td>';	
	}
	else
	{
		echo "<td>"."Fila"."</td>";
		echo "<td>"."Data"."</td>";
		echo "<td>"."Agent.Disp"."</td>";
		echo "<td>"."Agentes Pausa"."</td>";
		echo "<td>"."Chamadas Atendidas"."</td>";
		echo "<td>"."Chamadas Abandonadas"."</td>";
	}
	echo "</tr>";
