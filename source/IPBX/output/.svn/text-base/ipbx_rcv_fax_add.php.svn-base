...
include("include/ipbx_rcv_fax_variables.php");
...
if($inlineadd==ADD_INLINE)
	$templatefile = "ipbx_rcv_fax_inline_add.htm";
else
	$templatefile = "ipbx_rcv_fax_add.htm";
...
//	processing dt_fax - start
    
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
	$value = postvalue("value_dt_fax_".$id);
	$type=postvalue("type_dt_fax_".$id);
	if (FieldSubmitted("dt_fax_".$id))
	{
		$value=prepare_for_db("dt_fax",$value,$type);
	}
	else
		$value=false;
	if(!($value===false))
	{
		...
		if(0 && "dt_fax"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["dt_fax"]=$value;
	}
	}
//	processibng dt_fax - end
//	processing nm_telefone - start
    
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
	$value = postvalue("value_nm_telefone_".$id);
	$type=postvalue("type_nm_telefone_".$id);
	if (FieldSubmitted("nm_telefone_".$id))
	{
		$value=prepare_for_db("nm_telefone",$value,$type);
	}
	else
		$value=false;
	if(!($value===false))
	{
		...
		if(0 && "nm_telefone"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["nm_telefone"]=$value;
	}
	}
//	processibng nm_telefone - end
//	processing status - start
	...
	if(!($value===false))
	{
		...
		if(0 && "status"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["status"]=$value;
	}
	}
//	processibng status - end
//	processing arq_fax - start
    
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
	$value = postvalue("value_arq_fax_".$id);
	$type=postvalue("type_arq_fax_".$id);
	if (FieldSubmitted("arq_fax_".$id))
	{
	$fileNameForPrepareFunc = postvalue("filename_arq_fax_".$id);
			$value=prepare_upload("arq_fax","upload2",$fileNameForPrepareFunc,$fileNameForPrepareFunc,"" ,$id);
	...
	if(!($value===false))
	{
		...
		if(0 && "arq_fax"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["arq_fax"]=$value;
	}
	}
//	processibng arq_fax - end
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'ipbx_rcv_fax_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'ipbx_rcv_fax_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: ipbx_rcv_fax_".$pageObject->getPageType().".php");
...
if(array_key_exists("copyid1",$_REQUEST) || array_key_exists("editid1",$_REQUEST))
{
	...
	{
		$copykeys["id_rcv_fax"]=postvalue("copyid1");
	}
	else
	{
		$copykeys["id_rcv_fax"]=postvalue("editid1");
	}
...
if($inlineadd!=ADD_INLINE)
{
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='ipbx_rcv_fax_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_rcv_fax_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_rcv_fax_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$showKeys[] = htmlspecialchars($keys["id_rcv_fax"]);

	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_rcv_fax"]));
...
//	dt_fax - Datetime
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"dt_fax", "Datetime"),"field=dt%5Ffax".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dt_fax";
				$showRawValues[] = substr($data["dt_fax"],0,100);
	}	
//	nm_telefone - 
	$display = false;
	if($inlineadd==ADD_INLINE)
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"nm_telefone", ""),"field=nm%5Ftelefone".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "nm_telefone";
				$showRawValues[] = substr($data["nm_telefone"],0,100);
	}	
//	status - 
	$display = false;
	if($inlineadd==ADD_INLINE)
...
//	arq_fax - Document Download
	...
	{	
		$value="";
				$value = GetData($data,"arq_fax", "Document Download");
		$showValues[] = $value;
		$showFields[] = "arq_fax";
				$showRawValues[] = substr($data["arq_fax"],0,100);
	}	
...
//	control - dt_fax
$control_dt_fax=array();
$control_dt_fax["func"]="xt_buildeditcontrol";
$control_dt_fax["params"] = array();
$control_dt_fax["params"]["field"]="dt_fax";
$control_dt_fax["params"]["value"]=@$defvalues["dt_fax"];

//	Begin Add validation
$arrValidate = array();	
$control_dt_fax["params"]["validate"]=$arrValidate;
//	End Add validation

$control_dt_fax["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dt_fax["params"]["mode"]="inline_add";
else
	$control_dt_fax["params"]["mode"]="add";

if(!$detailKeys || !in_array("dt_fax", $detailKeys))
	$xt->assignbyref("dt_fax_editcontrol",$control_dt_fax);
else if(array_key_exists("dt_fax", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dt_fax", "Datetime"),"field=dt%5Ffax","",MODE_VIEW);
		$xt->assign("dt_fax_editcontrol",$value);
}
// add prevent submit on enter js if only one text record
//	control - nm_telefone
$control_nm_telefone=array();
$control_nm_telefone["func"]="xt_buildeditcontrol";
$control_nm_telefone["params"] = array();
$control_nm_telefone["params"]["field"]="nm_telefone";
$control_nm_telefone["params"]["value"]=@$defvalues["nm_telefone"];

//	Begin Add validation
$arrValidate = array();	
$control_nm_telefone["params"]["validate"]=$arrValidate;
//	End Add validation

$control_nm_telefone["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_nm_telefone["params"]["mode"]="inline_add";
else
	$control_nm_telefone["params"]["mode"]="add";

if(!$detailKeys || !in_array("nm_telefone", $detailKeys))
	$xt->assignbyref("nm_telefone_editcontrol",$control_nm_telefone);
else if(array_key_exists("nm_telefone", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"nm_telefone", ""),"field=nm%5Ftelefone","",MODE_VIEW);
		$xt->assign("nm_telefone_editcontrol",$value);
}
// add prevent submit on enter js if only one text record
//	control - status
...
$control_status["params"]["value"]=@$defvalues["status"];

//	Begin Add validation
$arrValidate = array();	
$control_status["params"]["validate"]=$arrValidate;
//	End Add validation

$control_status["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_status["params"]["mode"]="inline_add";
else
	$control_status["params"]["mode"]="add";

if(!$detailKeys || !in_array("status", $detailKeys))
	$xt->assignbyref("status_editcontrol",$control_status);
else if(array_key_exists("status", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"status", ""),"field=status","",MODE_VIEW);
		$xt->assign("status_editcontrol",$value);
}
// add prevent submit on enter js if only one text record
//	control - arq_fax
$control_arq_fax=array();
$control_arq_fax["func"]="xt_buildeditcontrol";
$control_arq_fax["params"] = array();
$control_arq_fax["params"]["field"]="arq_fax";
$control_arq_fax["params"]["value"]=@$defvalues["arq_fax"];

//	Begin Add validation
$arrValidate = array();	
$control_arq_fax["params"]["validate"]=$arrValidate;
//	End Add validation

$control_arq_fax["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_arq_fax["params"]["mode"]="inline_add";
else
	$control_arq_fax["params"]["mode"]="add";

if(!$detailKeys || !in_array("arq_fax", $detailKeys))
	$xt->assignbyref("arq_fax_editcontrol",$control_arq_fax);
else if(array_key_exists("arq_fax", $defvalues))
{
				$value = GetData($defvalues,"arq_fax", "Document Download");
		$xt->assign("arq_fax_editcontrol",$value);
}
// add prevent submit on enter js if only one text record
