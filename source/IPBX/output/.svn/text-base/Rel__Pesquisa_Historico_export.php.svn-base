...
$pageObject->body["begin"] .="<form action=\"Rel__Pesquisa_Historico_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel__Pesquisa_Historico_export.htm");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Pesquisa_Historico.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["nm_ramal"] = GetData($row,"nm_ramal","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Pesquisa_Historico.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			...
			$format="";
			$values["nm_ramal"] = htmlspecialchars(GetData($row,"nm_ramal",$format));
			$format="";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Pesquisa").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Pergunta").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Telefone (usuário)").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Ramal").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Antendente").'</td>';	
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Ordem").'</td>';	
	...
	else
	{
		...
		echo "<td>"."Pesquisa"."</td>";
		echo "<td>"."Pergunta"."</td>";
		echo "<td>"."Telefone (usuário)"."</td>";
		echo "<td>"."Ramal"."</td>";
		echo "<td>"."Antendente"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["nm_ramal"] = GetData($row,"nm_ramal",$format);
