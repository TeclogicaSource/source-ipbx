...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_k_gsm"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"interf_khomp_gsm_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("interf_khomp_gsm_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=interf_khomp_gsm.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=interf_khomp_gsm.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=interf_khomp_gsm.xml");
	...
	{
		$values = array();
			$values["id_k_gsm"] = GetData($row,"id_k_gsm","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=interf_khomp_gsm.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["id_k_gsm"] = htmlspecialchars(GetData($row,"id_k_gsm",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_k_gsm"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id K Gsm").'</td>';	
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["id_k_gsm"] = GetData($row,"id_k_gsm",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["id_k_gsm"]);
			echo '</td>';
							echo '<td>';
