...
$templatefile = ( $inlineedit ) ? "ipbx_interf_sip_generica_inline_edit.htm" : "ipbx_interf_sip_generica_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ipbx_interf_sip_generica_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_interf_sip_generica->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_interf_sip_generica_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	...
	$data["registro"]=$evalues["registro"];
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_sip_generica_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_interf_sip_generica_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_sip_generica_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_interf_sip_generica_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_sip_generica_list.php?a=return'});return false;\"");
...
$control_registro["params"]["value"]=@$data["registro"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_registro["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_registro["params"]["mode"]="inline_edit";
else
	$control_registro["params"]["mode"]="edit";
if(!$detailKeys || !in_array("registro", $detailKeys))	
	$xt->assign("registro_editcontrol",$control_registro);
else if(array_key_exists("registro",$data))
{
				$value = ProcessLargeText(GetData($data,"registro", ""),"field=registro","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_interf_sip_generica_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
