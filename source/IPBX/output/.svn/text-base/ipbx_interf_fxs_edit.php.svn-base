...
include("include/ipbx_interf_fxs_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_interf_fxs_inline_edit.htm" : "ipbx_interf_fxs_edit.htm";
...
//	processibng tp_chamada - end
	...
	if($condition)
	{
	$value = postvalue("value_khomp_interf_".$id);
	$type=postvalue("type_khomp_interf_".$id);
	if(FieldSubmitted("khomp_interf_".$id))
	{
		
		$value=prepare_for_db("khomp_interf",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "khomp_interf"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["khomp_interf"]=$value;
		
		
	}


//	processibng khomp_interf - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ipbx_interf_fxs_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_interf_fxs->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_interf_fxs_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if($readevalues)
{
	...
	$data["khomp_interf"]=$evalues["khomp_interf"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_interf_fxs_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_interf_fxs_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_fxs_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_interf_fxs_edit.php','".$skeys."','',function(){window.location.href='ipbx_interf_fxs_list.php?a=return'});return false;\"");
...
//	detail tables
	$masterquery="mastertable=ipbx%5Finterf%5Ffxs";
...
//	khomp_interf - 
		...
		$value="";
				$value = ProcessLargeText(GetData($data,"khomp_interf", ""),"field=khomp%5Finterf".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "khomp_interf";
				$showRawValues[] = substr($data["khomp_interf"],0,100);
...
// add prevent submit on enter js if only one text record
//	control - khomp_interf
$control_khomp_interf=array();
$control_khomp_interf["func"]="xt_buildeditcontrol";
$control_khomp_interf["params"] = array();
$control_khomp_interf["params"]["field"]="khomp_interf";
$control_khomp_interf["params"]["value"]=@$data["khomp_interf"];
//	Begin Add validation
$arrValidate = array();	

$control_khomp_interf["params"]["validate"]=$arrValidate;
//	End Add validation
$control_khomp_interf["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_khomp_interf["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_khomp_interf["params"]["mode"]="inline_edit";
else
	$control_khomp_interf["params"]["mode"]="edit";
if(!$detailKeys || !in_array("khomp_interf", $detailKeys))	
	$xt->assign("khomp_interf_editcontrol",$control_khomp_interf);
else if(array_key_exists("khomp_interf",$data))
{
				$value = ProcessLargeText(GetData($data,"khomp_interf", ""),"field=khomp%5Finterf","",MODE_VIEW);
		$xt->assign("khomp_interf_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_interf_fxs_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
