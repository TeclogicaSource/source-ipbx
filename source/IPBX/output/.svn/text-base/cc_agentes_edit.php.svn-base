...
$templatefile = "cc_agentes_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["idt_agentes"]=postvalue("editid1");
$savedKeys["idt_agentes"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["idt_agentes"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: cc_agentes_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_cc_agentes->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: cc_agentes_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["idt_agentes"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"idt_agentes", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="cc_agentes_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("idt_agentes"))
		$key[]=GetFieldIndex("idt_agentes");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('cc_agentes_edit.php','".$skeys."','',function(){window.location.href='cc_agentes_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('cc_agentes_edit.php','".$skeys."','',function(){window.location.href='cc_agentes_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["idt_agentes"]);
if($message)
{
...
{
				$value = ProcessLargeText(GetData($data,"tp_atendimento", ""),"field=tp%5Fatendimento","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('cc_agentes_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
