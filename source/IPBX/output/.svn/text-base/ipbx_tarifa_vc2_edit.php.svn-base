...
include("include/ipbx_tarifa_vc2_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_tarifa_vc2_inline_edit.htm" : "ipbx_tarifa_vc2_edit.htm";
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ipbx_tarifa_vc2_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_tarifa_vc2->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_tarifa_vc2_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_tarifa_vc2_edit.php" '.$onsubmit.'>'.
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_tarifa_vc2_edit.php','".$skeys."','',function(){window.location.href='ipbx_tarifa_vc2_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_tarifa_vc2_edit.php','".$skeys."','',function(){window.location.href='ipbx_tarifa_vc2_list.php?a=return'});return false;\"");
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_tarifa_vc2_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
