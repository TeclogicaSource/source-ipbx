...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_ramal_pabx"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["id_ramal_pabx"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"ramal_pabx_autorizado_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("ramal_pabx_autorizado_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=ramal_pabx_autorizado.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=ramal_pabx_autorizado.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=ramal_pabx_autorizado.xml");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=ramal_pabx_autorizado.csv");
