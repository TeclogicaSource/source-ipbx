...
function ExportToXML()
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["TempoPausaNProdutiva"] = GetData($row,"TempoPausaNProdutiva","");
			$values["TempoPausaProdutiva"] = GetData($row,"TempoPausaProdutiva","");
...
// write header
	...
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"TempoPausaNProdutiva\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"TempoPausaProdutiva\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"TempoPausa\"";
	if($outstr!="")
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values["Fila"] = DisplayLookupWizard("Fila",$row["Fila"],$row,"",MODE_EXPORT);
			...
			$format="Time";
			$values["TempoPausaNProdutiva"] = htmlspecialchars(GetData($row,"TempoPausaNProdutiva",$format));
			$format="Time";
			$values["TempoPausaProdutiva"] = htmlspecialchars(GetData($row,"TempoPausaProdutiva",$format));
			$format="Time";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["TempoPausaNProdutiva"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["TempoPausaProdutiva"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Tempo Pausa Obrigatória").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Tempo Pausa Produtiva").'</td>';	
	...
	else
	{
		...
		echo "<td>"."Tempo Pausa Obrigatória"."</td>";
		echo "<td>"."Tempo Pausa Produtiva"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["TempoPausaNProdutiva"] = GetData($row,"TempoPausaNProdutiva",$format);
					
							$format="Time";
			
			$values["TempoPausaProdutiva"] = GetData($row,"TempoPausaProdutiva",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format="Time";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["TempoPausaNProdutiva"]);
					else
						echo htmlspecialchars($values["TempoPausaNProdutiva"]);
			echo '</td>';
							echo '<td>';
									...
									$format="Time";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["TempoPausaProdutiva"]);
					else
						echo htmlspecialchars($values["TempoPausaProdutiva"]);
			echo '</td>';
							echo '<td>';
