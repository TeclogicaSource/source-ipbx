...
//--------------------------------------------------------------------------------------	
	...
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("empresa");
	...
	{
		$firstFieldParams['fName'] = "empresa";
	...
	if(isEnableSection508())
		$xt->assign_section("empresa_label","<label for=\"".GetInputElementId("empresa", $id)."\">","</label>");
	else 
		$xt->assign("empresa_label", true);
	
	$xt->assign("empresa_fieldblock", true);		
	$xt->assignbyref("empresa_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("empresa_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("empresa_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_empresa", $ctrlBlockArr['searchtype']);	
	
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("empresa", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("empresa")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
	$srchFields = $searchObject->getSearchCtrlParams("solicitante");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	if(isEnableSection508())
		$xt->assign_section("solicitante_label","<label for=\"".GetInputElementId("solicitante", $id)."\">","</label>");
	else 
	...
	$suggestJS = $searchControlBuilder->createSearchSuggestJS("solicitante", $id);
	$pageObject->AddJsCode($suggestJS);
	
	$ctrlsMap = $ctrlBlockArr['searchcontrol1'] ? "[0,1]" : "[0]";
	$regBlocksJS .= "searchController".$searchControllerId.".addRegCtrlsBlock('".jsreplace("solicitante")."', ".$pageObject->id.", ".$ctrlsMap.");";
	// search fields data
