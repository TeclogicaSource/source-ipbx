...
include("include/Rel__Simplificado___Centro_Custo_variables.php");
...
$pageObject->body["begin"] .="<form action=\"Rel__Simplificado___Centro_Custo_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel__Simplificado___Centro_Custo_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Simplificado___Centro_Custo.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Simplificado___Centro_Custo.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Simplificado___Centro_Custo.xml");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Simplificado___Centro_Custo.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["idt_custo"] = htmlspecialchars(GetData($row,"idt_custo",$format));
			$format="";
			$values["dsc_centro_cust"] = htmlspecialchars(GetData($row,"dsc_centro_cust",$format));
			$format="Number";
			$values["custo"] = htmlspecialchars($row["custo"]);
			$format="Short Date";
			$values["DataHora"] = htmlspecialchars(GetData($row,"DataHora",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["DataHora"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Desc. Interface").'</td>';	
	}
	else
	{
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["idt_custo"] = GetData($row,"idt_custo",$format);
			...
			$values["DataHora"] = GetData($row,"DataHora",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format="Short Date";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["DataHora"]);
					else
						echo htmlspecialchars($values["DataHora"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
