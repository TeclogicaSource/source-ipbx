...
include("include/cc_receptivo_filas_horario_desv_variables.php");
...
if($inlineadd==ADD_INLINE)
	$templatefile = "cc_receptivo_filas_horario_desv_inline_add.htm";
else
	$templatefile = "cc_receptivo_filas_horario_desv_add.htm";
...
{
	$ids = $id;
	$mKeys["cc_receptivo_filas_horario_desv_ramais"] = GetMasterKeysByDetailTable("cc_receptivo_filas_horario_desv_ramais", $strTableName);
	$dpParams['strTableNames'][] = "cc_receptivo_filas_horario_desv_ramais";
	$dpParams['ids'][] = ++$ids;
	if($inlineadd==ADD_SIMPLE)
	{
		$pageObject->AddJSCode("window.dpObj = new dpInlineOnAddEdit({
			'mTableName':'".jsreplace($strTableName)."',
			'mShortTableName':'cc_receptivo_filas_horario_desv', 
			'mForm':$('#".$formname."'),
			'mPageType':'".PAGE_ADD."',
			'mMessage':'', 
			'mId':".$id.",
...
if(@$_POST["a"]=="added")
{
	...
	$files_move=array();
	$files_save=array();
//	processing dsc_desvio - start
    
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd!=ADD_INLINE;
	if($inlineAddOption)
	{
	$value = postvalue("value_dsc_desvio_".$id);
	$type=postvalue("type_dsc_desvio_".$id);
	if (FieldSubmitted("dsc_desvio_".$id))
	{
		$value=prepare_for_db("dsc_desvio",$value,$type);
	}
	else
		$value=false;
	if(!($value===false))
	{
		...
		if(0 && "dsc_desvio"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
		$avalues["dsc_desvio"]=$value;
	}
	}
//	processibng dsc_desvio - end
...
//	after edit event
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if(GetTableData($strTableName,".edit",false) && $permis['edit'])
					$message .='&nbsp;<a href=\'cc_receptivo_filas_horario_desv_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
				if(GetTableData($strTableName,".view",false) && $permis['search'])
					$message .='&nbsp;<a href=\'cc_receptivo_filas_horario_desv_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				$mesClass = "mes_ok";	
			}
		}
		elseif($inlineadd!=ADD_INLINE)
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: cc_receptivo_filas_horario_desv_".$pageObject->getPageType().".php");
...
if($readavalues)
{
	$defvalues["dsc_desvio"]=@$avalues["dsc_desvio"];
}
//for basic files
$includes="";
...
if($inlineadd!=ADD_INLINE)
{
	...
	$xt->assign("dsc_desvio_fieldblock",true);
	$xt->assign("dsc_desvio_label",true);
	if(isEnableSection508())
		$xt->assign_section("dsc_desvio_label","<label for=\"".GetInputElementId("dsc_desvio", $id)."\">","</label>");
	...
	if($inlineadd!=ADD_ONTHEFLY)
	{
		...
		$xt->assign("backbutton_attrs","onclick=\"window.location.href='cc_receptivo_filas_horario_desv_list.php?a=return'\"");
		...
		$xt->assign('addForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="cc_receptivo_filas_horario_desv_add.php" '.$onsubmit.'>'.		
	...
	else
	{
		...
		$pageObject->body["begin"] .='<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="cc_receptivo_filas_horario_desv_add.php" '.$onsubmit.' target="flyframe'.$id.'">'.
		'<input type=hidden name="a" value="added">'.
...
{
	$LookupSQL="";
	if(postvalue("table")=="cc_receptivo_filas" && postvalue("field")=="id_desvio") 
	{
	$linkfield = "id_desvio";
	$dispfield = "dsc_desvio";
	$LookupSQL = "select ";
	$LookupSQL .= "`id_desvio`";
		$LookupSQL .= ",`dsc_desvio`";
	}
	if($LookupSQL)
...
if(@$_POST["a"]=="added" && ($inlineadd==ADD_INLINE || $inlineadd==ADD_MASTER)) 
{
	...
	$masterquery="mastertable=cc%5Freceptivo%5Ffilas%5Fhorario%5Fdesv";
	$masterquery.="&masterkey1=".rawurlencode($data["id_desvio"]);
	$showDetailKeys["cc_receptivo_filas_horario_desv_ramais"]=$masterquery;
...
//	dsc_desvio - 
	...
	{	
		$value="";
				$value = ProcessLargeText(GetData($data,"dsc_desvio", ""),"field=dsc%5Fdesvio".$keylink,"",MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "dsc_desvio";
				$showRawValues[] = substr($data["dsc_desvio"],0,100);
	}	
...
$regextype = '';
//	control - dsc_desvio
$control_dsc_desvio=array();
$control_dsc_desvio["func"]="xt_buildeditcontrol";
$control_dsc_desvio["params"] = array();
$control_dsc_desvio["params"]["field"]="dsc_desvio";
$control_dsc_desvio["params"]["value"]=@$defvalues["dsc_desvio"];

//	Begin Add validation
$arrValidate = array();	
$control_dsc_desvio["params"]["validate"]=$arrValidate;
//	End Add validation

$control_dsc_desvio["params"]["id"]=$id;
if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY)
	$control_dsc_desvio["params"]["mode"]="inline_add";
else
	$control_dsc_desvio["params"]["mode"]="add";

if(!$detailKeys || !in_array("dsc_desvio", $detailKeys))
	$xt->assignbyref("dsc_desvio_editcontrol",$control_dsc_desvio);
else if(array_key_exists("dsc_desvio", $defvalues))
{
				$value = ProcessLargeText(GetData($defvalues,"dsc_desvio", ""),"field=dsc%5Fdesvio","",MODE_VIEW);
		$xt->assign("dsc_desvio_editcontrol",$value);
}
