...
if(!$all)
{
	$keys=array();
	$strWhereClause="";
	$keys["accountcode"]=postvalue("editid1");
...
$fieldsArr = array();
$arr = array();
$arr['fName'] = "dsc_centro_cust";
$arr['viewFormat'] = ViewFormat("dsc_centro_cust", $strTableName);
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "idt_colab";
$arr['viewFormat'] = ViewFormat("idt_colab", $strTableName);
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "accountcode";
$arr['viewFormat'] = ViewFormat("accountcode", $strTableName);
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "idt_custo";
$arr['viewFormat'] = ViewFormat("idt_custo", $strTableName);
$fieldsArr[] = $arr;
...
while($data)
{
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"accountcode", "")));

	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["accountcode"]));

////////////////////////////////////////////
//dsc_centro_cust - 
	$value="";
	$value = ProcessLargeText(GetData($data,"dsc_centro_cust", ""),"","",MODE_VIEW);
	$xt->assign("dsc_centro_cust_value",$value);
	$xt->assign("dsc_centro_cust_fieldblock",true);
////////////////////////////////////////////
//idt_colab - 
	$value="";
	$value = ProcessLargeText(GetData($data,"idt_colab", ""),"","",MODE_VIEW);
	$xt->assign("idt_colab_value",$value);
	$xt->assign("idt_colab_fieldblock",true);
////////////////////////////////////////////
//accountcode - 
	$value="";
	$value = ProcessLargeText(GetData($data,"accountcode", ""),"","",MODE_VIEW);
	$xt->assign("accountcode_value",$value);
	$xt->assign("accountcode_fieldblock",true);
////////////////////////////////////////////
//idt_custo - 
	$value="";
	$value = ProcessLargeText(GetData($data,"idt_custo", ""),"","",MODE_VIEW);
	$xt->assign("idt_custo_value",$value);
	$xt->assign("idt_custo_fieldblock",true);
 


$jsKeysObj = 'window.recKeysObj = {';
	$jsKeysObj .= "'".jsreplace("accountcode")."': '".(jsreplace(@$data["accountcode"]))."', ";
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	$where=$_SESSION[$strTableName."_where"];
		if(GetFieldIndex("accountcode"))
			$key[]=GetFieldIndex("accountcode");
...
//Begin show Next Prev button
	...
	if(count($next))
    {
		...
		$xt->assign("nextbutton_attrs","onclick=\"window.location.href='centrocusto_view.php?".$nextlink."'\"");
	}
	else 
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","onclick=\"window.location.href='centrocusto_view.php?".$prevlink."'\"");
	}
    else 
...
$templatefile = "centrocusto_view.htm";
