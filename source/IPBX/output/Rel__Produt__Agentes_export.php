...
include("include/Rel__Produt__Agentes_variables.php");
...
$pageObject->body["begin"] .="<form action=\"Rel__Produt__Agentes_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Rel__Produt__Agentes_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Produt__Agentes.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Produt__Agentes.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Produt__Agentes.xml");
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		$values = array();
			...
			$values["tot_pausa_n_produtiva"] = GetData($row,"tot_pausa_n_produtiva","");
			$values["tot_pausa_produtiva"] = GetData($row,"tot_pausa_produtiva","");
			$values["tot_pausa"] = GetData($row,"tot_pausa","");
			$values["tot_parado"] = GetData($row,"tot_parado","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Rel__Produt__Agentes.csv");
...
// write header
	...
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"tot_pausa_n_produtiva\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"tot_pausa_produtiva\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"tot_pausa\"";
	if($outstr!="")
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values["Agente"] = DisplayLookupWizard("Agente",$row["Agente"],$row,"",MODE_EXPORT);
			...
			$format="Time";
			$values["tot_pausa_n_produtiva"] = htmlspecialchars(GetData($row,"tot_pausa_n_produtiva",$format));
			$format="Time";
			$values["tot_pausa_produtiva"] = htmlspecialchars(GetData($row,"tot_pausa_produtiva",$format));
			$format="Time";
			$values["tot_pausa"] = htmlspecialchars(GetData($row,"tot_pausa",$format));
			$format="Time";
			$values["tot_parado"] = htmlspecialchars(GetData($row,"tot_parado",$format));
			$format="";
			...
			$format="Number";
			$values["cntrb_indiv"] = htmlspecialchars($row["cntrb_indiv"]);
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["tot_pausa_n_produtiva"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["tot_pausa_produtiva"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["tot_pausa"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["tot_parado"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Total Disponível").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Atendidas").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Total Atendimento").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Média Atendimento").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Total Pausa não Produt.").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Total Pausa Produt.").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Total Pausa").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Total Livre").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Rejeitadas/N Atendidas").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Recebidas").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Contribuição Individual").'</td>';	
	...
	else
	{
		...
		echo "<td>"."Fila"."</td>";
		echo "<td>"."Agente"."</td>";
		echo "<td>"."Total Disponível"."</td>";
		echo "<td>"."Atendidas"."</td>";
		echo "<td>"."Total Atendimento"."</td>";
		echo "<td>"."Média Atendimento"."</td>";
		echo "<td>"."Total Pausa não Produt."."</td>";
		echo "<td>"."Total Pausa Produt."."</td>";
		echo "<td>"."Total Pausa"."</td>";
		echo "<td>"."Total Livre"."</td>";
		echo "<td>"."Rejeitadas/N Atendidas"."</td>";
		echo "<td>"."Recebidas"."</td>";
		echo "<td>"."Contribuição Individual"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["tot_pausa_n_produtiva"] = GetData($row,"tot_pausa_n_produtiva",$format);
					
							$format="Time";
			
			$values["tot_pausa_produtiva"] = GetData($row,"tot_pausa_produtiva",$format);
			...
			$values["tot_parado"] = GetData($row,"tot_parado",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
							echo '<td>';
									...
									$format="Time";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["tot_pausa_n_produtiva"]);
					else
						echo htmlspecialchars($values["tot_pausa_n_produtiva"]);
			echo '</td>';
							echo '<td>';
									...
									$format="Time";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["tot_pausa_produtiva"]);
					else
						echo htmlspecialchars($values["tot_pausa_produtiva"]);
			echo '</td>';
							echo '<td>';
			...
			echo '</td>';
							echo '<td>';
									...
									$format="Time";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["tot_parado"]);
					else
						echo htmlspecialchars($values["tot_parado"]);
			echo '</td>';
							echo '<td>';
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["cntrb_tot"]);
					else
