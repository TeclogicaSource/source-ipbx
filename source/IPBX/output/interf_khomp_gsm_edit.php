...
include("include/interf_khomp_gsm_variables.php");
...
$templatefile = ( $inlineedit ) ? "interf_khomp_gsm_inline_edit.htm" : "interf_khomp_gsm_edit.htm";
...
$savedKeys=array();
$skeys="";
$keys["id_k_gsm"]=postvalue("editid1");
$savedKeys["id_k_gsm"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_k_gsm"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: interf_khomp_gsm_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_interf_khomp_gsm->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: interf_khomp_gsm_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_k_gsm"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_k_gsm", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="interf_khomp_gsm_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_k_gsm"))
		$key[]=GetFieldIndex("id_k_gsm");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('interf_khomp_gsm_edit.php','".$skeys."','',function(){window.location.href='interf_khomp_gsm_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('interf_khomp_gsm_edit.php','".$skeys."','',function(){window.location.href='interf_khomp_gsm_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_k_gsm"]);
if($message)
{
...
//	detail tables
	...
	$keylink="";
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["id_k_gsm"]));


//	id_contrato - 

		$value="";
				$value = ProcessLargeText(GetData($data,"id_contrato", ""),"field=id%5Fcontrato".$keylink,"",MODE_LIST);
		$showValues[] = $value;
...
{
				$value = ProcessLargeText(GetData($data,"id_contrato", ""),"field=id%5Fcontrato","",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('interf_khomp_gsm_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
