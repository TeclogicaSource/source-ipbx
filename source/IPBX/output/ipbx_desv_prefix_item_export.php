...
//	process selection
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["id_desv_pref_item"]=refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$selected_recs[]=$keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			if(count($arr)<1)
				continue;
			$keys=array();
			$keys["id_desv_pref_item"]=urldecode($arr[0]);
			$selected_recs[]=$keys;
		}
	}
...
$pageObject->body["begin"] .="<form action=\"ipbx_desv_prefix_item_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("ipbx_desv_prefix_item_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_desv_prefix_item.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_desv_prefix_item.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_desv_prefix_item.xml");
	...
	{
		$values = array();
			$values["id_desv_pref_item"] = GetData($row,"id_desv_pref_item","");
			$values["id_desv_prefix"] = GetData($row,"id_desv_prefix","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=ipbx_desv_prefix_item.csv");
...
// write header
	...
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"id_desv_pref_item\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"id_desv_prefix\"";
	if($outstr!="")
		$outstr.=",";
	$outstr.= "\"prefixo\"";
	echo $outstr;
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();
			$format="";
			$values["id_desv_pref_item"] = htmlspecialchars(GetData($row,"id_desv_pref_item",$format));
			$format="";
			$values["id_desv_prefix"] = htmlspecialchars(GetData($row,"id_desv_prefix",$format));
			$format="";
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_desv_pref_item"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["id_desv_prefix"]).'"';
			if($outstr!="")
				$outstr.=",";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Desv Pref Item").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Id Desv Prefix").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Prefixo").'</td>';	
	}
	else
	{
		echo "<td>"."Id Desv Pref Item"."</td>";
		echo "<td>"."Id Desv Prefix"."</td>";
		echo "<td>"."Prefixo"."</td>";
	}
	echo "</tr>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["id_desv_pref_item"] = GetData($row,"id_desv_pref_item",$format);
					
							$format="";
			
			$values["id_desv_prefix"] = GetData($row,"id_desv_prefix",$format);
		...
		if ($eventRes)
		{
			...
			echo "<tr>";
							...
							echo '<td>';
									...
									$format="";
									echo htmlspecialchars($values["id_desv_pref_item"]);
			echo '</td>';
							echo '<td>';
						
			
									$format="";
									echo htmlspecialchars($values["id_desv_prefix"]);
			echo '</td>';
							if($_REQUEST["type"]=="excel")
					echo '<td x:str>';
				else
