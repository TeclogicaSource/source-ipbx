...
//	regiao - 
			$value="";
				$value = ProcessLargeText(GetData($data,"regiao", ""),"field=regiao".$keylink);
...
//	tp_destino - 
			$value="";
				$value = ProcessLargeText(GetData($data,"tp_destino", ""),"field=tp%5Fdestino".$keylink);
			$xt->assign("tp_destino_mastervalue",$value);
	$strTableName=$oldTableName;
	$xt->display("plano_discagem_regiao_masterlist.htm");
}
