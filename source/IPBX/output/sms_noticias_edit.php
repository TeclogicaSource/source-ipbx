...
$templatefile = "sms_noticias_edit.htm";
...
$savedKeys["id_noticia"]=postvalue("editid1");
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// key get query
	$keyGetQ = "";
		$keyGetQ.="editid1=".rawurldecode($keys["id_noticia"])."&";
	// cut last &
	$keyGetQ = substr($keyGetQ, 0, strlen($keyGetQ)-1);	
	// redirect
	header("Location: sms_noticias_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_sms_noticias->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: sms_noticias_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
//if(!$data["dsc_noticia"])
	$data["dsc_noticia"] = "<nome>, texto informativo...";

if($readevalues)
{
	$data["dsc_noticia"]=$evalues["dsc_noticia"];
	$data["id_grupo"]=$evalues["id_grupo"];
}
/////////////////////////////////////////////////////////////
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["id_noticia"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"id_noticia", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="sms_noticias_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("id_noticia"))
		$key[]=GetFieldIndex("id_noticia");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('sms_noticias_edit.php','".$skeys."','',function(){window.location.href='sms_noticias_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('sms_noticias_edit.php','".$skeys."','',function(){window.location.href='sms_noticias_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["id_noticia"]);
if($message)
{
...
$control_dsc_noticia["params"]["value"]=@$data["dsc_noticia"];
...
$control_dsc_noticia["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_dsc_noticia["params"]["mode"]="inline_edit";
else
	$control_dsc_noticia["params"]["mode"]="edit";
if(!$detailKeys || !in_array("dsc_noticia", $detailKeys))	
	$xt->assign("dsc_noticia_editcontrol",$control_dsc_noticia);
else if(array_key_exists("dsc_noticia",$data))
{
				$value = ProcessLargeText(GetData($data,"dsc_noticia", ""),"field=dsc%5Fnoticia","",MODE_VIEW);
...
$control_id_grupo["params"]["value"]=@$data["id_grupo"];
//	Begin Add validation
$arrValidate = array();	
...
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_id_grupo["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_id_grupo["params"]["mode"]="inline_edit";
else
	$control_id_grupo["params"]["mode"]="edit";
if(!$detailKeys || !in_array("id_grupo", $detailKeys))	
	$xt->assign("id_grupo_editcontrol",$control_id_grupo);
else if(array_key_exists("id_grupo",$data))
{
				$value=DisplayLookupWizard("id_grupo",$data["id_grupo"],$data,"",MODE_VIEW);
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('sms_noticias_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
