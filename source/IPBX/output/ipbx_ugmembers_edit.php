...
include("include/ipbx_ugmembers_variables.php");
...
$templatefile = ( $inlineedit ) ? "ipbx_ugmembers_inline_edit.htm" : "ipbx_ugmembers_edit.htm";
...
$savedKeys["UserName"]=postvalue("editid1");
...
if(@$_POST["a"] == "edited")
{
	...
	if($condition)
	{
	$value = postvalue("value_GroupID_".$id);
	$type=postvalue("type_GroupID_".$id);
	if(FieldSubmitted("GroupID_".$id))
	{
		
		$value=prepare_for_db("GroupID",$value,$type);
	}
	else
	{
		$value=false;
	...
	if(0 && "GroupID"=="secret" && $url_page=="admin_users_")
			$value=md5($value);
	$evalues["GroupID"]=$value;
		
		
	}


//	processibng GroupID - end
	}
...
if ($IsSaved && no_output_done() && !$inlineedit )
{
	...
	// redirect
	header("Location: ipbx_ugmembers_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_ipbx_ugmembers->Copy();
...
if(!$data)
{
	if(!$inlineedit)
	{
		header("Location: ipbx_ugmembers_list.php?a=return");
		exit();
	}
	else
		$data=array();
...
if(!$inlineedit)
{
	...
	$hiddenKeys = '';
	$hiddenKeys .= "<input type=\"hidden\" name=\"editid1\" value=\"".htmlspecialchars($keys["UserName"])."\">";
	$xt->assign("show_key1", htmlspecialchars(GetData($data,"UserName", "")));
	
	$xt->assign('editForm', array('begin'=>'<form name="'.$formname.'" id="'.$formname.'" encType="multipart/form-data" method="post" action="ipbx_ugmembers_edit.php" '.$onsubmit.'>'.
...
if(!@$_SESSION[$strTableName."_noNextPrev"])
{
	...
	if(GetFieldIndex("UserName"))
		$key[]=GetFieldIndex("UserName");
...
//create a line where and order for the two queries
	...
	$resetEditors="";
	$unblRec="UnlockRecord('ipbx_ugmembers_edit.php','".$skeys."','',function(){window.location.href='ipbx_ugmembers_edit.php?";
	if(count($next))
	{
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
	$xt->assign("backbutton_attrs","onclick=\"UnlockRecord('ipbx_ugmembers_edit.php','".$skeys."','',function(){window.location.href='ipbx_ugmembers_list.php?a=return'});return false;\"");
...
$showKeys[] = rawurlencode($keys["UserName"]);
if($message)
{
...
//	UserName - 
		...
		$value="";
				$value=DisplayLookupWizard("UserName",$data["UserName"],$data,$keylink,MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "UserName";
...
//	GroupID - 
		...
		$value="";
				$value=DisplayLookupWizard("GroupID",$data["GroupID"],$data,$keylink,MODE_LIST);
		$showValues[] = $value;
		$showFields[] = "GroupID";
				$showRawValues[] = substr($data["GroupID"],0,100);
/////////////////////////////////////////////////////////////
//	start inline output
/////////////////////////////////////////////////////////////
...
$regextype = '';
//	control - GroupID
$control_GroupID=array();
$control_GroupID["func"]="xt_buildeditcontrol";
$control_GroupID["params"] = array();
$control_GroupID["params"]["field"]="GroupID";
$control_GroupID["params"]["value"]=@$data["GroupID"];
//	Begin Add validation
$arrValidate = array();	


$control_GroupID["params"]["validate"]=$arrValidate;
//	End Add validation
$control_GroupID["params"]["id"]=$id;
$additionalCtrlParams = array();
$additionalCtrlParams["disabled"] = !$enableCtrlsForEditing;
$control_GroupID["params"]["additionalCtrlParams"]=$additionalCtrlParams;
if($inlineedit)
	$control_GroupID["params"]["mode"]="inline_edit";
else
	$control_GroupID["params"]["mode"]="edit";
if(!$detailKeys || !in_array("GroupID", $detailKeys))	
	$xt->assign("GroupID_editcontrol",$control_GroupID);
else if(array_key_exists("GroupID",$data))
{
				$value=DisplayLookupWizard("GroupID",$data["GroupID"],$data,"",MODE_VIEW);
		$xt->assign("GroupID_editcontrol",$value);
}
...
if($lockingObj && $enableCtrlsForEditing)
	$pageObject->AddJSCode("window.timeid".$id."=setInterval( function() {ConfirmLock('ipbx_ugmembers_edit.php','".jsreplace($strTableName)."','".$skeys."',".$id.",'".$inlineedit."');},".($lockingObj->ConfirmTime*1000).");");
