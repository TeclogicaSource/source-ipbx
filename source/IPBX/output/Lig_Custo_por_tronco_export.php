...
$pageObject->body["begin"] .="<form action=\"Lig_Custo_por_tronco_export.php\" method=POST id=frmexport name=frmexport>";
$pageObject->body["end"] .= "</form><script>".$pageObject->PrepareJS()."</script>";
$xt->assignbyref("body",$pageObject->body);

$xt->display("Lig_Custo_por_tronco_export.htm");
...
function ExportToExcel()
{
	...
	header("Content-Disposition: attachment;Filename=Lig_Custo_por_tronco.xls");
...
function ExportToWord()
{
	...
	header("Content-Disposition: attachment;Filename=Lig_Custo_por_tronco.doc");
...
function ExportToXML()
{
	...
	header("Content-Disposition: attachment;Filename=Lig_Custo_por_tronco.xml");
	...
	{
		$values = array();
			$values["Origem"] = DisplayLookupWizard("Origem",$row["Origem"],$row,"",MODE_EXPORT);		
			$values["Mes"] = DisplayLookupWizard("Mes",$row["Mes"],$row,"",MODE_EXPORT);		
			$values["Ano"] = DisplayLookupWizard("Ano",$row["Ano"],$row,"",MODE_EXPORT);		
			$values["Interface"] = DisplayLookupWizard("Interface",$row["Interface"],$row,"",MODE_EXPORT);		
			$values["Custo"] = GetData($row,"Custo","");
			$values["c.origem"] = GetData($row,"c.origem","");
...
function ExportToCSV()
{
	...
	header("Content-Disposition: attachment;Filename=Lig_Custo_por_tronco.csv");
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values["Interface"] = DisplayLookupWizard("Interface",$row["Interface"],$row,"",MODE_EXPORT);
			...
			$format="";
			$values["c.origem"] = htmlspecialchars(GetData($row,"c.origem",$format));
		...
		if ($eventRes)
		{
			...
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Interface"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["Custo"]).'"';
			if($outstr!="")
				$outstr.=",";
			$outstr.='"'.htmlspecialchars($values["c.origem"]).'"';
			echo $outstr;
		}
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Interface").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("Custo").'</td>';	
		echo '<td style="width: 100" x:str>'.PrepareForExcel("C.origem").'</td>';	
	}
	else
	{
		echo "<td>"."Origem"."</td>";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		$values = array();	
			...
			$values["c.origem"] = GetData($row,"c.origem",$format);
		...
		if ($eventRes)
		{
			...
			echo '</td>';
					...
					echo '<td>';
								...
								if($_REQUEST["type"]=="excel")
					echo PrepareForExcel($values["Interface"]);
				else
					echo htmlspecialchars($values["Interface"]);
			...
			echo '</td>';
					...
					echo '<td>';
									...
									$format="";
									if($_REQUEST["type"]=="excel")
						echo PrepareForExcel($values["c.origem"]);
					else
						echo htmlspecialchars($values["c.origem"]);
			echo '</td>';
			echo "</tr>";
		}		
